...
public abstract class AmpereOverlayBase extends AmpereWindow implements IAmpereOverlay {
    ...
    private AmpereContextMenu<?> contextMenu;

    protected AmpereOverlayBase() {
        this(null, 0, 0);
    }

    protected AmpereOverlayBase(String overlayId, int initialWidth, int initialHeight) {
    ...
    public void setBounds(AmpereWindowLayout bounds) {
        ...
        AmpereWindowLayout c = clampToScreen(this, bounds);
        panelX = c.x;
        panelY = c.y;
        panelWidth = c.width;
    ...
    protected void setContextMenu(AmpereContextMenu<?> menu) {
        this.contextMenu = menu;
    }

    protected AmpereContextMenu<?> contextMenu() {
        return contextMenu;
    }
    ...
    protected int hoverBlocked(int mouse, int axis) {
        ...
        return covered ? AmpereOverlayManager.HOVER_BLOCKED_MOUSE : mouse;
    }
    ...
    protected int bodyMouseX(int mx, int my) {
        if (contextMenu != null && contextMenu.isMouseOver(mx, my)) {
            return AmpereOverlayManager.HOVER_BLOCKED_MOUSE;
        }
        return mx;
    }
