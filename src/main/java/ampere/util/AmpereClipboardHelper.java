...
public class AmpereClipboardHelper {
    private static final int CLIPBOARD_VERSION = 1;
    private static final String MACRO_CLIPBOARD_TYPE = "Ampere_macro";
    private static final String MACRO_STEPS_CLIPBOARD_TYPE = "Ampere_macro_steps";

    public static void copyToClipboard(List<AmpereSharedState.QueuedPacket> queue) {
        try {
            CompoundTag rootTag = new CompoundTag();
            ListTag packetList = new ListTag();

            for (AmpereSharedState.QueuedPacket qp : queue) {
            ...
            AmpereClientAddon.LOG.info("[Ampere] Copied {} packets to clipboard", queue.size());

        } catch (Exception e) {
            AmpereClientAddon.LOG.error("[Ampere] Failed to copy packets to clipboard", e);
        }
    }

    public static boolean copyMacroToClipboard(AmpereMacro macro) {
        ...
        try {
            ...
            Minecraft.getInstance().keyboardHandler.setClipboard(base64);
            AmpereClientAddon.LOG.info("[Ampere] Copied macro '{}' to clipboard", macro.name);
            return true;
        } catch (Exception e) {
            AmpereClientAddon.LOG.error("[Ampere] Failed to copy macro to clipboard", e);
            return false;
        }
    }

    public static AmpereMacro pasteMacroFromClipboard() {
        try {
            String base64 = Minecraft.getInstance().keyboardHandler.getClipboard();
            if (base64 == null || base64.trim().isEmpty()) {
                AmpereClientAddon.LOG.warn("[Ampere] Macro clipboard is empty");
                return null;
            }
            ...
            if (version != CLIPBOARD_VERSION) {
                AmpereClientAddon.LOG.error("[Ampere] Unsupported macro clipboard version: {}", version);
                return null;
            }

            String type = rootTag.getStringOr("type", "");
            if (!MACRO_CLIPBOARD_TYPE.equals(type)) {
                AmpereClientAddon.LOG.warn("[Ampere] Clipboard data is not a Ampere macro payload");
                return null;
            }

            CompoundTag macroTag = rootTag.getCompound("macro").orElse(new CompoundTag());
            if (macroTag.isEmpty()) {
                AmpereClientAddon.LOG.warn("[Ampere] Macro clipboard payload was empty");
                return null;
            }

            return new AmpereMacro().fromTag(macroTag).sanitizeForSharing();
        } catch (Exception e) {
            AmpereClientAddon.LOG.error("[Ampere] Failed to paste macro from clipboard", e);
            return null;
        }
    }
    ...
    public static boolean copyMacroStepsToClipboard(List<MacroAction> actions) {
        ...
        try {
            ...
            Minecraft.getInstance().keyboardHandler.setClipboard(base64);
            AmpereClientAddon.LOG.info("[Ampere] Copied {} macro steps to clipboard", actionList.size());
            return true;
        } catch (Exception e) {
            AmpereClientAddon.LOG.error("[Ampere] Failed to copy macro steps to clipboard", e);
            return false;
        }
    }
        ...
        try {
            ...
            for (int i = 0; i < actionList.size(); i++) {
                if (actionList.get(i) instanceof CompoundTag actionTag) {
                    MacroAction action = AmpereMacro.createActionFromTag(actionTag);
                    if (action != null) {
            ...
            AmpereClientAddon.LOG.error("[Ampere] Failed to paste macro steps from clipboard", e);
            return null;
        }
    }

    public static List<AmpereSharedState.QueuedPacket> pasteFromClipboard() {
        try {
            String base64 = Minecraft.getInstance().keyboardHandler.getClipboard();
            if (base64 == null || base64.trim().isEmpty()) {
                AmpereClientAddon.LOG.warn("[Ampere] Clipboard is empty");
                return null;
            }
            ...
            int version = rootTag.getIntOr("version", 0);
            if (version != 1) {
                AmpereClientAddon.LOG.error("[Ampere] Unsupported clipboard format version: {}", version);
                return null;
            }

            ListTag packetList = (ListTag) rootTag.get("packets");
            List<AmpereSharedState.QueuedPacket> queue = new ArrayList<>();

            for (int i = 0; i < packetList.size(); i++) {
                CompoundTag packetTag = (CompoundTag) packetList.get(i);
                AmpereSharedState.QueuedPacket qp = deserializeQueuedPacket(packetTag);
                if (qp != null) {
                    queue.add(qp);
                }
            }

            AmpereClientAddon.LOG.info("[Ampere] Pasted {} packets from clipboard", queue.size());
            return queue.isEmpty() ? null : queue;

        } catch (Exception e) {
            AmpereClientAddon.LOG.error("[Ampere] Failed to paste packets from clipboard", e);
            return null;
        }
    }
    ...
    public static Packet<?> deserializePacketFromBase64(String base64) {
        try {
            ...
            if (tag.contains("packets")) {
                ...
                AmpereSharedState.QueuedPacket qp = deserializeQueuedPacket(tag);
                return qp != null ? qp.packet : null;
            } else {
            ...
            AmpereClientAddon.LOG.error("[Ampere] Failed to deserialize packet from Base64", e);
            return null;
        }
    }

    public static CompoundTag serializeQueuedPacket(AmpereSharedState.QueuedPacket qp) {
        try {
            ...
            AmpereClientAddon.LOG.error("[Ampere] Failed to serialize queued packet", e);
            return null;
        }
    }

    public static AmpereSharedState.QueuedPacket deserializeQueuedPacket(CompoundTag tag) {
        try {
            int delay = tag.getIntOr("delay", 0);
            int id = tag.getIntOr("id", 0);
            AmpereSharedState.ReplayMode replayMode = AmpereSharedState.ReplayMode.REGENERATE;
            if (tag.contains("replayMode")) {
                try {
                    replayMode = AmpereSharedState.ReplayMode.valueOf(tag.getStringOr("replayMode", "REGENERATE"));
                } catch (IllegalArgumentException ignored) {
                    replayMode = AmpereSharedState.ReplayMode.REGENERATE;
                }
            }
            ...
            if (packet != null) {
                return id > 0
                    ? new AmpereSharedState.QueuedPacket(packet, delay, id, replayMode)
                    : new AmpereSharedState.QueuedPacket(packet, delay, replayMode);
            }

            return null;
        } catch (Exception e) {
            AmpereClientAddon.LOG.error("[Ampere] Failed to deserialize queued packet", e);
            return null;
        }
    }
    ...
    private static CompoundTag serializePacket(Packet<?> packet) {
        ...
        String friendlyName = AmperePacketRegistry.getName((Class<? extends Packet<?>>) packet.getClass());
        ...
        if (packet instanceof ServerboundCustomPayloadPacket) {
            ...
            AmperePayloadSupport.PayloadSnapshot snapshot = AmperePayloadSupport.snapshot(packet, "C2S");
            if (snapshot != null) {
    ...
    private static Packet<?> deserializePacket(CompoundTag tag) {
        try {
            if (tag.getBooleanOr("customPayloadPacket", false)) {
                ...
                if (!channel.isBlank()) {
                    return AmperePayloadSupport.createC2SPacket(channel, bytes);
                }
            }
            ...
            try {
                ...
                if (tag.contains("friendlyName")) {
                    ...
                    packetClass = AmperePacketRegistry.getPacket(friendlyName);
                }
            }

            if (packetClass == null) {
                AmpereClientAddon.LOG.error("[Ampere] Could not find packet class: {}", className);
                return null;
            }
            ...
            if (!AmperePacketRegistry.getC2SPackets().contains(packetClass)) {
                AmpereClientAddon.LOG.warn("[Ampere] Deserialized packet {} is NOT a registered C2S packet! It may be skipped when sending.", className);
            }

            return packet;

        } catch (Exception e) {
            AmpereClientAddon.LOG.error("[Ampere] Failed to deserialize packet", e);
            return null;
        }
    }
        ...
        while (clazz != null && clazz != Object.class) {
            for (Field field : clazz.getDeclaredFields()) {
                ...
                try {
                    ...
                    AmpereClientAddon.LOG.warn("[Ampere] Failed to serialize field: {}", field.getName(), e);
                }
            }
        ...
        while (clazz != null && clazz != Object.class) {
            for (Field field : clazz.getDeclaredFields()) {
                ...
                try {
                    ...
                    AmpereClientAddon.LOG.warn("[Ampere] Failed to deserialize field: {}", fieldName, e);
                }
            }
            clazz = clazz.getSuperclass();
        }
    ...
    private static Object getSafeDefault(Class<?> type, String fieldName) {
        ...
        if (type == CustomPacketPayload.class || CustomPacketPayload.class.isAssignableFrom(type)) {
            return new AmperePayloadSupport.RawCustomPacketPayload(Identifier.fromNamespaceAndPath("minecraft", "empty"), new byte[0]);
        }
        ...
        try {
            try {
                ...
                if (Modifier.isStatic(emptyField.getModifiers())) {
                    ...
                    if (empty != null) {
                        AmpereClientAddon.LOG.warn("[Ampere] Using {}.EMPTY for unsupported field: {}",
            ...
            try {
                ...
                if (Modifier.isStatic(defaultField.getModifiers())) {
                    ...
                    if (defaultValue != null) {
                        AmpereClientAddon.LOG.warn("[Ampere] Using {}.DEFAULT for unsupported field: {}",
                            type.getSimpleName(), fieldName);
                        return defaultValue;
                    }
                }
            ...
            if (type.isRecord()) {
                ...
                for (int i = 0; i < components.length; i++) {
                    ...
                    if (paramTypes[i] == byte[].class) {
                        ...
                        try {
                            ...
                            AmpereClientAddon.LOG.warn("[Ampere] Failed to create nested Record {}: {}",
                ...
                AmpereClientAddon.LOG.warn("[Ampere] Created default {} for unsupported field: {}",
                    type.getSimpleName(), fieldName);
                return instance;
            }

        } catch (Exception e) {
            AmpereClientAddon.LOG.warn("[Ampere] Could not create safe default for {}: {}",
                type.getSimpleName(), e.getMessage());
        }

        AmpereClientAddon.LOG.error("[Ampere] No safe default found for unsupported type: {} field: {}",
            type.getSimpleName(), fieldName);
        return null;
    }
    ...
    public static String serializeQueueToBase64(List<AmpereSharedState.QueuedPacket> queue) {
        try {
            CompoundTag rootTag = new CompoundTag();
            ListTag packetList = new ListTag();
            ...
            AmpereClientAddon.LOG.error("[Ampere] Failed to serialize queue to Base64", e);
            return null;
        }
    }

    public static List<AmpereSharedState.QueuedPacket> deserializeQueueFromBase64(String base64) {
        try {
            ...
            AmpereClientAddon.LOG.info("[Ampere] Deserialized {} packets from Base64", queue.size());
            return queue.isEmpty() ? null : queue;
        } catch (Exception e) {
            AmpereClientAddon.LOG.error("[Ampere] Failed to deserialize queue from Base64", e);
            return null;
        }
    }
}
