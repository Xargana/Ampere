...
public final class AmpereBookPayloadBuilder {
    ...
    private AmpereBookPayloadBuilder() {
    }
    ...
    public static List<String> randomPages(int requestedPages, int requestedCharsPerPage, String randomType, Random random) {
        ...
        PrimitiveIterator.OfInt chars = RANDOM_ASCII.equals(type)
            ? rng.ints(0x21, 0x80).filter(AmpereBookPayloadBuilder::validBookCodepoint).iterator()
            : rng.ints(0x21, 0xD800).filter(AmpereBookPayloadBuilder::validBookCodepoint).iterator();
    ...
    private static List<String> paperMcPages(Random random) {
        ...
        PrimitiveIterator.OfInt oneByte = rng.ints(0x21, 0x80).filter(AmpereBookPayloadBuilder::validBookCodepoint).iterator();
        PrimitiveIterator.OfInt twoBytes = rng.ints(0x0080, 0x0800).filter(AmpereBookPayloadBuilder::validBookCodepoint).iterator();
        PrimitiveIterator.OfInt threeBytes = rng.ints(0x0800, 0xD800).filter(AmpereBookPayloadBuilder::validBookCodepoint).iterator();
