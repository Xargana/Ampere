...
import ampere.gui.vanillaui.direct.DirectScrollList;
...
public class AmpereLANSyncOverlay extends AmpereOverlayBase {
    private static final Minecraft MC = Minecraft.getInstance();
    private static final CompactTheme PACKUI_THEME = new CompactTheme();
    private static AmpereLANSyncOverlay sharedOverlay;
    ...
    private DirectScrollList<AmpereMacro> sameMacroList = null;
    ...
    private AmpereChatField lanChatField;
    ...
    public AmpereLANSyncOverlay(Font textRenderer) {
        this.textRenderer = textRenderer;
        applyPresetMetrics();
        this.PANEL_WIDTH = PANEL_BASE_WIDTH;
        this.PANEL_HEIGHT = defaultPanelHeight();

        AmpereLANSync sync = AmpereLANSync.getInstance();
    ...
    public static synchronized AmpereLANSyncOverlay getSharedOverlay(Font textRenderer) {
        if (sharedOverlay == null) {
            sharedOverlay = new AmpereLANSyncOverlay(textRenderer);
    ...
    public AmpereWindowLayout getBounds() {
        return new AmpereWindowLayout(panelX, panelY, PANEL_WIDTH, getPanelHeight(), visible, collapsed);
    }
    ...
    private int estimateTabContentHeight(AmpereLANSync sync) {
        switch (activeTab) {
            case 0: {
                int h = 4 + 12 * 4 + 4 + 14;
            ...
            case 2: {
                ...
                if (perUserMode) {
                    ...
                    int macros = AmpereMacroManager.get().getAll().size();
                    h += 14 + (macros == 0 ? 12 : macrolistViewportHeight(macros)) + 8;

                    if (selectedMacroName != null) h += 4 * 12;
                    h += BUTTON_HEIGHT + 6;
                }
                if (ampere.util.macro.MacroExecutor.isVisibleRunning()) h += BUTTON_HEIGHT + 6;
                h += 1 + 6 + 14 + 14;
                return h;
            }
            case 3: {
    ...
    private boolean hasScrollableSessionContent(AmpereLANSync sync) {
    ...
    private void syncPerUserAssignments(AmpereLANSync sync) {
    ...
    private int estimatePerUserListContentHeight(AmpereLANSync sync) {
    ...
    private int estimateExpandedPeerPickerHeight(AmpereLANSync sync, String peer) {
        int contentHeight = 0;
        List<AmpereMacro> myMacroList = AmpereMacroManager.get().getAll();
        java.util.Map<String, Integer> myMacroMap = new java.util.LinkedHashMap<>();
        for (AmpereMacro macro : myMacroList) {
        ...
        Map<String, Map<String, AmpereMacro>> allRemote = sync.getAllRemoteMacros();
        Map<String, AmpereMacro> peerMacroData = allRemote.getOrDefault(peer, java.util.Collections.emptyMap());
        java.util.Map<String, Integer> peerMacroMap = new java.util.LinkedHashMap<>();
        for (Map.Entry<String, AmpereMacro> entry : peerMacroData.entrySet()) {
        ...
        for (String name : peerMacroMap.keySet()) {
            if (myMacroMap.containsKey(name)) {
                int diffs = countActionDifferences(AmpereMacroManager.get().get(name), peerMacroData.get(name));
                if (diffs == 0) {
                    shared.add(name);
                } else {
    ...
    public void render(GuiGraphicsExtractor ctx, int mouseX, int mouseY, float delta) {
        ...
        ampere.gui.vanillaui.UiScissorStack.global().push(ctx,
            ampere.gui.vanillaui.UiBounds.of(panelX, tabContentY, PANEL_WIDTH, actualTabContentH));
            switch (activeTab) {
    ...
    private void renderNotRunning(GuiGraphicsExtractor ctx, int mx, int my, int y) {
        ...
        drawUiText(ctx, "LAN Sync is currently stopped", UiTone.MUTED, AmpereColors.textSecondary(), lx, y + 22);
    }
    ...
    private void renderNoSession(GuiGraphicsExtractor ctx, int mx, int my, int y) {
        ...
        drawUiText(ctx, "Host creates, others search & join", UiTone.MUTED, AmpereColors.textSecondary(), lx, y + 20);
        ...
        clickRegions.add(new ClickRegion(panelX + 8, btnY, halfW, BUTTON_HEIGHT, () -> {
            AmpereLANSync.getInstance().createSession();
        }));
        ...
        clickRegions.add(new ClickRegion(panelX + 12 + halfW, btnY, halfW, BUTTON_HEIGHT, () -> {
            AmpereLANSync.getInstance().startSearching();
        }));
    }

    private void renderSearching(GuiGraphicsExtractor ctx, int mx, int my, int y) {
        int secondsLeft = AmpereLANSync.getInstance().getSearchSecondsRemaining();
        ...
        clickRegions.add(new ClickRegion(panelX + 30, btnY, PANEL_WIDTH - 60, BUTTON_HEIGHT, () -> {
            AmpereLANSync.getInstance().cancelSearch();
        }));
    }

    private void ensureLanChatField(int panelHeight) {
        if (lanChatField == null) {
            lanChatField = new AmpereChatField(MC, textRenderer, panelX + CONTENT_PADDING, panelY, PANEL_WIDTH - (CONTENT_PADDING * 2), CHAT_FIELD_HEIGHT, true);
            lanChatField.setHistoryNavigationEnabled(true);
            lanChatField.setSubmitHandler(message -> AmpereLANSync.getInstance().sendChatMessage(message));
        }
        ...
        boolean enabled = AmpereLANSync.getInstance().isRunning() && AmpereLANSync.getInstance().isInSession();
    ...
    private void renderLanChatField(GuiGraphicsExtractor ctx, int mouseX, int mouseY) {
        ...
        AmpereText.draw(ctx, textRenderer, "Sync Send", AmpereText.Tone.MUTED, lanChatField.getX(), labelY, false);
    ...
    private void renderDashboard(GuiGraphicsExtractor ctx, int mx, int my, int y, int h) {
        ...
        drawUiText(ctx, "Session", UiTone.MUTED, AmpereColors.textSecondary(), lx, cy);
        ...
        drawUiText(ctx, "Role", UiTone.MUTED, AmpereColors.textSecondary(), lx, cy);
        drawUiText(ctx, role, UiTone.BODY, roleColor, lx + 50, cy);
        cy += 12;

        drawUiText(ctx, "Peers", UiTone.MUTED, AmpereColors.textSecondary(), lx, cy);
        ...
        cy += 4;
        AmpereLANSync.SyncState state = sync.getSyncState();
        String stateStr = state.name();
        int stateColor = getSyncStateColor(state);
        drawUiText(ctx, "Sync", UiTone.MUTED, AmpereColors.textSecondary(), lx, cy);
        drawUiText(ctx, stateStr, UiTone.BODY, stateColor, lx + 50, cy);
        cy += 14;

        AmpereLANSync.SpreadResult spread = sync.getLastSpreadResult();
        if (spread != null) {
            String spreadStr = spread.executionMethod == AmpereLANSync.ExecutionMethod.TICK
            ...
            drawUiText(ctx, "Spread", UiTone.MUTED, AmpereColors.textSecondary(), lx, cy);
            ...
            if (spread.lateClient != null && !spread.lateClient.isEmpty() && spread.getSpreadMs() > 0) {
                drawUiText(ctx, " (" + spread.lateClient + ")", UiTone.MUTED, AmpereColors.textDim(),
                    lx + 50 + UiText.width(textRenderer, spreadStr, PACKUI_THEME.fontFor(UiTone.BODY), spreadColor), cy);
            }
            cy += 12;
        }

        List<AmpereLANSync.SpreadResult> history = sync.getSpreadHistory();
        if (!history.isEmpty()) {
            drawUiText(ctx, "History", UiTone.MUTED, AmpereColors.textSecondary(), lx, cy);
            int dotX = lx + 52;
            for (AmpereLANSync.SpreadResult sr : history) {
        ...
        clickRegions.add(new ClickRegion(panelX + CONTENT_PADDING, btnY, PANEL_WIDTH - CONTENT_PADDING * 2, BUTTON_HEIGHT, () -> {
            AmpereLANSync.getInstance().leaveSession();
        }));
    }
    ...
    private void renderPeers(GuiGraphicsExtractor ctx, int mx, int my, int y, int h) {
        ...
        Map<String, AmpereLANSync.ClientInfo> clients = sync.getConnectedClients();
        List<AmpereLANSync.ClientInfo> sortedClients = new ArrayList<>(clients.values());
        sortedClients.sort((a, b) -> Boolean.compare(b.isHost, a.isHost));

        drawUiText(ctx, "Execution Method", UiTone.MUTED, AmpereColors.textSecondary(), lx, cy);
        cy += 12;

        int methodW = (rowWidth - 4) / 2;
        boolean instant = sync.getExecutionMethod() == AmpereLANSync.ExecutionMethod.INSTANT;
        boolean syncIdle = sync.getSyncState() == AmpereLANSync.SyncState.IDLE || sync.getSyncState() == AmpereLANSync.SyncState.DONE;
        ...
        if (canChangeMethod) {
            clickRegions.add(new ClickRegion(lx, cy, methodW, BUTTON_HEIGHT, () -> sync.setExecutionMethod(AmpereLANSync.ExecutionMethod.INSTANT)));
        }
        ...
        if (canChangeMethod) {
            clickRegions.add(new ClickRegion(lx + methodW + 4, cy, rowWidth - methodW - 4, BUTTON_HEIGHT, () -> sync.setExecutionMethod(AmpereLANSync.ExecutionMethod.TICK)));
        }
        cy += BUTTON_HEIGHT + 4;
        ...
        drawUiText(ctx, methodHint, UiTone.MUTED, AmpereColors.textDim(), lx, cy);
        cy += 14;

        for (AmpereLANSync.ClientInfo client : sortedClients) {
            ...
            AmpereLANSync.PeerStatus status = sync.getPeerStatus(client.username);
            int dotColor = status == AmpereLANSync.PeerStatus.HEALTHY ? 0xFF55FF55
                : status == AmpereLANSync.PeerStatus.STALE ? 0xFFFFAA00 : 0xFF888888;
            ...
            int rightReserve = 4;
            if (isSelf) rightReserve += UiText.width(textRenderer, " (YOU)", PACKUI_THEME.fontFor(UiTone.MUTED), AmpereColors.textSecondary()) + 2;
            ...
            String displayName = AmpereText.trimToWidth(textRenderer, client.username, nameMaxW, AmpereText.Tone.BODY);
            ...
            if (isSelf) {
                drawUiText(ctx, "(YOU)", UiTone.MUTED, AmpereColors.textSecondary(), badgeX, cy + 3);
                badgeX += UiText.width(textRenderer, "(YOU)", PACKUI_THEME.fontFor(UiTone.MUTED), AmpereColors.textSecondary()) + 4;
            }
            ...
            if (isSelected) {
                int detailH = 62;
                CompactSurfaces.insetPanel(ctx, lx + 4, cy, panelX + PANEL_WIDTH - CONTENT_PADDING - lx - 8, detailH, AmpereColors.secondary());

                int dx = lx + 10;
                int dy = cy + 4;

                drawUiText(ctx, "Status: " + status.name(), UiTone.BODY,
                    status == AmpereLANSync.PeerStatus.HEALTHY ? 0xFF55FF55 : 0xFFFFAA00, dx, dy);
                dy += 12;

                drawUiText(ctx, isSelf ? "(This is you)" : "Connected peer", UiTone.MUTED, AmpereColors.textDim(), dx, dy);
                dy += 13;

                drawUiText(ctx, "Exec delay", UiTone.MUTED, AmpereColors.textSecondary(), dx, dy);
                dy += 11;
    ...
    private void renderQueue(GuiGraphicsExtractor ctx, int mx, int my, int y, int h) {
        ...
        if (syncIdle) {
            clickRegions.add(new ClickRegion(lx + methodW + 4, cy, btnW - methodW - 4, BUTTON_HEIGHT, () -> sync.setExecutionMethod(AmpereLANSync.ExecutionMethod.TICK)));
        }
        cy += BUTTON_HEIGHT + 4;

        drawUiText(ctx, "My Queue", UiTone.LABEL, 0xFFFFFFFF, lx, cy);
        cy += 12;
        drawUiText(ctx, queueSize + " packets queued", UiTone.BODY, queueSize > 0 ? 0xFF55FF55 : AmpereColors.textDim(), lx, cy);
        cy += 16;
        ...
        if (syncIdle) {
            clickRegions.add(new ClickRegion(lx, cy, btnW, BUTTON_HEIGHT, () ->
                sync.setDelayPacketsSynchronized(!AmpereSharedState.get().shouldDelayGuiPackets())));
        }
        cy += BUTTON_HEIGHT + 8;
        ...
        clickRegions.add(new ClickRegion(lx, cy, btnW, BUTTON_HEIGHT, () -> {
            try {
                String queueData = AmpereClipboardHelper.serializeQueueToBase64(
                    AmpereSharedState.get().getDelayedPackets());
                sync.broadcastQueueSync(queueData);
                AmpereMessaging.sendPrefixed("§aQueue broadcasted to " + (sync.getConnectedCount() - 1) + " peers");
            } catch (Exception e) {
                AmpereMessaging.sendPrefixed("§cFailed to broadcast queue");
            }
        }));
        cy += BUTTON_HEIGHT + 8;
    ...
    private void renderExecute(GuiGraphicsExtractor ctx, int mx, int my, int y, int h) {
        ...
        boolean anyRunning = ampere.util.macro.MacroExecutor.isVisibleRunning();
        if (anyRunning) {
        ...
        if (perUserMode && !setBy.isEmpty()) {
            drawUiText(ctx, "Assigned by: " + setBy, UiTone.MUTED, AmpereColors.textDim(), lx, cy);
            cy += 12;
        }
    ...
    private int renderExecuteSameMacro(GuiGraphicsExtractor ctx, int mx, int my, int cy, int lx, int btnW, AmpereLANSync sync) {
        ...
        if (localMacros.isEmpty()) {
            clearSameMacrolistViewport();
            drawUiText(ctx, "No macros", UiTone.MUTED, AmpereColors.textDim(), lx, cy);
            cy += 12;
        } else {
        ...
        if (selectedMacroName != null) {
            String selectedLine = AmpereText.trimToWidth(textRenderer, "Selected: " + selectedMacroName, btnW, AmpereText.Tone.BODY);
            drawUiText(ctx, selectedLine, UiTone.BODY, 0xFFFFFFFF, lx, cy);
            cy += 12;

            if (!syncStatus.missingPeers.isEmpty()) {
                String missingLine = AmpereText.trimToWidth(
                    textRenderer,
                    "Missing on: " + formatNameSummary(syncStatus.missingPeers),
                    btnW,
                    AmpereText.Tone.BODY
                );
            ...
            if (!syncStatus.differentPeers.isEmpty()) {
                String differentLine = AmpereText.trimToWidth(
                    textRenderer,
        ...
        int shareW = Math.min(112, Math.max(94, UiText.width(textRenderer, "Send To Peers", PACKUI_THEME.fontFor(UiTone.BODY), AmpereColors.textPrimary()) + 12));
    ...
    private int renderExecutePerUser(GuiGraphicsExtractor ctx, int mx, int my, int cy, int lx, int btnW, AmpereLANSync sync) {
        syncPerUserAssignments(sync);
        ...
        ampere.gui.vanillaui.UiScissorStack.global().push(ctx,
            ampere.gui.vanillaui.UiBounds.of(perUserListX + 1, perUserListY + 1,
        ...
        try {
            ...
            for (Map.Entry<String, String> entry : new ArrayList<>(perUserAssignments.entrySet())) {
                ...
                int assignColor = assigned.isEmpty() ? AmpereColors.textDim() : 0xFF55FF55;
                ...
                if (rowVisible) {
                    ...
                    if (rowBottom > rowTop) {
                        ...
                        if (textY < clipBottom - 2) {
                            String truncPeerLabel = AmpereText.trimToWidth(textRenderer, peerLabel, peerNameMaxW, AmpereText.Tone.BODY);
                            drawUiText(ctx, truncPeerLabel, UiTone.BODY, isMe ? AmpereColors.textSecondary() : 0xFFFFFFFF, lx + 10, textY);

                            String truncAssign = AmpereText.trimToWidth(textRenderer, assignLabel, maxAssignW, AmpereText.Tone.BODY);
                ...
                if (isExpanded) {
                    ...
                    if (isMe) {
                        ...
                        Map<String, AmpereMacro> peerMacroData = allRemote.getOrDefault(peer, java.util.Collections.emptyMap());
                        java.util.Map<String, Integer> peerMacroMap = new java.util.LinkedHashMap<>();
                        for (Map.Entry<String, AmpereMacro> e : peerMacroData.entrySet()) {
                        ...
                        for (String name : peerMacroMap.keySet()) {
                            if (myMacroMap.containsKey(name)) {
                                int diffs = countActionDifferences(
                                    AmpereMacroManager.get().get(name), peerMacroData.get(name));
                                if (diffs == 0) {
                                    shared.add(name);
                                } else {
                                    different.put(name, diffs);
                        ...
                        if (shared.isEmpty() && different.isEmpty() && peerOnly.isEmpty() && mineOnly.isEmpty()) {
                            if (cy + rowPitch > clipTop && cy < clipBottom) {
                                ...
                                if (rowBottom > rowTop) {
                                    ...
                                    int textY = Math.max(cy + 3, clipTop + 1);
                                    if (textY < clipBottom - 2) {
                                        drawUiText(ctx, "No macros available", UiTone.MUTED, AmpereColors.textDim(), detailLx, textY);
                                    }
                                }
                            }
                            cy += rowPitch;
        ...
        int shareW = Math.min(112, Math.max(96, UiText.width(textRenderer, "Send Missing", PACKUI_THEME.fontFor(UiTone.BODY), AmpereColors.textPrimary()) + 12));
        int executeX = lx + shareW + 4;
        int executeW = btnW - shareW - 4;
    ...
    private MacroSyncStatus analyzeSameMacroStatus(AmpereLANSync sync, String macroName) {
        ...
        AmpereMacro localMacro = AmpereMacroManager.get().get(macroName);
        if (localMacro == null) {
            return new MacroSyncStatus(missingPeers, differentPeers);
        }
        ...
        for (String peer : peers) {
            ...
            Map<String, AmpereMacro> peerMacros = allRemote.get(peer);
            if (peerMacros == null) continue;
            AmpereMacro remoteMacro = peerMacros.get(macroName);
    ...
    private AssignmentSyncStatus analyzeAssignmentStatus(AmpereLANSync sync) {
    ...
    private int renderPickerRowClipped(GuiGraphicsExtractor ctx, int mx, int my, int cy,
            ...
            String peerKey, AmpereLANSync sync, boolean allowImport,
        ...
        String displayName = AmpereText.trimToWidth(textRenderer, macroName, nameMaxW, AmpereText.Tone.BODY);
        int textY = Math.max(cy + 3, clipTop + 1);
        if (textY < clipBottom - 2) {
            UiText.draw(ctx, textRenderer, displayName, PACKUI_THEME.fontFor(UiTone.BODY), isCurrent ? AmpereColors.rowSelectedText() : 0xFFFFFFFF, tx, textY, false);
        }

        if (badge != null && textY < clipBottom - 2) {
            int badgeX = tx + UiText.width(textRenderer, displayName, PACKUI_THEME.fontFor(UiTone.BODY), isCurrent ? AmpereColors.rowSelectedText() : 0xFFFFFFFF) + 4;
        ...
        if (allowImport) {
            ...
            if (rowVisible) clickRegions.add(new ClickRegion(importX, cy, importW - 4, rowPitch, () -> {
                Map<String, Map<String, AmpereMacro>> allRemote = sync.getAllRemoteMacros();
                Map<String, AmpereMacro> peerMacros = allRemote.get(peerKey);
                AmpereMacro sourceMacro = peerMacros != null ? peerMacros.get(mName) : null;
                if (sourceMacro == null) {
                    AmpereMessaging.sendPrefixed("§cMacro not found: " + mName);
                    return;
                }

                AmpereMacro imported = AmpereMacroManager.get().addImportedCopy(sourceMacro, sourceMacro.name);
                if (imported == null) {
                    AmpereMessaging.sendPrefixed("§cFailed to import macro: " + mName);
                    return;
                }

                AmpereMessaging.sendPrefixed("§aImported macro: " + imported.name);
            }));
        }
        return cy + rowPitch;
    }

    private static int countActionDifferences(AmpereMacro a, AmpereMacro b) {
        if (a == null || b == null) return -1;
        List<ampere.util.macro.MacroAction> aActions = a.actions != null ? a.actions : java.util.Collections.emptyList();
        List<ampere.util.macro.MacroAction> bActions = b.actions != null ? b.actions : java.util.Collections.emptyList();
    ...
    public boolean mouseClicked(double mouseX, double mouseY, int button) {
        ...
        if (button == 0 && mouseX >= panelX && mouseX <= panelX + PANEL_WIDTH
            && mouseY >= panelY && mouseY <= panelY + HEADER_HEIGHT) {
            AmpereWindowLayout bounds = new AmpereWindowLayout(panelX, panelY, PANEL_WIDTH, collapsed ? HEADER_HEIGHT : getPanelHeight(), visible, collapsed);
        ...
        if (button == 0 && activeTab == 2 && !perUserMode && hasSameMacrolistViewport() && sameMacroList.contains(mouseX, mouseY)) {
            ...
            if (sameMacroList.mouseClicked(mouseX, mouseY, button)) {
                ...
                if (!sameMacroList.isScrollbarDragging()) {
                    AmpereMacro selected = sameMacroList.getSelectedItem();
                    if (selected != null) {
    ...
    public boolean mouseDragged(double mouseX, double mouseY, int button, double deltaX, double deltaY) {
        ...
        if (isDragging) {
            AmpereWindowLayout nextBounds = clampToScreen(this,
                new AmpereWindowLayout((int) (mouseX - dragOffsetX), (int) (mouseY - dragOffsetY),
    ...
    private int getSyncStateColor(AmpereLANSync.SyncState state) {
        switch (state) {
            case IDLE: return AmpereColors.textDim();
            ...
            default: return AmpereColors.textSecondary();
        }
    }

    private String formatSyncState(AmpereLANSync.SyncState state, AmpereLANSync sync) {
        switch (state) {
