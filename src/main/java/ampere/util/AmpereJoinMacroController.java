...
public final class AmpereJoinMacroController {
    public enum Timing {
    ...
    private AmpereJoinMacroController() {
    }

    public static Timing timing() {
        return Timing.fromConfig(AmpereConfig.getGlobal().joinMacroTiming);
    }
    ...
    public static TriggerJoin triggerJoin() {
        return normalizeTriggerJoin(TriggerJoin.fromConfig(AmpereConfig.getGlobal().joinMacroTriggerJoin), keepEnabled());
    }
    ...
    public static boolean keepEnabled() {
        return AmpereConfig.getGlobal().joinMacroKeepEnabled;
    }
    ...
    public static String selectedMacroName() {
        String name = AmpereConfig.getGlobal().joinMacroName;
    ...
    private static void executeOnce(Timing timing) {
        ...
        if (macro == null) {
            AmpereNotifications.warning("Join macro missing: " + macroName);
            setSelectedMacro("");
            return;
        }
        ...
        Runnable run = () -> {
            try {
                macro.execute();
                AmpereNotifications.show("Join macro: " + macro.name, 0xFF35D873);
            } catch (Throwable t) {
                AmpereNotifications.error("Join macro failed.");
            }
        };
