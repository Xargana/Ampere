...
import ampere.util.AmpereDropAction;
import ampere.util.AmpereDropHelper;
...
public class ItemAction implements MacroAction, WaitsForGui {
    ...
    private static final AmpereDropAction[] ACTIONS = AmpereDropAction.values();
    private static final int[] ITEM_CLICK_ACTIONS = {
        AmpereDropAction.PICKUP.ordinal(),
        AmpereDropAction.QUICK_MOVE.ordinal(),
        AmpereDropAction.SWAP.ordinal(),
        AmpereDropAction.CLONE.ordinal(),
        AmpereDropAction.DROP_ITEM.ordinal(),
        AmpereDropAction.DROP_STACK.ordinal()
    };
    private static final int[] LEGACY_ITEM_CLICK_ACTIONS = {
        AmpereDropAction.PICKUP.ordinal(),
        AmpereDropAction.PICKUP_ALL.ordinal(),
        AmpereDropAction.QUICK_MOVE.ordinal(),
        AmpereDropAction.SWAP.ordinal(),
        AmpereDropAction.CLONE.ordinal(),
        AmpereDropAction.DROP_ITEM.ordinal(),
    ...
    public AmpereDropAction getItemAction(int index) {
        return ACTIONS[getItemActionIdx(index)];
    }

    public String getItemButtonName(int index) {
        if (getItemAction(index) == AmpereDropAction.PICKUP_ALL) return "All";
        if (getItemAction(index) == AmpereDropAction.SWAP) {
    ...
    public void cycleItemButton(int index) {
        ...
        if (getItemAction(index) == AmpereDropAction.PICKUP_ALL) return;
        int limit = getItemAction(index) == AmpereDropAction.SWAP ? 9 : 3;
    ...
    public void execute(Minecraft mc) {
        ...
        if (!itemNames.isEmpty()) {
            ...
            for (int ei = 0; ei < itemNames.size(); ei++) {
                ...
                AmpereDropAction eiAction = getItemAction(ei);
                ...
                if (eiAction == AmpereDropAction.PICKUP_ALL && useCursorItemForPickupAll) {
                    AmpereCursorClickHelper.click(mc, entryTarget, clickCount);
                    continue;
                }
                int slotToUse;
                if (configuredEntrySlot >= 0 && !entryName.isEmpty()) {
                    int resolvedSlot = AmpereInventoryHelper.resolveConfiguredHandlerSlot(mc, configuredEntrySlot);
                    if (resolvedSlot < 0) continue;
                    slotToUse = findExactMatchingSlot(handler, entryTarget, resolvedSlot);
                } else if (configuredEntrySlot >= 0) {
                    slotToUse = AmpereInventoryHelper.resolveConfiguredHandlerSlot(mc, configuredEntrySlot);
                } else if (useSlot && targetSlot >= 0) {
                    int configuredSlot = AmpereInventoryHelper.resolveConfiguredHandlerSlot(mc, targetSlot);
                ...
                final AmpereDropAction fa = eiAction;
                if (fa == AmpereDropAction.DROP_ITEM || fa == AmpereDropAction.DROP_STACK) {
                    AmpereDropHelper.dropFromHandlerSlot(mc, fs, fa == AmpereDropAction.DROP_STACK ? 0 : clickCount);
                    continue;
                }
                for (int j = 0; j < clickCount; j++) {
                    AmpereInventoryClickHelper.click(mc, fs, fb, fa.toContainerInput());
                }
            }
        } else if (useSlot && targetSlot >= 0) {

            AmpereDropAction action = getAction();
            int effectiveButton = button;
            if (action.usesFixedButton()) {
                effectiveButton = action.getButton();
            }
            if (action == AmpereDropAction.PICKUP_ALL && useCursorItemForPickupAll) {
                AmpereCursorClickHelper.click(mc, ItemTarget.slotOnly(targetSlot), times);
                return;
            }
            int resolvedTargetSlot = AmpereInventoryHelper.resolveConfiguredHandlerSlot(mc, targetSlot);
            if (resolvedTargetSlot < 0) {
                return;
            }
            if (action == AmpereDropAction.DROP_ITEM || action == AmpereDropAction.DROP_STACK) {
                AmpereDropHelper.dropFromHandlerSlot(mc, resolvedTargetSlot, action == AmpereDropAction.DROP_STACK ? 0 : times);
                return;
            }

            for (int i = 0; i < times; i++) {
                AmpereInventoryClickHelper.click(mc, resolvedTargetSlot, effectiveButton, action.toContainerInput());
            }
        } else if (getAction() == AmpereDropAction.PICKUP_ALL && useCursorItemForPickupAll) {
            AmpereCursorClickHelper.click(mc, null, times);
        }
    }
    ...
    private int findMatchingSlotInPartition(AbstractContainerMenu handler, ItemTarget target, StackAmountMode mode, Boolean playerInventoryOnly) {
        ...
        for (Slot slot : handler.slots) {
            ...
            if (playerInventoryOnly != null) {
                boolean isPlayerInventory = AmpereInventoryHelper.isInventorySlot(MC, slot);
    ...
    private int findExactMatchingSlot(AbstractContainerMenu handler, ItemTarget target, int requiredSlot) {
        ...
        int visibleSlot = AmpereInventoryHelper.toUserVisibleSlot(MC, requiredSlot);
    ...
    public AmpereDropAction getAction() {
    ...
    public String getButtonName() {
        if (getAction() == AmpereDropAction.PICKUP_ALL) return "All";
        if (getAction() == AmpereDropAction.SWAP) {
    ...
    public void cycleButton() {
        if (getAction() == AmpereDropAction.PICKUP_ALL) return;
        int limit = getAction() == AmpereDropAction.SWAP ? 9 : 3;
    ...
    private static int normalizeItemClickActionIndex(int rawIndex) {
        ...
        return AmpereDropAction.PICKUP.ordinal();
    }
