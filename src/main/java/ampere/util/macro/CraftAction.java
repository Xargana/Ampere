...
public class CraftAction implements MacroAction {
    public static final class CraftEntry {
        ...
        public static CraftEntry fromOption(AmpereCraftingHelper.CraftableRecipeOption option, int amount, boolean useMaxAmount) {
            CraftEntry entry = new CraftEntry();
    ...
    public void execute(Minecraft mc) {
        if (mc.player == null || mc.getConnection() == null) {
            AmpereMessaging.sendPrefixed("No network connection!");
            return;
        }
        if (!hasEntries()) {
            AmpereMessaging.sendPrefixed("No craft recipe selected!");
            return;
        }
        ...
        for (CraftEntry entry : entries) {
            ...
            if (option == null) {
                AmpereMessaging.sendPrefixed("§cRecipe not found: " + entry.resultName);
                return;
            }
