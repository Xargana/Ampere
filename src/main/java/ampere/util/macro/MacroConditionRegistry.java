...
public class MacroConditionRegistry {
    ...
    public static CompletableFuture<Void> waitForGamemodeChange(WaitGamemodeChangeAction action) {
        CompletableFuture<Void> future = new CompletableFuture<>();
        GamemodeChangeCondition condition = new GamemodeChangeCondition(action, AmpereFakeGamemode.snapshot(), future);
        pendingConditions.add(condition);
        Minecraft mc = Minecraft.getInstance();
        if (checkOnClientThread(mc, condition)) {
            condition.complete();
    ...
    public static CompletableFuture<Void> await(ampere.api.macro.AddonCondition condition) {
        CompletableFuture<Void> future = new CompletableFuture<>();
        if (condition == null) {
    ...
    static final class AddonConditionAdapter implements PendingCondition {
        private final ampere.api.macro.AddonCondition condition;
        private final CompletableFuture<Void> future;

        AddonConditionAdapter(ampere.api.macro.AddonCondition condition, CompletableFuture<Void> future) {
    ...
    static class GamemodeChangeCondition implements PendingCondition {
        ...
        final AmpereFakeGamemode.Snapshot baseline;
        final CompletableFuture<Void> future;
        private volatile boolean cancelled = false;

        GamemodeChangeCondition(WaitGamemodeChangeAction action, AmpereFakeGamemode.Snapshot baseline, CompletableFuture<Void> future) {
            this.action = action == null ? new WaitGamemodeChangeAction() : action;
            this.baseline = baseline == null ? AmpereFakeGamemode.snapshot() : baseline;
            this.future = future;
        }

        @Override
        public boolean check(Minecraft mc) {
            AmpereFakeGamemode.Snapshot now = AmpereFakeGamemode.snapshot();
    ...
    public static CompletableFuture<Void> waitForSlotChange(WaitForSlotChangeAction action) {
        ...
        if (mc.player != null) {
            ...
            for (int i = 0; i < action.entries.size(); i++) {
                ...
                int handlerSlot = AmpereInventoryHelper.resolveConfiguredHandlerSlot(mc, slotNum);
                ...
                if (e.waitMode == WaitForSlotChangeAction.WaitMode.IS_EMPTY) {
                    ...
                    if (handler != null) {
                        for (net.minecraft.world.inventory.Slot s : handler.slots) {
                            ...
                            int visibleSlot = AmpereInventoryHelper.toUserVisibleSlot(mc, s.index);
