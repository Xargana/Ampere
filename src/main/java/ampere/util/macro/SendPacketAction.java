...
import ampere.util.AmperePacketSender;
...
public class SendPacketAction implements MacroAction, WaitsForGui {
    public List<AmpereSharedState.QueuedPacket> packets = new ArrayList<>();
    ...
    public List<AmpereSharedState.QueuedPacket> getPackets() {
        return packets;
    }

    public void regeneratePackets() {
        List<AmpereSharedState.QueuedPacket> newPackets = new ArrayList<>(packets.size());
        for (AmpereSharedState.QueuedPacket qp : packets) {
            if (qp.packet != null) {
                if (qp.isExactReplay()) {
                    newPackets.add(new AmpereSharedState.QueuedPacket(qp.packet, qp.getDelay(), qp.getId(), qp.getReplayMode()));
                    continue;
                }
                Packet<?> regenerated = PacketRegenerator.regenerate(qp.packet);
                if (regenerated != null) {

                    newPackets.add(new AmpereSharedState.QueuedPacket(regenerated, qp.getDelay(), qp.getId(), qp.getReplayMode()));
                }
    ...
    public void execute(Minecraft mc) {
        ...
        for (AmpereSharedState.QueuedPacket qp : packets) {
            ...
            try {
                AmperePacketSender.send(qp.packet);
                successCount++;
            } catch (Exception e) {
        ...
        if (failCount > 0) {
            AmpereMessaging.sendPrefixed(String.format("§eSent %d/%d packets (%d failed)",
            ...
            if (packetList != null) {
                for (int i = 0; i < packetList.size(); i++) {
                    ...
                    if (element instanceof CompoundTag) {
                        AmpereSharedState.QueuedPacket qp = AmpereClipboardHelper.deserializeQueuedPacket((CompoundTag) element);
                        if (qp != null) {
                            packets.add(qp);
                        }
                    }
    ...
    public String getDisplayName() {
        ...
        for (AmpereSharedState.QueuedPacket qp : packets) {
            if (qp.packet != null) {
                String name = AmperePacketNamer.getFriendlyName(qp.packet);
