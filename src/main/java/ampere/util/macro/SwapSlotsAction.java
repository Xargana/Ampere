...
public class SwapSlotsAction implements MacroAction {
    ...
    public void execute(Minecraft mc) {
        ...
        if (fromUseItemName) {
            ...
            resolvedFrom = AmpereInventoryHelper.findUserVisibleSlot(mc, target);
            if (resolvedFrom == -1) return;
        }
        if (toUseItemName) {
            ItemTarget target = resolvedToItemTarget();
            if (!target.hasSlot() && !target.hasIdentity()) return;
            resolvedTo = AmpereInventoryHelper.findUserVisibleSlot(mc, target);
            if (resolvedTo == -1) return;
        }

        int fromHandlerSlot = AmpereInventoryHelper.resolveConfiguredHandlerSlot(mc, resolvedFrom);
        int toHandlerSlot = AmpereInventoryHelper.resolveConfiguredHandlerSlot(mc, resolvedTo);
        if (fromHandlerSlot < 0 || toHandlerSlot < 0) return;

        AmpereInventoryHelper.swapHandlerSlots(mc, fromHandlerSlot, toHandlerSlot);
    }
