...
public class DisconnectAction implements MacroAction {
    ...
    private void executeAutoDisconnect(Minecraft mc) {
        ...
        AmpereMessaging.sendPrefixed("§eAuto Disconnect: Monitoring for " + trigger.name() + "...");
        ...
        while (!triggered && MacroExecutor.isRunning() && (System.currentTimeMillis() - startTime) < timeoutSec * 1000L) {
            if (mc.player == null || mc.getConnection() == null) {
                AmpereMessaging.sendPrefixed("§cConnection lost during wait!");
                return;
            }
            ...
            switch (trigger) {
                case TELEPORT -> {
                    ...
                    if (now - lastPositionCheck >= 50) {
                        ...
                        if (dist > 5.0) {
                            AmpereMessaging.sendPrefixed(String.format("§aTeleport detected! Jumped %.1f blocks.", dist));
                            triggered = true;
                        }
                        if (!triggered && mc.level != null) {
            String cw = mc.level.dimension().identifier().toString();
                            if (!cw.isEmpty() && !cw.equals(initialWorld)) {
                                AmpereMessaging.sendPrefixed("§aWorld change detected during teleport!");
                                triggered = true;
                            }
                        }
                ...
                case POSITION -> {
                    ...
                    if (dist > tolerance) {
                        AmpereMessaging.sendPrefixed(String.format(
        ...
        String currentWorld = mc.level != null ? mc.level.dimension().identifier().toString() : "";
                    if (!currentWorld.equals(initialWorld) && !currentWorld.isEmpty()) {
                        AmpereMessaging.sendPrefixed("§aWorld change detected!");
                        triggered = true;
                    }
                }
                case GUI_CLOSE -> {
                    ...
                    if (prevScreen != null && curScreen == null) {
                        AmpereMessaging.sendPrefixed("§aGUI close detected!");
                        triggered = true;
                    }
                ...
                case INVENTORY_CLEAR -> {
                    ...
                    if (!initialInventoryEmpty && isMainInventoryEmpty(mc)) {
                        AmpereMessaging.sendPrefixed("§aInventory clear detected!");
                        triggered = true;
                    }
                }
            }
        ...
        if (!triggered) {
            AmpereMessaging.sendPrefixed("§cAuto Disconnect: Timeout after " + timeoutSec + "s");
            return;
        }

        if (bufferMs > 0) {
            AmpereMessaging.sendPrefixed("§eBuffer delay: " + bufferMs + "ms");
            try {
        ...
        AmpereMessaging.sendPrefixed("§cExecuting disconnect!");
    ...
    private void executeKick(Minecraft mc) {
        ...
        sendKickPacket(mc);
        sendLagPackets(mc);
        AmpereMessaging.sendPrefixed("§aKick executed!");
    }
    ...
    private void executeKickDupe(Minecraft mc) {
        ...
        if (useNextAction) {
            ...
            if (actions == null || actions.isEmpty()) {
                AmpereMessaging.sendPrefixed("§cKick Dupe: no next action in macro! Add action(s) after this one.");
                return;
            }
            sendLagPackets(mc);
            ...
            for (MacroAction act : actions) {
                try {
                    ...
                    AmpereMessaging.sendPrefixed("§cError in dupe action '" + act.getDisplayName() + "': " + e.getMessage());
                }
            }
            sendKickPacket(mc);
            sendLagPackets(mc);
            ...
            AmpereMessaging.sendPrefixed("§aKick Dupe executed! (" + actNames + ")");
        } else {
            ...
            sendKickPacket(mc);
            sendLagPackets(mc);
            AmpereMessaging.sendPrefixed("§aKick Dupe executed! (bundle)");
        }
    }
    ...
    private boolean findAndUseBundle(Minecraft mc) {
        ...
        if (bundleSlot < 0) {
            AmpereMessaging.sendPrefixed("§cNo bundle found in inventory or hands!");
            return false;
        }
        ...
        if (bundleSlot > 8) {
            ...
            AmpereInventoryHelper.swapInventorySlots(mc, bundleSlot, target);
            hotbarSlot = target;
        }
        ...
        switch (lagMethod) {
            case BOAT_NBT -> {
                ...
                if (!(vehicle instanceof ChestBoat) && !(vehicle instanceof AbstractMinecartContainer)) {
                    AmpereMessaging.sendPrefixed("§cYou must be in a Chest Boat or Minecart with Chest for BoatNBT!");
                    return false;
                }
            }
            case ENTITY_NBT -> {
                if (mc.hitResult == null || mc.hitResult.getType() != HitResult.Type.ENTITY) {
                    AmpereMessaging.sendPrefixed("§cYou must be looking at an entity for EntityNBT!");
                    return false;
                }
                ...
                if (!(target instanceof ChestBoat) && !(target instanceof AbstractMinecartContainer)) {
                    AmpereMessaging.sendPrefixed("§cTarget must be a Chest Boat or Minecart with Chest!");
                    return false;
                }
            }
            default -> {}
