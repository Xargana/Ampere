    ...
    public void execute(Minecraft mc) {
        ...
        if (useItemFilter && (target.hasSlot() || target.hasIdentity())) {
            ...
            if (mc.player.containerMenu != null) {
                ...
                int resolvedTargetSlot = AmpereInventoryHelper.resolveConfiguredHandlerSlot(mc, effectiveSlot);
                if (resolvedTargetSlot >= 0 && resolvedTargetSlot < mc.player.containerMenu.slots.size()) {
                    ...
                    int visibleSlot = AmpereInventoryHelper.toUserVisibleSlot(mc, resolvedTargetSlot);
        ...
        if (matchGui && matchItem) {
            AmpereGuiActions.closeCurrentScreen(mc, sendPacket, false);
        }
    }
