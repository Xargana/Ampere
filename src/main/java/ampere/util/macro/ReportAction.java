...
public class ReportAction implements MacroAction {
    ...
    public void execute(Minecraft mc) {
        if (mc.getConnection() == null) {
            AmpereMessaging.sendPrefixed("\u00A7cReport: no network connection");
            return;
        }
        ...
        if (remainingTimeout <= 0) {
            AmpereMessaging.sendPrefixed("\u00A7eReport \"" + safeLabel() + "\": no time left after start event");
            return;
        }
        ...
        if (!fired) {
            AmpereMessaging.sendPrefixed("\u00A7eReport \"" + safeLabel() + "\": timeout waiting for " + describeEnd());
            return;
        }

        long deltaMs = (System.nanoTime() - startNanos) / 1_000_000L;
        AmpereMessaging.sendPrefixed("\u00A7b" + safeLabel() + ": " + deltaMs + " ms ("
            + describeStart() + " -> " + describeEnd() + ")");

        if (stashToSharedState) {
            AmpereSharedState.get().pushReport(new AmpereSharedState.ReportSample(
    ...
    private long resolveStartAnchor(Minecraft mc) {
        ...
        try {
            if (startEvent == StartEvent.COMMAND_SENT) {
                ...
                if (command.isBlank()) {
                    sendListener.cancel();
                    AmpereMessaging.sendPrefixed("Report \"" + safeLabel() + "\": set a command for COMMAND_SENT.");
                    return 0L;
                }
                mc.execute(() -> {
            ...
            sendListener.cancel();
            AmpereMessaging.sendPrefixed("\u00A7eReport \"" + safeLabel() + "\": timeout waiting for start event "
                + startEvent.name());
            return 0L;
        }
    }
