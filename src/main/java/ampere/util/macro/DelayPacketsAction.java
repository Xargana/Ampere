...
public class DelayPacketsAction implements MacroAction {
    ...
    public void applyDefaultPreset() {
        ...
        for (String target : defaultNames) {
            ...
            for (Class<?> clz : AmperePacketRegistry.getC2SPackets()) {
                String registryName = AmperePacketRegistry.getName((Class<? extends Packet<?>>) clz);
                if (registryName != null && (registryName.equals(target) || registryName.equals(targetNoSuffix))) {
                    c2sPacketNames.add(AmperePacketNamer.getFriendlyName((Class<? extends Packet<?>>) clz));
                    break;
                }
            }
        }
    ...
    public void applyModulePreset() {
        ...
        for (Class<?> clz : shared.getS2CPackets())
            s2cPacketNames.add(AmperePacketNamer.getFriendlyName((Class<? extends Packet<?>>) clz));
    }
    ...
    private Set<Class<? extends Packet<?>>> resolvePackets(List<String> names, boolean isC2S) {
        ...
        Set<Class<? extends Packet<?>>> pool = isC2S
                ? (Set<Class<? extends Packet<?>>>) (Set<?>) AmperePacketRegistry.getC2SPackets()
                : (Set<Class<? extends Packet<?>>>) (Set<?>) AmperePacketRegistry.getS2CPackets();
        for (String name : names) {
            ...
            for (Class<? extends Packet<?>> clazz : pool) {
                String friendlyName = AmperePacketNamer.getFriendlyName(clazz);
