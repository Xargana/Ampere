...
public class UseItemPhaseAction implements MacroAction {
    ...
    public void sendUsePacket(Minecraft mc) {
        ...
        if (target.hasIdentity()) ampere.util.AmpereInventoryHelper.selectHotbarItem(mc, target, mc.player.getInventory().getSelectedSlot());
    ...
    private void clickSlot(Minecraft mc, int slot, int button, ContainerInput input) {
        int handlerSlot = useCustomSlotMapping ? AmpereInventoryHelper.resolveConfiguredHandlerSlot(mc, slot) : slot;
        if (handlerSlot < 0) return;
        AmpereInventoryClickHelper.click(mc, handlerSlot, button, input);
    }
