...
public class DropAction implements MacroAction, WaitsForGui {
    ...
    public void execute(Minecraft mc) {
        ...
        if (mode == DropMode.ALL && itemNames.isEmpty()) {
            for (int i = 0; i < 36; i++) {
                if (!mc.player.getInventory().getItem(i).isEmpty()) {
                    if (useHandlerSlots) {
                        int resolved = AmpereInventoryHelper.toHandlerSlot(mc, i);
                        if (resolved >= 0) dropFromHandlerSlot(mc, resolved, 0);
                    } else {
        ...
        if (!itemNames.isEmpty()) {
            for (int ei = 0; ei < itemNames.size(); ei++) {
                ...
                if (configuredSlot >= 0 && !entryName.isEmpty()) {
                    ...
                    if (useHandlerSlots) {
                        int resolvedSlot = AmpereInventoryHelper.resolveConfiguredHandlerSlot(mc, configuredSlot);
    ...
    private void dropMatchingItemInSpecificSlot(Minecraft mc, int configuredSlot, ItemTarget target, int count) {
        ...
        if (useHandlerSlots) {
            ...
            int visibleSlot = AmpereInventoryHelper.toUserVisibleSlot(mc, resolvedSlot);
    ...
    private void dropFromInvSlot(Minecraft mc, int invSlot, int count) {
        AmpereDropHelper.dropFromInventorySlot(mc, invSlot, count);
    }

    private void dropFromHandlerSlot(Minecraft mc, int handlerSlotId, int count) {
        AmpereDropHelper.dropFromHandlerSlot(mc, handlerSlotId, count);
    }
    ...
    public String getDisplayName() {
        ...
        if (!targets.isEmpty()) {
            ...
            String base = mode == DropMode.ALL ? "Drop All [" + first + "]" : "Drop [" + first + "]";
            return base + WaitsForGui.timingLabel(this);
        }
        return "Drop (no items set)" + WaitsForGui.timingLabel(this);
    }
