...
public class SelectSlotAction implements MacroAction {
    ...
    public void execute(Minecraft mc) {
        ...
        if (strategy == Strategy.EMPTY_SLOT) {
            ...
            if (empty >= 0) {
                AmpereInventoryHelper.selectHotbarSlot(mc, empty);
        ...
        if (!targets.isEmpty()) {
            if (strategy != Strategy.FIRST_MATCH) {
                ...
                if (best >= 0) {
                    AmpereInventoryHelper.selectHotbarSlot(mc, best);
                    MacroVariables.set(outputVariable, best);
                    return;
                }
            }
            for (ItemTarget target : targets) {
                int selectedSlot = AmpereInventoryHelper.selectHotbarItem(mc, target, slot);
                if (selectedSlot >= 0) {
        ...
        AmpereInventoryHelper.selectHotbarSlot(mc, actualSlot);
