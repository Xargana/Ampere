...
public class GoToAction implements MacroAction {
    ...
    public void execute(Minecraft mc) {
        ...
        if (!AmpereCompatManager.sendBaritoneCommand(mc, command)) {
            AmpereMessaging.sendPrefixed("§cBaritone is not available, cannot send goto command.");
        }
    }
    ...
    public void fromTag(CompoundTag tag) {
        ...
        if (tag.contains("waitForArrival")) waitForArrival = tag.getBooleanOr("waitForArrival", true);
