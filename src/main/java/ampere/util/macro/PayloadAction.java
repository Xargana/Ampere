...
import ampere.util.AmperePayloadScriptExecutor;
...
public class PayloadAction implements MacroAction {
    ...
    public void execute(Minecraft mc) {
        ...
        if (mc == null || mc.getConnection() == null) {
            AmpereMessaging.sendPrefixed("§cCannot send payload while disconnected.");
            return;
        }
        ...
        try {
            ...
            if (hasStructuredPayload) {
                AmperePayloadTemplate.Template template = AmperePayloadTemplate.fromAction(this);
                ...
                AmperePayloadJsonSupport.EncodedPayload encoded = AmperePayloadJsonSupport.encodeAction(this);
                targetChannel = encoded.channel();
                rawBytes = encoded.bytes();
            } else {
                rawBytes = AmperePayloadSupport.parsePayloadBytes(payloadData);
            }
            if (!useJsonModel && !hasStructuredPayload && !hasRawPayload && AmperePayloadSupport.isBrandChannel(targetChannel)) {
                rawBytes = AmperePayloadSupport.encodeMinecraftStringPayload(
                    AmperePayloadSupport.defaultBrandPayloadString());
            }
            if (commandApiRecognized && commandApiOverride) {
                rawBytes = AmperePayloadSupport.withCommandApiValue(rawBytes, commandApiValue);
            }
            AmperePayloadScriptExecutor.Context context = new AmperePayloadScriptExecutor.Context(targetChannel, rawBytes,
                commandApiRecognized && commandApiOverride ? commandApiValue : null);
            AmperePayloadScriptExecutor.ScriptResult result = AmperePayloadScriptExecutor.execute(
                payloadScriptEnabled ? javaSource : "", context);
            if (AmperePayloadSupport.sendPayload(result.channel(), result.bytes(), sendProtocol)) {
                AmpereMessaging.sendPrefixed("Sent payload: " + result.channel());
            }
        } catch (Exception e) {
            AmpereMessaging.sendPrefixed("§cPayload action failed: " + AmperePayloadSupport.safeMessage(e));
        }
    }
