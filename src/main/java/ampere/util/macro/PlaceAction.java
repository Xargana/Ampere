...
import ampere.util.AmpereMacroSneak;
...
public class PlaceAction implements MacroAction, WaitsForGui {
    ...
    public void execute(Minecraft mc) {
        ...
        if (!AmpereContainerTarget.isWithinBlockReach(mc, blockPos)) return;
        ...
        if (sneaking) AmpereMacroSneak.hold(mc, sneakMode);
        try {
            mc.gameMode.useItemOn(mc.player, InteractionHand.MAIN_HAND, hit);
        } finally {
            if (sneaking) AmpereMacroSneak.release(mc, sneakMode);
        }

        if (silentSwitch && mc.player.getInventory().getSelectedSlot() != originalSlot) {
            AmpereInventoryHelper.selectHotbarSlot(mc, originalSlot);
        }
    }
    ...
    static BlockHitResult resolvePlaceHit(Minecraft mc, BlockPos targetPos, boolean manualDirection, Direction manualFace) {
        ...
        for (Direction face : order) {
            ...
            if (!AmpereContainerTarget.isWithinBlockReach(mc, supportPos)) continue;
    ...
    private static void selectItemForPlace(Minecraft mc, ItemTarget target) {
        ...
        if (target.hasIdentity()) {
            AmpereInventoryHelper.selectHotbarItem(mc, target, mc.player.getInventory().getSelectedSlot());
            return;
        }

        int visibleSlot = target.slot;
        if (visibleSlot >= 0 && visibleSlot <= 8) {
            AmpereInventoryHelper.selectHotbarSlot(mc, visibleSlot);
            return;
        }

        int handlerSlot = AmpereInventoryHelper.resolveConfiguredHandlerSlot(mc, visibleSlot);
        if (handlerSlot < 0) return;

        int hotbarSlot = mc.player.getInventory().getSelectedSlot();
        int hotbarHandlerSlot = AmpereInventoryHelper.resolveConfiguredHandlerSlot(mc, hotbarSlot);
        if (hotbarHandlerSlot >= 0) {
            AmpereInventoryHelper.swapHandlerSlots(mc, handlerSlot, hotbarHandlerSlot);
        }
    }
