...
public class SignEditAction implements MacroAction, WaitsForGui {
    ...
    private BlockPos resolvePos(Minecraft mc) {
        ...
        if (targetMode == TargetMode.LAST_INTERACTED_BLOCK) return AmpereSharedState.get().getLastInteractedBlockPos();
        if (mc.screen instanceof AmpereSignEditAccess access) return access.Ampere$getSignPos();
        return null;
    }

    private boolean resolveFront(Minecraft mc) {
        if (targetMode == TargetMode.CURRENT_SIGN_GUI && mc.screen instanceof AmpereSignEditAccess access) return access.Ampere$isFrontText();
        return frontText;
    }
