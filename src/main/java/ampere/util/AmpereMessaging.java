...
import ampere.mixin.accessor.AmpereChatComponentAccessor;
...
public final class AmpereMessaging {
    private static final Minecraft MC = Minecraft.getInstance();
    private static final long DUPLICATE_WINDOW_MS = 600L;
    private static final String PREFIX_TEXT = "[Ampere] ";
    ...
    public static void sendPrefixed(String message) {
        MutableComponent text = Component.empty()
            .append(styled("[", AmpereColors.textMuted()))
            .append(styled("Ampere", AmpereColors.accent(), true))
            .append(styled("] ", AmpereColors.textMuted()))
    ...
    public static void send(Component text) {
        ...
        synchronized (AmpereMessaging.class) {
    ...
    public static void clearClientMessages() {
        ...
        MC.execute(() -> {
            ...
            try {
                AmpereChatComponentAccessor chat = (AmpereChatComponentAccessor) MC.gui.getChat();
                List<GuiMessage> messages = chat.Ampere$getAllMessages();
                ...
                if (changed) chat.Ampere$refreshTrimmedMessages();
    ...
    private static boolean isClientGeneratedMessage(String message) {
        ...
        if (lower.startsWith("[Ampere]") || lower.startsWith("[lan]")) return true;
    ...
    private static MutableComponent tryBuildLanChatText(String message) {
        ...
        return Component.empty()
            .append(styled("[", AmpereColors.textMuted()))
            .append(styled("LAN", AmpereColors.packetCyan(), true))
            .append(styled("] ", AmpereColors.textMuted()))
            .append(styled("<", AmpereColors.textMuted()))
            .append(styled(sender, AmpereColors.packetPink(), true))
            .append(styled("> ", AmpereColors.textMuted()))
            .append(colorizeTokenStream(body, AmpereColors.textPrimary()));
    }
    ...
    private static MutableComponent colorizeStructuredText(String message, int defaultColor) {
        ...
        if (colon > 0 && colon < 24 && isLikelyLabel(message.substring(0, colon))) {
            ...
            MutableComponent root = Component.empty()
                .append(styled(label, AmpereColors.packetGray(), true))
                .append(styled(":", AmpereColors.textMuted()));

            if (!rest.isEmpty()) {
                root.append(styled(leadingWhitespace(rest), AmpereColors.textMuted()));
    ...
    private static int punctuationColor(char ch, int defaultColor) {
        return switch (ch) {
            case '[', ']', '(', ')', '<', '>', ':', '|', ',' -> AmpereColors.textMuted();
            case '"' -> AmpereColors.packetLightYellow();
    ...
    private static int colorForToken(String token, int defaultColor) {
        ...
        if (isNumericToken(normalized)) return AmpereColors.packetLightYellow();
        ...
        if (normalized.equals("enabled")
            ...
            return AmpereColors.successText();
        }
        ...
        if (normalized.equals("failed")
            ...
            return AmpereColors.dangerText();
        }
    ...
    private static Integer mapLegacyColor(char code) {
        return switch (code) {
            ...
            case '1' -> 0xFF6B92D8;
            case '2' -> 0xFF59B974;
            case '3' -> AmpereColors.packetCyan();
            case '4' -> AmpereColors.dangerText();
            case '5' -> 0xFFE78ACB;
            case '6' -> AmpereColors.packetOrange();
            case '7' -> AmpereColors.packetGray();
            case '8' -> AmpereColors.textMuted();
            case '9' -> AmpereColors.packetBlue();
            case 'a' -> AmpereColors.successText();
            case 'b' -> AmpereColors.packetCyan();
            case 'c' -> AmpereColors.dangerText();
            case 'd' -> AmpereColors.packetPink();
            case 'e' -> AmpereColors.packetYellow();
            case 'f' -> AmpereColors.textSecondary();
            case 'r' -> null;
            default -> null;
        };
    }
    ...
    private static int inferDefaultColor(String message) {
        ...
        if (lower.isEmpty()) return AmpereColors.textSecondary();
        ...
        if (lower.startsWith("[lan] <")) {
            return AmpereColors.textPrimary();
        }
    ...
    private static String replacementGroupKey(String message) {
        String clean = stripAmperePrefix(stripLegacyCodes(normalizeLegacyFormatting(message)).replaceAll("\\s+", " ").trim());
        ...
        try {
            ...
            while (iterator.hasNext()) {
                ...
                if (group.equals(replacementGroupKey(existing.content().getString()))) {
                    iterator.remove();
                    chat.Ampere$refreshTrimmedMessages();
                    return;
                }
            }
        } catch (Throwable ignored) {
    ...
    private static String stripAmperePrefix(String message) {
        if (message == null) return "";
        String trimmed = message.trim();
        if (trimmed.startsWith(PREFIX_TEXT)) return trimmed.substring(PREFIX_TEXT.length()).trim();
        if (trimmed.startsWith("[Ampere]")) return trimmed.substring("[Ampere]".length()).trim();
        return trimmed;
    }
