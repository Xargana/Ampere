...
import ampere.gui.vanillaui.direct.DirectProgressBar;
import ampere.gui.vanillaui.direct.DirectInfoRow;
import ampere.gui.vanillaui.direct.DirectRow;
import ampere.gui.vanillaui.components.CompactSurfaces;
import ampere.gui.vanillaui.direct.DirectSlider;
import ampere.gui.vanillaui.direct.DirectSpacer;
import ampere.gui.vanillaui.direct.DirectTabStrip;
...
public class AmpereServerInfoOverlay extends AmpereOverlayBase {
    private static final Minecraft MC = Minecraft.getInstance();
    private static final String OVERLAY_ID = "Ampere-serverinfo";
    ...
    public AmpereServerInfoOverlay(Font textRenderer) {
    ...
    public static boolean shouldRestoreSavedVisible() {
        AmpereWindowLayout layout = AmpereSharedState.get().getWindowLayout(OVERLAY_ID);
    ...
    private void buildInfoUi() {
        ...
        double estimatedTps = AmpereSharedState.get().getEstimatedTps();
        ...
        if (!"--".equals(versionNote)) addInfoRow("Version Note:", versionNote, AmpereColors.packetYellow());
    ...
    private void buildPluginSetupUi() {
        boolean autoOn = AmpereConfig.getGlobal().autoProbePlugins;
        ...
        windowNode.content().add(new DirectUiButton("Start Probing", DirectUiButton.Variant.SECONDARY, () -> {
            AmpereConfig.getGlobal().removePluginScan(currentServerAddress(), currentPluginContextSignature());
            resetScan();
            scanPlugins();
        }).setGrowX(true).setButtonHeight(actionButtonHeight()));
    }
    ...
    private void setPluginProbeDelayMs(int delayMs) {
        ...
        AmpereSharedState.get().setServerDataOverlayProbeDelayMs(pluginProbeDelayMs);
    }

    private void applyClampedBounds() {
        AmpereWindowLayout clamped = clampToViewport(new AmpereWindowLayout(panelX, panelY, panelW, panelH, visible, collapsed));
        panelX = clamped.x;
        panelY = clamped.y;
    ...
    private boolean loadCachedScan(String address, String contextSignature) {
        AmpereConfig.PluginScanCacheEntry cached = AmpereConfig.getGlobal().getPluginScan(address, contextSignature);
    ...
    private void cacheCurrentScan() {
        ...
        AmpereConfig.getGlobal().putPluginScan(scannedServerAddress, scannedPluginContextSignature,
    ...
    private String normalizePluginKey(String plugin) {
        return AmperePluginPayloadFingerprints.canonicalPluginKey(plugin);
    }
    ...
    private int getConfidenceColor(PluginConfidence confidence, boolean hovered) {
        if (hovered) return AmpereColors.packetWhite();
        return switch (confidence == null ? PluginConfidence.UNKNOWN : confidence) {
            case EXACT -> AmpereColors.packetGreen();
            case STRONG -> AmpereColors.packetCyan();
            case FEATURE -> AmpereColors.packetYellow();
    ...
    private boolean mergePayloadFingerprintEntries(Map<String, PluginScanEntry> entries) {
        ...
        for (AmperePluginPayloadFingerprints.PluginFingerprint fingerprint : AmperePluginPayloadFingerprints.pluginsFor(address, getLiveBrand())) {
    ...
    private PluginConfidence payloadFingerprintConfidence(AmperePluginPayloadFingerprints.PluginFingerprint fingerprint) {
    ...
    private boolean promoteMatchingFeatureEntries(Map<String, PluginScanEntry> entries,
                                                  AmperePluginPayloadFingerprints.PluginFingerprint fingerprint) {
    ...
    private Set<String> fingerprintKeywords(AmperePluginPayloadFingerprints.PluginFingerprint fingerprint) {
    ...
    private void syncPayloadFingerprintsIntoCurrentScan() {
        ...
        String address = currentServerAddress();
        long revision = AmperePluginPayloadFingerprints.revisionFor(address, getLiveBrand());
    ...
    public boolean autoProbeOnSpawn() {
        ...
        long cachedAt = AmpereConfig.getGlobal().getPluginScanTimestamp(address, context);
        ...
        lastAutoScanContext = context;
        AmpereConfig.getGlobal().removePluginScan(address, context);
        resetScan();
        scanPlugins();
    ...
    private int getPluginScanPhaseColor() {
        return queuedPluginProbes.isEmpty() ? AmpereColors.packetCyan() : AmpereColors.packetYellow();
    }
    ...
    private void announcePluginScanComplete() {
        ...
        if (count <= 0) {
            AmpereMessaging.sendPrefixed("Plugin probing finished: no plugins found.");
        } else if (count == 1) {
            AmpereMessaging.sendPrefixed("Plugin probing finished: found 1 plugin.");
        } else {
            AmpereMessaging.sendPrefixed("Plugin probing finished: found " + count + " plugins.");
        }
    }
    ...
    public void setBounds(AmpereWindowLayout b) {
        if (b == null) return;
        AmpereWindowLayout clamped = clampToViewport(b);
    ...
    private String getRealServerVersion() {
        String version = AmpereSharedState.get().getRealServerVersion(getDisplayedServerAddress());
                        ...
                        if (rowVisible && clickableRow && row.actionCommand != null && !row.actionCommand.isBlank()) {
                            ...
                            clickRegions.add(new ClickRegion(clipLeft + 9, hitTop, Math.max(1, rowContentW - 15), Math.max(1, hitBottom - hitTop), () -> {
                                ...
                                if (commandRow) {
                                    ...
                                    AmpereOverlayManager.get().setTemporarilyHidden(this, true);
    ...
    private int pluginDetailColor(PluginListRow row) {
        ...
        return switch (row.type) {
            ...
            case GUI -> AmpereColors.packetYellow();
    ...
    private List<String> getPluginChannelDetailRows(String plugin) {
        ...
        List<String> probableNamespaces = AmperePluginPayloadFingerprints.probableNamespacesForPlugin(plugin);
    ...
    private int channelDetailColor(String detail) {
        ...
        if (lower.contains("[registered")) return AmpereColors.packetGreen();
        if (lower.contains("[live")) return AmpereColors.packetCyan();
    ...
    private String bestGuessPayloadChannel(String rawChannel) {
        ...
        try {
            AmperePayloadSupport.parseChannel(channel);
            return channel;
        } catch (IllegalArgumentException ignored) {
            return "";
        }
    ...
    private void copyClipboardValue(String value, String successMessage, String unavailableMessage) {
        ...
        if (trimmed.isEmpty() || "--".equals(trimmed) || "Resolving...".equalsIgnoreCase(trimmed) || "Failed".equalsIgnoreCase(trimmed)) {
            AmpereNotifications.error(unavailableMessage);
            return;
        }
        MC.keyboardHandler.setClipboard(trimmed);
        AmpereNotifications.copied(successMessage);
    }
    ...
    private void copyServerData() {
        ...
        AmpereNotifications.copied("Server info copied.");
    }

    private void copyPluginList() {
        if (detectedPlugins.isEmpty()) {
            AmpereNotifications.error("No plugins detected.");
            return;
        }
        ...
        if (copyPlugins.isEmpty()) {
            AmpereNotifications.error("No non-hint plugins to copy.");
            return;
        }
        StringBuilder sb = new StringBuilder();
        ...
        MC.keyboardHandler.setClipboard(sb.toString());
        AmpereNotifications.copied("Plugin list copied.");
    }
}
