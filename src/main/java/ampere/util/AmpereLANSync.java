...
public class AmpereLANSync {
    private static AmpereLANSync instance;
    ...
    private volatile ConcurrentHashMap<String, Map<String, AmpereMacro>> clientMacroLists = new ConcurrentHashMap<>();
    private volatile ConcurrentHashMap<String, Map<String, AmpereMacro>> remoteMeteorMacros = new ConcurrentHashMap<>();
    ...
    public static class SyncContext {
        ...
        public final AmpereMacro macro;
        ...
        public SyncContext(long executionId, boolean isMacro, String macroName,
                           String command, AmpereMacro macro, boolean isInitiator) {
    ...
    private AmpereLANSync() {}

    public static AmpereLANSync getInstance() {
        if (instance == null) {
            instance = new AmpereLANSync();
        }
        return instance;
    }
    ...
    public void start() {
        ...
        running.set(true);
        AmpereMessaging.sendPrefixed("§aLAN Sync started (TCP mode on port " + TCP_PORT + ").");
        AmpereClientAddon.LOG.info("[Ampere-LAN] Started in TCP-only mode on port {}", TCP_PORT);
    }
    ...
    public void stop() {
        ...
        AmpereMessaging.sendPrefixed("§eLAN Sync stopped.");
    }
    ...
    public void createSession() {
        if (!running.get()) {
            AmpereMessaging.sendPrefixed("§cStart LAN Sync first.");
            return;
        }
        ...
        AmpereMessaging.sendPrefixed("§aSession created: " + sessionId + " [TCP Sync]");
    ...
    public void joinSession(String targetSessionId, String hostIp) {
        ...
        if (sessionId != null && sessionId.equals(targetSessionId)) {
            AmpereMessaging.sendPrefixed("§cAlready in session " + sessionId);
            return;
        }
        ...
        AmpereClientAddon.LOG.info("[TCP] Connecting to {}:{}", lastHostIp, TCP_PORT);
    ...
    public void leaveSession() {
        ...
        sendTcpPacket(new LanPacket.LeavePacket(sessionId, getUsername()));
        clearSession();
        AmpereMessaging.sendPrefixed("§eLeft session.");
        fireCallback(onSessionStateChanged);
    }
    ...
    private void initiateGoSync(boolean isMacro, String macroName, String command, Map<String, String> macroAssignments) {
        ...
        if (isMacro) {
            if (macroAssignments != null && !macroAssignments.isEmpty()) {
                ...
                if (!missingAssignments.isEmpty()) {
                    AmpereMessaging.sendPrefixed("§cMissing assigned macros on: " + summarizeAssignments(missingAssignments));
                    return;
                }
            } else if (macroName != null && !macroName.isBlank()) {
                List<String> missingPeers = getPeersMissingMacro(macroName);
                if (!missingPeers.isEmpty()) {
                    AmpereMessaging.sendPrefixed("§cMissing '" + macroName + "' on: " + summarizeNames(missingPeers));
                    return;
                }
            }
        }
        ...
        AmpereMacro macro = null;
        if (isMacro && myMacroName != null) {
            macro = AmpereMacroManager.get().get(myMacroName);
            if (macro != null) {
                macro.regenerateAllPackets();
            } else {
                AmpereMessaging.sendPrefixed("§cMacro not found: " + myMacroName);
                return;
            }
        }
        ...
        AmpereClientAddon.LOG.info("[Sync] Initiating GO-sync: execId={}", executionId);
            ...
            try {
                ...
                while (clientsReady.size() < expectedClients) {
                    if (System.currentTimeMillis() - waitStart > 3000) {
                        AmpereClientAddon.LOG.warn("[Sync] Timeout waiting for clients. Got {}/{}",
                ...
                AmpereClientAddon.LOG.info("[Sync] All {} clients ready!", clientsReady.size());
                ...
                if (method == ExecutionMethod.TICK) {
                    ...
                    if (currentTick < 0L) {
                        AmpereMessaging.sendPrefixed("§cCannot tick-sync without a loaded world.");
                        setSyncState(SyncState.IDLE);
                        return;
                    }
                ...
                for (Map.Entry<String, Socket> entry : tcpClients.entrySet()) {
                    ...
                    try {
                        ...
                        AmpereClientAddon.LOG.warn("[Sync] Failed to send GO to {}", clientName);
                    }
                }
                ...
                AmpereClientAddon.LOG.error("[Sync] Error in GO-sync: ", e);
                setSyncState(SyncState.IDLE);
            }
    ...
    private void initiateGoSyncForQueue() {
        ...
        AmpereClientAddon.LOG.info("[Sync] Initiating queue flush GO-sync: execId={}", executionId);
        ...
        new Thread(() -> {
            ...
            try {
                ...
                AmpereClientAddon.LOG.error("[Sync] Error in queue GO-sync: ", e);
                setSyncState(SyncState.IDLE);
            }
    ...
    private void handlePrepare(long executionId, boolean isMacro, String macroName, String command,
        ...
        AmpereClientAddon.LOG.info("[Sync] Received PREPARE from {}", senderUsername);
        peerStepProgress.clear();
        tickExecutionReports.clear();
        locallyExecutedIds.clear();
        scheduledTickExecution = null;
        ...
        if (isMacro && myMacroName != null && !myMacroName.isEmpty()) {
            ...
            AmpereSharedState.get().regenerateQueue();
        }
        ...
        AmpereClientAddon.LOG.info("[Sync] Sent READY signal");
    }
    ...
    private void handleClientReady(long executionId, String username) {
        ...
        if (ctx != null && executionId == ctx.executionId) {
            ...
            broadcastSyncState(SyncState.PREPARING, clientsReady.size() + "/" + connectedClients.size() + " ready");
            AmpereClientAddon.LOG.info("[Sync] Client ready: {} (total: {})", username, clientsReady.size());
        }
    }
    ...
    private void handleGo(long executionId, int methodOrdinal, long targetTick) {
        SyncContext ctx = activeSyncCtx;
        if (ctx == null || executionId != ctx.executionId) {
            AmpereClientAddon.LOG.warn("[Sync] GO for wrong execId, ignoring");
            return;
        }
        ...
        if (method == ExecutionMethod.TICK && targetTick >= 0L) {
            AmpereClientAddon.LOG.info("[Sync] Received GO! Scheduling tick execution for {}", targetTick);
            setSyncState(SyncState.EXECUTING);
            scheduleTickExecution(executionId, targetTick);
            return;
        }
        ...
        if (offset > 0) {
            AmpereClientAddon.LOG.info("[Sync] Received GO! Applying {}ms offset before executing.", offset);
            setSyncState(SyncState.EXECUTING);
            try { Thread.sleep(offset); } catch (InterruptedException ignored) {}
            executeAction();
        } else {
            AmpereClientAddon.LOG.info("[Sync] Received GO! Executing immediately.");
            setSyncState(SyncState.EXECUTING);
            executeAction();
        }
    }
    ...
    private void executeAction() {
        ...
        if (COMMAND_QUEUE_FLUSH.equals(ctx.command)) {
            ...
            if (nh != null) {
                int count = AmpereSharedState.get().flushDelayedPackets(nh);
                AmpereClientAddon.LOG.info("[Sync] Queue flushed! {} packets sent", count);
            }
            ...
            AmpereClientAddon.LOG.info("[Sync] Delay packets set to {} (flushed={})", enabled, flushed);
            ...
            if (nh != null) {
                ...
                AmpereClientAddon.LOG.info("[Sync] Chat sent: {}", chatMsg);
            }
        } else if (ctx.isMacro && ctx.macro != null) {
            ctx.macro.execute(false);
        }

        AmpereClientAddon.LOG.info("[Sync] EXECUTED at {} tick {}", executedAtMs, actualTick);
        ...
        if (ctx.isInitiator) {
            new Thread(() -> {
                try {
                    ...
                    AmpereClientAddon.LOG.error("[Sync] Error calculating spread", e);
                }
    ...
    private void handleExecutionTimestamp(String username, long timestampMs, int methodOrdinal, long targetTick, long actualTick, boolean late) {
        ...
        AmpereClientAddon.LOG.info("[Sync] ACK from {}: remoteMs={}, hostReceiptNano={}, tick={}", username, timestampMs, ackReceiptTimes.get(username), actualTick);
    }
    ...
    private void scheduleTickExecution(long executionId, long targetTick) {
        ...
        if (ampere.util.macro.ServerTickTracker.isReady()) {
            long ticksOut = targetTick - currentTick;
            long now = System.nanoTime();
            long nextTickNano = ampere.util.macro.ServerTickTracker.getNextServerTickNanos();

            long targetBoundaryNano = nextTickNano + (Math.max(0, ticksOut - 1) * 50_000_000L);
            int pingMs = ampere.util.macro.ServerTickTracker.getPingMs();
    ...
    private void calculateAndDisplaySpread() {
        ...
        AmpereMessaging.sendPrefixed(report);
    ...
    public void sendQueuedPackets() {
        if (sessionId == null) {
            AmpereMessaging.sendPrefixed("§cJoin a session first.");
            return;
        }
        AmpereSharedState.get().regenerateQueue();
        ...
        if (isHost) {
            ...
            AmpereMessaging.sendPrefixed("§eRequested sync execution from host...");
        }
    }
    ...
    public void setDelayPacketsSynchronized(boolean enabled) {
        ...
        if (isHost) {
            ...
            request.executionMethod = executionMethod.ordinal();
            sendTcpPacket(request);
            AmpereMessaging.sendPrefixed("§eRequested sync delay toggle from host...");
        }
    }
    ...
    public void executeMacrosSynchronized(Map<String, String> assignments) {
        ...
        if (isHost) {
            ...
            sendTcpPacket(rsp);
            AmpereMessaging.sendPrefixed("§eRequested per-user sync from host...");
        }
    }
    ...
    public void startSearching() {
        ...
        if (isInSession()) {
            AmpereMessaging.sendPrefixed("§cAlready in session.");
            return;
        }
    ...
    public void tick() {
        ...
        if (isSearching && searchTicksRemaining > 0) {
            ...
            if (!discoveredSessions.isEmpty()) {
                ...
                AmpereMessaging.sendPrefixed("§aJoined session " + firstSessionId + " (Host: " + hostName + ").");
                return;
            }

            if (searchTicksRemaining == 0) {
                isSearching = false;
                AmpereMessaging.sendPrefixed("§cNo sessions found.");
            }
        }
    }
    ...
    private void checkPeerHealth() {
        ...
        for (String dead : deadPeers) {
            ...
            AmpereMessaging.sendPrefixed("§e" + dead + " timed out.");
            AmpereClientAddon.LOG.info("[Heartbeat] Peer timed out: {}", dead);
            fireCallback(onClientLeft);
            fireCallback(onPeerStatusChanged);
            ampere.util.macro.MacroConditionRegistry.onLanStepProgress();
        }
    ...
    private void handlePacket(LanPacket packet) {
        try {
                ...
                case SESSION:
                    if (packet instanceof LanPacket.SessionPacket) {
                        ...
                        if (!host.equals(myUsername) && !isInSession()) {
                            ...
                            if (isNew) {
                                AmpereMessaging.sendPrefixed("§eFound session " + sid + " (Host: " + host + ").");
                            }
                        }
                    }
                    break;
                ...
                case JOIN:
                    if (packet instanceof LanPacket.JoinPacket && packet.getSessionId().equals(sessionId)) {
                        ...
                        if (username != null && !username.isEmpty()
                            ...
                            AmpereMessaging.sendPrefixed("§e" + username + " joined.");
                            executionTimestamps.clear();
                            if (isHost) {
                                broadcastClientList();
                                broadcastMacroList();
                ...
                case LEAVE:
                    if (packet instanceof LanPacket.LeavePacket && packet.getSessionId().equals(sessionId)) {
                        ...
                        if (username != null && !username.isEmpty() && !username.equals(myUsername)) {
                            ...
                            AmpereMessaging.sendPrefixed("§e" + username + " left.");
                            ...
                            if (leavingWasHost && !connectedClients.isEmpty()) {
                                ...
                                if (newHostUsername != null && newHostUsername.equals(myUsername)) {
                                    isHost = true;
                                    connectedClients.put(myUsername, new ClientInfo(myUsername, true));
                                    AmpereMessaging.sendPrefixed("§aYou are now the host!");
                                    executionTimestamps.clear();
                                    broadcastClientList();
                                }
                            }
            ...
            AmpereClientAddon.LOG.error("[Ampere-LAN] Error handling packet: {}", packet.getType(), e);
        }
    }
    ...
    private void startTcpServer() {
        try {
                ...
                while (running.get() && isHost && tcpServer != null && !tcpServer.isClosed()) {
                    try {
                        ...
                        if (running.get() && isHost) {
                            AmpereClientAddon.LOG.warn("[TCP] Accept error: {}", e.getMessage());
                        }
                    }
                }
            }, "TCP-Accept");
            ...
            AmpereClientAddon.LOG.info("[TCP] Server started on port {}", TCP_PORT);
        } catch (IOException e) {
            AmpereClientAddon.LOG.error("[TCP] Failed to start server: {}", e.getMessage());
        }
    }
        ...
        new Thread(() -> {
            try {
                ...
                if (packetType == LanPacketType.SEARCH_REQUEST.getId()) {
                    AmpereClientAddon.LOG.info("[TCP] Received search request, responding with session info");
                ...
                if (packetType == LanPacketType.JOIN.getId()) {
                    ...
                    if (clientUsername == null || clientUsername.isBlank()) {
                        AmpereClientAddon.LOG.warn("[TCP] Rejected client with empty username");
                        clientSocket.close();
                        return;
                    }

                    if (sessionId == null || !sessionId.equals(packetSession)) {
                        AmpereClientAddon.LOG.warn("[TCP] Rejected client '{}' with wrong/missing session code", clientUsername);
                        clientSocket.close();
                        return;
                    }
                    ...
                    if (!clientUsername.equals(myUsername) && !connectedClients.containsKey(clientUsername)) {
                        ...
                        AmpereMessaging.sendPrefixed("§e" + clientUsername + " joined.");
                        executionTimestamps.clear();
                    ...
                    AmpereClientAddon.LOG.info("[TCP] Client joined: {}", clientUsername);
                ...
                if (running.get() && isHost) {
                    ...
                    if (disconnectedUser != null) {
                        ...
                        AmpereMessaging.sendPrefixed("§e" + disconnectedUser + " disconnected.");
                        AmpereClientAddon.LOG.info("[TCP] Client disconnected: {}", disconnectedUser);
    ...
    private void sendClientListToClient(Socket clientSocket, String clientUsername) {
        try {
            ...
            AmpereClientAddon.LOG.warn("[TCP] Failed to send client list: {}", e.getMessage());
        }
    }

    private void sendMacroListToClient(Socket clientSocket, String clientUsername) {
        try {
            List<AmpereMacro> localMacros = AmpereMacroManager.get().getAll();
            Object lock = tcpClientWriteLocks.computeIfAbsent(clientUsername, k -> new Object());

            for (AmpereMacro macro : localMacros) {
                writeMacroDataPacketsToStream(clientSocket.getOutputStream(), lock, myUsername, macro, (byte)0);
            }
        } catch (IOException e) {
            AmpereClientAddon.LOG.warn("[TCP] Failed to send macro list: {}", e.getMessage());
        }
    }
    ...
    private void connectToTcpServer(String hostIp) {
        new Thread(() -> {
            try {
                ...
                AmpereClientAddon.LOG.info("[TCP] Connected to host at {}:{}", hostIp, TCP_PORT);
                reconnecting = false;
                ...
                tcpReadThread = new Thread(() -> {
                    try {
                        ...
                        AmpereClientAddon.LOG.warn("[TCP] Disconnected from host");
                        attemptReconnect();
                    }
                }, "TCP-Reader");
                ...
                AmpereClientAddon.LOG.warn("[TCP] Failed to connect to host: {}", e.getMessage());
                attemptReconnect();
            }
        }, "TCP-Connect").start();
    }
    ...
    private void attemptReconnect() {
        ...
        reconnecting = true;
        AmpereMessaging.sendPrefixed("§eConnection lost. Reconnecting...");
        ...
        new Thread(() -> {
            ...
            while (reconnecting && running.get() && lastHostIp != null) {
                ...
                attempt++;
                AmpereClientAddon.LOG.info("[TCP] Reconnect attempt {}...", attempt);
                ...
                try {
                    ...
                    connectToTcpServer(lastHostIp);
                    AmpereMessaging.sendPrefixed("§aReconnected to session!");
                    return;
                } catch (IOException e) {
                    AmpereClientAddon.LOG.warn("[TCP] Reconnect attempt {} failed", attempt);
                }
    ...
    private void writeMacroDataPacketsToStream(OutputStream out, Object lock, String senderUsername,
                                               AmpereMacro macro, byte sourceType) throws IOException {
    ...
    private void sendTcpPacket(LanPacket packet) {
        try {
            ...
            AmpereClientAddon.LOG.warn("[TCP] Failed to send packet: {}", e.getMessage());
        }
    }
    ...
    private void relayTcpPacketToClients(LanPacket packet, String excludedUsername) {
        ...
        try {
            ...
            AmpereClientAddon.LOG.warn("[TCP] Failed to relay packet: {}", e.getMessage());
        }
    }
    ...
    private void stopTcp() {
        try {
            ...
            AmpereClientAddon.LOG.warn("[TCP] Stop error: {}", e.getMessage());
        }
    }
    ...
    private void checkAndBroadcastMacroListChanges() {
        try {
            ...
            AmpereClientAddon.LOG.error("[Ampere-LAN] Failed to check macro list changes", e);
        }
    }
    ...
    public void broadcastMacroList() {
        ...
        List<AmpereMacro> meteorMacros = MeteorMacroAdapter.getMeteorMacros();
        for (AmpereMacro macro : meteorMacros) {
    ...
    private void sendMacroData(String senderUsername, AmpereMacro macro, byte sourceType) {
    ...
    private Map<String, String> snapshotMeteorMacros() {
        ...
        for (AmpereMacro macro : MeteorMacroAdapter.getMeteorMacros()) {
    ...
    private void handleMacroData(LanPacket packet, boolean relayIfHost) {
        ...
        try {
            ...
            AmpereClientAddon.LOG.error("[Ampere-LAN] Failed to deserialize macro", e);
        }
    }
    ...
    private void importSharedMacroIfMissing(AmpereMacro macro, String sender) {
        if (macro == null || macro.name == null || macro.name.isBlank()) return;

        macro.sanitizeForSharing();

        AmpereMacroManager manager = AmpereMacroManager.get();
        AmpereMacro existing = manager.get(macro.name);
        if (existing != null) {

            AmpereMacro fresh = macro.deepCopy(existing.name);
            existing.actions.clear();
            existing.actions.addAll(fresh.actions);
            existing.loop = fresh.loop;
            existing.loopCount = fresh.loopCount;
            manager.save();
            AmpereMessaging.sendPrefixed("§aUpdated macro from " + sender + ": " + existing.name);
            return;
        }

        AmpereMacro installed = macro.deepCopy(macro.name);
        manager.add(installed);
        AmpereMessaging.sendPrefixed("§aReceived macro from " + sender + ": " + installed.name);
    }
    ...
    private void handleMacroRequest(LanPacket packet) {
        ...
        AmpereMacro macro = AmpereMacroManager.get().get(requestedName);
        if (macro != null) {
            sendMacroData(myUsername, macro, (byte)2);
            return;
        }

        for (AmpereMacro meteorMacro : MeteorMacroAdapter.getMeteorMacros()) {
    ...
    private void handlePresetData(LanPacket packet) {
        ...
        if (AmperePresetManager.get().importSharedPreset(p.presetName, p.presetJson, p.senderUsername)) {
    ...
    private void handleQueueSync(String queueData, String senderUsername) {
        try {
            ...
            AmpereSharedState.QueuedPacket.resetIdCounter();
            List<AmpereSharedState.QueuedPacket> receivedQueue =
                AmpereClipboardHelper.deserializeQueueFromBase64(queueData);

            if (receivedQueue == null || receivedQueue.isEmpty()) {
                AmpereMessaging.sendPrefixed("§cReceived empty queue");
                return;
            }

            AmpereSharedState.get().setDelayedPackets(receivedQueue);
            AmpereMessaging.sendPrefixed("§aReceived queue: " + receivedQueue.size() + " packets");
        } catch (Exception e) {
            AmpereClientAddon.LOG.error("[Ampere-LAN] Failed to process queue sync", e);
            AmpereMessaging.sendPrefixed("§cFailed to receive queue");
        }
    }
    ...
    private void handleChatMessage(String senderUsername, String message) {
        ...
        if ("__SYNC__".equals(safeSender)) {
            AmpereMessaging.sendPrefixed(safeMessage);
            return;
        }

        if ("__STOP_ALL__".equals(safeSender)) {
            ampere.util.macro.MacroExecutor.stop();
            AmpereMessaging.sendPrefixed("§c[LAN] Macros stopped by " + safeMessage);
            return;
        }
        ...
        if (isExecMethodControl) {
            ...
            if (isHost) {
                ...
                if (!originator.equals(myUsername)) {
                    AmpereMessaging.sendPrefixed("§eExecution method set to "
        ...
        AmpereMessaging.send("[LAN] <" + safeSender + "> " + safeMessage);
    }
    ...
    private void handleRequestSync(String senderUsername, boolean isMacro, String macroName, String command) {
        ...
        if (!isHost) {
            AmpereClientAddon.LOG.warn("[Sync] Non-host received REQUEST_SYNC from {} - ignoring", senderUsername);
            return;
        }

        AmpereClientAddon.LOG.info("[Sync] Received sync request from {}: isMacro={}, name={}, cmd={}",
    ...
    private void handleOffsetSync(String senderUsername, String targetUsername, int offsetMs) {
        ...
        if (info != null) {
            ...
            AmpereClientAddon.LOG.info("[Sync] Offset for {} set to {}ms by {}", targetUsername, offsetMs, senderUsername);
        }
    ...
    public void setExecutionMethod(ExecutionMethod method) {
        ...
        if (syncState != SyncState.IDLE && syncState != SyncState.DONE) {
            AmpereMessaging.sendPrefixed("§cCannot change execution method while LAN Sync is active.");
            return;
        }
        if (sessionId == null || isHost) {
            ...
            if (previous != next) {
                AmpereMessaging.sendPrefixed("§eExecution method set to " + next.name());
            }
            fireCallback(onPeerStatusChanged);
        } else {
            sendTcpPacket(new LanPacket.ChatMessagePacket(sessionId, myUsername,
                    "__EXEC_METHOD__:" + next.name()));
            AmpereMessaging.sendPrefixed("§eRequested execution method: " + next.name());
        }
    }
    ...
    public void broadcastStopAll() {
        ...
        ampere.util.macro.MacroExecutor.stop();
        AmpereMessaging.sendPrefixed("§c[LAN] Macros stopped by " + myUsername);
    ...
    public boolean sendChatMessage(String message) {
        ...
        if (!isInSession()) {
            AmpereMessaging.sendPrefixed("Join a LAN Sync session first.");
            return false;
        }
        ...
        if (isHost) {
            ...
            request.executionMethod = executionMethod.ordinal();
            sendTcpPacket(request);
            AmpereMessaging.sendPrefixed("§eRequested sync chat from host...");
        }
        return true;
    }
    ...
    public Map<String, Map<String, AmpereMacro>> getAllRemoteMacros() { return new java.util.HashMap<>(clientMacroLists); }
    public Map<String, Map<String, AmpereMacro>> getAllRemoteMeteorMacros() { return new java.util.HashMap<>(remoteMeteorMacros); }

    public List<String> getPeersMissingMacro(String macroName) {
        List<String> missing = new ArrayList<>();
        if (macroName == null || macroName.isBlank()) return missing;

        Map<String, Map<String, AmpereMacro>> remote = getAllRemoteMacros();
        java.util.TreeSet<String> peers = new java.util.TreeSet<>(connectedClients.keySet());
        for (String peer : peers) {
            if (peer.equals(myUsername)) continue;
            Map<String, AmpereMacro> peerMacros = remote.get(peer);
    ...
    public Map<String, String> getMissingAssignedMacros(Map<String, String> assignments) {
        ...
        for (Map.Entry<String, String> entry : new java.util.LinkedHashMap<>(assignments).entrySet()) {
            ...
            if (username.equals(myUsername)) {
                if (AmpereMacroManager.get().get(macroName) == null) {
                    missing.put(username, macroName);
                }
                continue;
            }

            Map<String, AmpereMacro> peerMacros = remote.get(username);
            if (peerMacros == null || !peerMacros.containsKey(macroName)) {
                missing.put(username, macroName);
            }
        }
    ...
    public List<String> getRemoteMacros() {
        ...
        ownMacros.addAll(AmpereCompatManager.getMeteorMacroNames());

        for (Map.Entry<String, Map<String, AmpereMacro>> entry : clientMacroLists.entrySet()) {
    ...
    public void importMacro(String clientUsername, String macroName) {
        Map<String, AmpereMacro> clientMacros = clientMacroLists.get(clientUsername);
        if (clientMacros == null || !clientMacros.containsKey(macroName)) {
            AmpereMessaging.sendPrefixed("§cMacro not found: " + macroName);
            return;
        }

        AmpereMacro remoteMacro = clientMacros.get(macroName);
        String newName = macroName;
        int suffix = 1;
        while (AmpereMacroManager.get().get(newName) != null) {
            newName = macroName + " (" + suffix + ")";
            suffix++;
        }

        AmpereMacro newMacro = new AmpereMacro(newName);
        newMacro.actions.addAll(remoteMacro.actions);
        AmpereMacroManager.get().add(newMacro);
        AmpereMacroManager.get().save();

        AmpereMessaging.sendPrefixed("§aImported macro: " + newName);
        AmpereClientAddon.LOG.info("[Ampere-LAN] Imported macro '{}' from {}", newName, clientUsername);
    }
    ...
    public List<String> getRemoteMacroNamesForPeer(String peerUsername) {
        Map<String, AmpereMacro> peerMacros = clientMacroLists.get(peerUsername);
    ...
    public boolean shareMacroWithPeers(String macroName) {
        ...
        if (macroName == null || macroName.isBlank()) {
            AmpereMessaging.sendPrefixed("§cSelect a macro first.");
            return false;
        }

        AmpereMacro macro = AmpereMacroManager.get().get(macroName);
        if (macro == null) {
            AmpereMessaging.sendPrefixed("§cMacro not found: " + macroName);
            return false;
        }
        ...
        sendMacroData(myUsername, macro, (byte)2);
        AmpereMessaging.sendPrefixed("§aSent macro to peers: " + macro.name);
        return true;
    }

    public void sharePreset(String presetName) {
        if (sessionId == null) {
            AmpereMessaging.sendPrefixed("§cNot in session.");
            return;
        }

        java.io.File file = new java.io.File(new java.io.File(AmpereClientAddon.FOLDER, "presets"), presetName + ".json");
        if (!file.exists()) {
            AmpereMessaging.sendPrefixed("§cPreset not found: " + presetName);
            return;
        }
        ...
        try {
            ...
            AmpereMessaging.sendPrefixed("§aShared preset '" + presetName + "' with LAN.");
        } catch (IOException e) {
            AmpereMessaging.sendPrefixed("§cFailed to read preset: " + e.getMessage());
        }
    }

    public void requestMacro(String macroName) {
        if (sessionId == null || !isInSession()) {
            AmpereMessaging.sendPrefixed("§cNot in a LAN Sync session!");
            return;
        }
        sendTcpPacket(new LanPacket.RequestMacroPacket(sessionId, macroName));
        AmpereMessaging.sendPrefixed("§eRequesting macro: " + macroName);
    }
