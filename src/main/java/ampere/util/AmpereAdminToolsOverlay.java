...
import ampere.gui.screen.AmpereForceOpPreviewScreen;
import ampere.gui.screen.AmpereAdminItemOptionScreen;
import ampere.gui.screen.AmpereAdminItemStructuredScreen;
import ampere.gui.screen.AmpereItemPickerScreen;
...
import ampere.gui.vanillaui.components.CompactFieldFactory;
import ampere.gui.vanillaui.components.CompactForm;
import ampere.gui.vanillaui.components.CompactToolbar;
...
import ampere.gui.vanillaui.components.TabStrip;
...
public final class AmpereAdminToolsOverlay extends AmpereOverlayBase {
    private static AmpereAdminToolsOverlay sharedOverlay;
    ...
    private AmpereAdminToolsOverlay() {
        super("admin-tools-panel", DEFAULT_W, DEFAULT_H);
        panelX = 88;
        panelY = 32;
    }

    public static synchronized AmpereAdminToolsOverlay getSharedOverlay() {
        if (sharedOverlay == null) sharedOverlay = new AmpereAdminToolsOverlay();
    ...
    public void render(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float delta) {
        ...
        applyAutoHeight();
        AmpereWindowLayout bounds = clampToScreen(this);
        panelX = bounds.x;
        panelY = bounds.y;
    ...
    private void renderBody(UiContext context, PackBuiltinModules.AdminToolsModule module, AmpereWindowLayout bounds, int mouseX, int mouseY) {
        int x = bounds.x + 5;
    ...
    private void renderBindRow(UiContext context, PackModule module, int x, int y, int w, String label, String optionId, int mouseX, int mouseY) {
        ...
        Button.render(context, bind, focused ? "Press key" : AmpereBindUtil.getBindName(parseInt(module.value(option.id()), -1)), Button.Tone.NORMAL, bind.contains(mouseX, mouseY), focused);
    ...
    private void handleAction(PackBuiltinModules.AdminToolsModule module, Hit hit) {
        ...
        switch (hit.value) {
            ...
            case "edit-lore" -> openItemOptionEditor(module, "nbt-item-lore", AmpereAdminItemOptionScreen.Mode.LORE);
            case "edit-enchants" -> openItemStructuredEditor(module, "nbt-enchants", AmpereAdminItemStructuredScreen.Mode.ENCHANTMENTS);
            case "edit-attributes" -> openItemStructuredEditor(module, "nbt-attributes", AmpereAdminItemStructuredScreen.Mode.ATTRIBUTES);
            case "edit-custom-data" -> openItemOptionEditor(module, "nbt-custom-data", AmpereAdminItemOptionScreen.Mode.RAW);
            case "edit-components" -> openItemOptionEditor(module, "nbt-item-components", AmpereAdminItemOptionScreen.Mode.RAW);
            ...
            case "forceop-preview" -> {
                Minecraft mc = Minecraft.getInstance();
                if (mc != null) mc.setScreen(new AmpereForceOpPreviewScreen(mc.screen, module.getForceOpPasswords()));
            }
            default -> {
            }
        }
    ...
    private void openItemOptionEditor(PackModule module, String optionId, AmpereAdminItemOptionScreen.Mode mode) {
        Minecraft mc = Minecraft.getInstance();
        PackModuleOption option = option(module, optionId);
        if (mc != null && option != null) mc.setScreen(new AmpereAdminItemOptionScreen(mc.screen, module, option, mode));
    }

    private void openItemStructuredEditor(PackModule module, String optionId, AmpereAdminItemStructuredScreen.Mode mode) {
        Minecraft mc = Minecraft.getInstance();
        PackModuleOption option = option(module, optionId);
        if (mc != null && option != null) mc.setScreen(new AmpereAdminItemStructuredScreen(mc.screen, module, option, mode));
    }
    ...
    private void openItemPicker(PackModule module, UiBounds anchor) {
        ...
        Minecraft mc = Minecraft.getInstance();
        if (mc != null) {
            mc.setScreen(new AmpereItemPickerScreen(mc.screen, module.value("nbt-item-id"), value -> {
    ...
    private void startInventoryItemPicker(PackBuiltinModules.AdminToolsModule module) {
        ...
        if (mc == null || mc.player == null) {
            AmpereNotifications.warning("Join a world first.");
            return;
        }
        finishEditing(true);
        clearDropdown();
        ...
        AmpereSharedState.get().setCaptureCancelCallback(this::cancelInventoryItemPicker);
        AmpereNotifications.show("Right-click an inventory item to pick it.", 0xFF35D873);
    ...
    public String getAbstractContainerScreenCaptureHoverText(Slot slot, String itemName, String registryId) {
        ...
        int visibleSlot = AmpereInventoryHelper.toUserVisibleSlot(Minecraft.getInstance(), slot);
    ...
    public boolean onInventoryItemStackCapture(Slot slot) {
        ...
        if (module == null || stack.isEmpty()) {
            AmpereNotifications.warning("Pick a non-empty item slot.");
            return true;
        }
        module.fillItemEditorFromStack(stack, false);
        AmpereNotifications.show("NBT editor filled from " + BuiltInRegistries.ITEM.getKey(stack.getItem()) + ".", 0xFF35D873);
    ...
    public void cancelInventoryItemPicker() {
        ...
        AmpereNotifications.warning("Item pick cancelled.");
        finishInventoryItemPicker();
    }
    ...
    private void finishInventoryItemPicker() {
        ...
        AmpereSharedState.get().setCaptureCancelCallback(null);
    ...
    public boolean mouseDragged(double mouseX, double mouseY, int button, double deltaX, double deltaY) {
        if (button == 0 && headerDragging) {
            AmpereWindowLayout current = getBounds();
            setBounds(new AmpereWindowLayout(
