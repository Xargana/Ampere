...
import ampere.util.macro.PacketClickAction;
...
public class AmpereFabricatorOverlay extends AmpereOverlayBase {
    private static final Minecraft MC = Minecraft.getInstance();
    private static final CompactTheme THEME = new CompactTheme();
    private static AmpereFabricatorOverlay sharedOverlay;
    ...
    private enum FabricatorAction {
        CLICK("Click", "Use Left or Right on the resolved slot.") {
            ...
            @Override AmpereDropAction toPacketAction(boolean dropWholeStack) { return AmpereDropAction.PICKUP; }
        },
        QUICK_MOVE("Quick Move", "Shift-click the resolved slot.") {
            @Override AmpereDropAction toPacketAction(boolean dropWholeStack) { return AmpereDropAction.QUICK_MOVE; }
        },
        PICKUP_ALL("Pick Up All", "Double-click the resolved slot. Usually needs you to hold a matching stack first.") {
            @Override AmpereDropAction toPacketAction(boolean dropWholeStack) { return AmpereDropAction.PICKUP_ALL; }
        },
        ...
        DROP("Drop", "Drop from the resolved slot. If slot and item are both set, both must match.") {
            ...
            @Override AmpereDropAction toPacketAction(boolean dropWholeStack) {
                return dropWholeStack ? AmpereDropAction.DROP_STACK : AmpereDropAction.DROP_ITEM;
            }
        ...
        abstract AmpereDropAction toPacketAction(boolean dropWholeStack);
    }
    ...
    private final List<AmpereChatField> textFields = new ArrayList<>();

    private AmpereChatField slotField;
    private AmpereChatField itemNameField;
    private AmpereChatField timesField;
    private AmpereChatField craftSearchField;
    ...
    private AmperePacketClick.Target packetClickTarget = null;
    ...
    private final List<AmpereCraftingHelper.CraftableRecipeOption> craftableRecipes = new ArrayList<>();
    private List<AmpereCraftingHelper.CraftableRecipeOption> filteredCraftableRecipes = new ArrayList<>();
    ...
    private AmpereCraftingHelper.CraftableRecipeOption selectedCraftOption = null;
    ...
    private int getClickSelectorOptionCount() {
        return currentAction.isPacketAction() ? AmperePacketClick.Mode.values().length : 2;
    }

    private AmperePacketClick.Mode getCurrentPacketMode() {
        return AmperePacketClick.Mode.byIndex(currentButtonIndex);
    }
    ...
    private int getEffectiveButton() {
        ...
        AmpereDropAction packetAction = currentAction.toPacketAction(dropWholeStack);
    ...
    public AmpereFabricatorOverlay(AbstractContainerScreen<?> parentScreen) {
    ...
    public static synchronized AmpereFabricatorOverlay getSharedOverlay(AbstractContainerScreen<?> parentScreen) {
        if (sharedOverlay == null) {
            sharedOverlay = new AmpereFabricatorOverlay(parentScreen);
    ...
    public boolean hasTextFieldFocused() {
        for (AmpereChatField field : textFields) {
    ...
    private void initWidgets() {
        ...
        slotField = new AmpereChatField(MC, MC.font, fieldX, getSlotRowY(), fieldWidth, 16, false);
        ...
        slotField.setChangedListener(text -> {
            ...
            try {
                ...
                int slotId = AmpereInventoryHelper.resolveConfiguredHandlerSlot(MC, stableSlot);
        ...
        itemNameField = new AmpereChatField(MC, MC.font, fieldX, getItemRowY(), fieldWidth, 16, false);
        ...
        timesField = new AmpereChatField(MC, MC.font, timesX, timesY, timesWidth, 16, false);
        ...
        if (isCraftMode()) {
            ...
            craftSearchField = new AmpereChatField(MC, MC.font, panelX + 10, searchY, PANEL_WIDTH - 20, 16, false);
    ...
    public boolean mouseClicked(double mouseX, double mouseY, int button) {
        ...
        if (button == 0 && mouseX >= panelX && mouseX <= panelX + PANEL_WIDTH &&
            ...
            AmpereWindowLayout bounds = new AmpereWindowLayout(panelX, panelY, PANEL_WIDTH, collapsed ? 18 : PANEL_HEIGHT, visible, collapsed);
            ...
            if (mouseX >= listX && mouseX <= listX + listWidth && mouseY >= planListY && mouseY <= planListY + CRAFT_PLAN_HEIGHT && button == 0) {
                ...
                for (int index = firstIndex; index < plannedCraftEntries.size() && rowY < planListY + CRAFT_PLAN_HEIGHT; index++, rowY += 13) {
                    ...
                    if (mouseX >= listX && mouseX < listX + planContentWidth) {
                        ...
                        for (AmpereChatField field : textFields) field.setFocused(false);
                        return true;
                    }
                }
                if (mouseX >= listX && mouseX < listX + planContentWidth) {
        ...
        if (mouseY >= btnY && mouseY < btnY + BUTTON_HEIGHT && button == 0) {
            ...
            if (mouseX >= bx && mouseX < bx + bw) {
                ...
                AmpereMacroEditorOverlay macroEditor = null;
                for (IAmpereOverlay ov : AmpereOverlayManager.get().getOverlays()) {
                    if (ov instanceof AmpereMacroEditorOverlay) {
                        macroEditor = (AmpereMacroEditorOverlay) ov;
                        break;
                    }
                }
    ...
    private ItemTarget getItemTargetFromSlot(int slotId) {
        ...
        int visibleSlot = AmpereInventoryHelper.toUserVisibleSlot(MC, slot.index);
    ...
    private void refreshCraftableRecipes(boolean force) {
        ...
        if (!force && now - lastCraftRefreshAt < AmpereCraftingHelper.CRAFT_REFRESH_DEBOUNCE_MS) return;
        lastCraftRefreshAt = now;

        craftableRecipes.clear();
        filteredCraftableRecipes = new ArrayList<>();

        craftableRecipes.addAll(AmpereCraftingHelper.getCraftableRecipes(MC));
        readyCraftableRecipeCount = 0;
        for (AmpereCraftingHelper.CraftableRecipeOption option : craftableRecipes) {
    ...
    private void updateCraftRecipeFilter(String query, boolean resetScroll) {
        ...
        filteredCraftableRecipes = AmpereCraftingHelper.filterRecipes(craftableRecipes, query);
        int maxScroll = Math.max(0, filteredCraftableRecipes.size() - CRAFT_LIST_ROWS);
    ...
    private void selectCraftRecipe(AmpereCraftingHelper.CraftableRecipeOption option) {
        ...
        if (itemNameField != null) itemNameField.setText(savedItemNameValue);
        setStatus(
            "Selected craft: " + option.label + " | " + (option.craftSource == AmpereCraftingHelper.CraftSource.TABLE_3X3 ? "Table" : "2x2") + " | " + AmpereCraftingHelper.getAvailabilityLabel(option),
    ...
    private AmpereCraftingHelper.CraftableRecipeOption resolveCraftOption(CraftAction.CraftEntry entry) {
        if (entry == null) return null;
        return AmpereCraftingHelper.findInList(craftableRecipes, entry.recipeKey, entry.recipeId);
    }
    ...
    private void selectCraftPlanEntry(int index) {
        ...
        if (entry != null) {
            ...
            AmpereCraftingHelper.CraftableRecipeOption option = resolveCraftOption(entry);
            if (option != null) {
    ...
    public void render(GuiGraphicsExtractor context, int mouseX, int mouseY, float delta) {
        ...
        AmpereText.draw(context, MC.font, "Action", AmpereText.Tone.MUTED, labelX, getActionRowY() + 3, false);
        ...
        if (showsSlotItemRows()) {
            AmpereText.draw(context, MC.font, "Slot", AmpereText.Tone.MUTED, labelX, getSlotRowY() + 3, false);
            AmpereText.draw(context, MC.font, "Item", AmpereText.Tone.MUTED, labelX, getItemRowY() + 3, false);
        }

        if (showsTargetFields()) {
            if (isClickSelectorRelevant() || isDropModeRelevant()) {
                AmpereText.draw(context, MC.font, isDropModeRelevant() ? "Drop Mode" : "Click", AmpereText.Tone.MUTED, labelX, getOptionRowY() + 3, false);
            ...
            AmpereText.draw(context, MC.font, getTimesLabel(), AmpereText.Tone.MUTED, labelX, getStandardTimesRowY() + 3, false);
        } else {
            refreshCraftableRecipes(false);
            int listX = getCraftListX();
            int listWidth = getCraftListWidth();
            ...
            if (plannedCraftEntries.isEmpty()) {
                ...
                ampere.gui.vanillaui.UiScissorStack.global().push(context,
                    ampere.gui.vanillaui.UiBounds.of(listX + 1, planListY + 1, Math.max(0, listWidth - 2), Math.max(0, CRAFT_PLAN_HEIGHT - 2)));
            ...
            if (selectedCraftResult.isEmpty()) {
                AmpereText.draw(context, MC.font, "Recipe: none selected", AmpereText.Tone.MUTED, listX, getCraftSummaryY(), false);
            } else {
            ...
            AmpereText.draw(context, MC.font, summaryText.getString(), AmpereColors.textMuted(), listX, getCraftSummaryY(), false);
            }

            AmpereText.draw(context, MC.font, "Amount", AmpereText.Tone.MUTED, labelX, getCraftAmountRowY() + 3, false);
            ...
            ampere.gui.vanillaui.UiScissorStack.global().push(context,
                ampere.gui.vanillaui.UiBounds.of(listX + 1, listY + 1, Math.max(0, listWidth - 2), Math.max(0, CRAFT_LIST_HEIGHT - 2)));
            ...
            if (filteredCraftableRecipes.isEmpty()) {
                ...
                for (int index = startIndex; index < filteredCraftableRecipes.size() && rowY < listY + CRAFT_LIST_HEIGHT; index++, rowY += LINE_HEIGHT) {
                    AmpereCraftingHelper.CraftableRecipeOption option = filteredCraftableRecipes.get(index);
                    ...
                    Component richLabel = Component.empty()
                        ...
                        .append(Component.literal(" x" + option.result.getCount() + " | " + AmpereCraftingHelper.getAvailabilityLabel(option)));
                    CompactListRenderer.drawRow(
                        context,
                        MC.font,
                        richLabel,
            ...
            String recipeCountLine = AmpereText.trimToWidth(
                MC.font,
                readyCraftableRecipeCount + " ready / " + craftableRecipes.size() + " total",
                listWidth,
                AmpereText.Tone.MUTED
            );
            AmpereText.draw(context, MC.font, recipeCountLine, AmpereText.Tone.MUTED, listX, listY + CRAFT_LIST_HEIGHT + 2, false);
        }
        ...
        for (int i = 0; i < actionLines.size(); i++) {
            ...
            AmpereText.draw(context, MC.font, actionLines.get(i), AmpereText.Tone.MUTED, panelX + 10, lineY, false);
        }
    ...
    public void onSlotClick(Slot slot, int button) {
        ...
        int stableSlotId = AmpereInventoryHelper.toUserVisibleSlot(MC, slot.index);
        ...
        if (currentAction.isPacketAction()) {
            ...
            packetClickTarget = new AmperePacketClick.Target(
                handler.containerId,
                handler.getStateId(),
                slot.index,
                stableSlotId,
    ...
    private void sendToMacro(AmpereMacroEditorOverlay macroEditor) {
        try {
            if (isCraftMode()) {
            ...
            if (currentAction.isPacketAction()) {
                AmperePacketClick.Target target = getEffectivePacketClickTarget();
                if (target == null) {
            ...
            AbstractContainerMenu handler = getActiveHandler();
            int resolvedSlot = enteredSlot == null ? -1 : AmpereInventoryHelper.resolveConfiguredHandlerSlot(MC, enteredSlot);
            ...
            if (currentAction == FabricatorAction.DROP) {
                ampere.util.macro.DropAction dropAction = new ampere.util.macro.DropAction();
                dropAction.useHandlerSlots = true;
                dropAction.mode = dropWholeStack
                    ? ampere.util.macro.DropAction.DropMode.ALL
                    : ampere.util.macro.DropAction.DropMode.TIMES;
            ...
            ampere.util.macro.ItemAction itemAction = new ampere.util.macro.ItemAction();
    ...
    private AmperePacketClick.Target getEffectivePacketClickTarget() {
    ...
    private void send(boolean queue) {
        ...
        try {
            ...
            if (currentAction == FabricatorAction.DROP && !queue) {
                int queuedCount = AmpereSharedState.get().flushDelayedPackets(handler);
                int sent = AmpereDropHelper.dropFromHandlerSlot(MC, slot, dropWholeStack ? 0 : repeats);
                if (sent <= 0) {
            ...
            if (!queue) {
                AmpereSharedState.get().flushDelayedPackets(handler);
                net.minecraft.world.inventory.ContainerInput actionType = currentAction.toPacketAction(dropWholeStack).toContainerInput();
                int effectiveButton = getEffectiveButton();
                for (int i = 0; i < repeats; i++) {
                    AmpereInventoryClickHelper.click(MC, slot, effectiveButton, actionType);
                }
                return;
            }
        ...
        try {
            ...
            if (queue) {
                for (Packet<?> packet : packets) {
                    AmpereSharedState.get().enqueueExactPacket(packet);
                }
            ...
            int queuedCount = AmpereSharedState.get().flushDelayedPackets(handler);
            for (Packet<?> packet : packets) {
                AmpereSharedState.get().sendPacketBypassDelay(handler, packet);
            }
            if (queuedCount > 0) {
    ...
    private void sendCraft(boolean queue) {
        ...
        if (queue) {
            for (CraftAction.CraftEntry entry : entries) {
                ...
                if (option == null) {
                    ...
                    AmpereMessaging.sendPrefixed("§c" + msg);
                    return;
                }
                ...
                List<Packet<?>> packets = AmpereCraftingHelper.buildCraftSequence(MC, entry.recipeKey, entry.recipeId, desiredAmount);
                if (packets.isEmpty()) {
                ...
                for (Packet<?> packet : packets) {
                    AmpereSharedState.get().enqueuePacket(packet);
                }
            }
            return;
        }
        ...
        Thread craftThread = new Thread(() -> {
            AmpereCraftingHelper.CraftExecutionResult result = null;
            for (CraftAction.CraftEntry entry : entries) {
                AmpereCraftingHelper.CraftableRecipeOption option = AmpereCraftingHelper.findCraftableRecipe(MC, entry.recipeKey);
                if (option == null) option = AmpereCraftingHelper.findCraftableRecipe(MC, entry.recipeId);
                if (option == null) {
                    result = AmpereCraftingHelper.CraftExecutionResult.failure("Recipe not found: " + entry.resultName);
                    break;
                }
                int desiredAmount = AmpereCraftingHelper.getEffectiveRequestedOutput(option, entry.amount, entry.useMaxAmount);
                if (desiredAmount <= 0) {
                    result = AmpereCraftingHelper.CraftExecutionResult.failure("No space or materials for " + entry.resultName + ".");
                    break;
                }

                result = AmpereCraftingHelper.executeCraftImmediately(MC, entry.recipeKey, entry.recipeId, desiredAmount);
                if (!result.success) break;
            }
            craftExecutionInProgress = false;
            AmpereCraftingHelper.CraftExecutionResult finalResult = result;
            if (finalResult != null && !finalResult.success) {
                MC.execute(() -> {
                    setStatus(finalResult.message, ChatFormatting.RED);
                    AmpereMessaging.sendPrefixed("§c" + finalResult.message);
                });
            }
        }, "Ampere-Fabricator-Craft");
    ...
    private void sendFabricatedPackets(int slot, int repeats, boolean queue, ClientPacketListener handler) {
        ...
        int queuedCount = AmpereSharedState.get().flushDelayedPackets(handler);
        for (Packet<?> packet : packets) {
            AmperePacketSender.send(packet);
        }
    ...
    private String getTargetSummary() {
        if (isCraftMode()) {
            ...
            String source = selectedCraftOption != null && selectedCraftOption.craftSource == AmpereCraftingHelper.CraftSource.TABLE_3X3 ? "Table" : "2x2";
            String availability = selectedCraftOption != null ? AmpereCraftingHelper.getAvailabilityLabel(selectedCraftOption) : "Unavailable";
    ...
    private AbstractContainerMenu getActiveHandler() {
        ...
        return parentScreen != null ? ((ampere.mixin.accessor.AbstractContainerScreenAccessor) parentScreen).Ampere$getMenu() : null;
    }
