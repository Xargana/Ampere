...
public final class AmperePerf {
    ...
    public static void endJoinSpike(String name, long startNanos, long thresholdNanos) {
        ...
        if (joinTicksRemaining > 0 && elapsed >= thresholdNanos) {
            AmpereClientAddon.LOG.warn("[AmperePerf] join spike {}={}ms",
                name,
    ...
    private static void record(String name, long elapsed) {
        ...
        if (samples % REPORT_EVERY == 0L) {
            ...
            AmpereClientAddon.LOG.info("[AmperePerf] {} avg={}ms max={}ms samples={}",
                name,
    ...
    public static void beginJoinWindow() {
        ...
        AmpereClientAddon.LOG.info("[AmperePerf] join profiling window started.");
    }
    ...
    public static boolean enabled() {
        if (Boolean.getBoolean("Ampere.perf")) return true;
