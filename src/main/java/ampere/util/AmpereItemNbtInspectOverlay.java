...
import ampere.modules.AmpereAdminToolsBridge;
...
public class AmpereItemNbtInspectOverlay extends AmpereOverlayBase {
    private static final Minecraft MC = Minecraft.getInstance();
    private static final int SCROLLBAR_WIDTH = InspectorLayout.SCROLLBAR_WIDTH;
    private static AmpereItemNbtInspectOverlay sharedOverlay;

    private final Font textRenderer;
    private final CompactTheme theme = new CompactTheme();
    private AmpereItemNbtInspector.ItemInspection inspection;
    ...
    public AmpereItemNbtInspectOverlay(Font textRenderer) {
    ...
    public static AmpereItemNbtInspectOverlay getSharedOverlay(Font textRenderer) {
        if (textRenderer == null) return null;
        if (sharedOverlay == null) sharedOverlay = new AmpereItemNbtInspectOverlay(textRenderer);
        return sharedOverlay;
    }

    public static boolean openGlobal(ItemStack stack) {
        if (stack == null || stack.isEmpty() || MC.font == null) return false;
        AmpereItemNbtInspectOverlay overlay = getSharedOverlay(MC.font);
        if (overlay == null) return false;
        int screenW = MC.getWindow() == null ? 640 : AmpereUiScale.getVirtualScreenWidth();
    ...
    public void open(ItemStack stack, int anchorX, int anchorY) {
        ...
        this.inspection = AmpereItemNbtInspector.inspect(stack);
        this.rawView = false;
        this.visible = true;
        ...
        this.wrapDirty = true;
        int screenW = MC.getWindow() == null ? 640 : AmpereUiScale.getVirtualScreenWidth();
        int screenH = MC.getWindow() == null ? 360 : AmpereUiScale.getVirtualScreenHeight();
        ...
        setBounds(new AmpereWindowLayout(Math.max(8, Math.min(anchorX, screenW - width - 8)),
    ...
    public boolean mouseDragged(double mouseX, double mouseY, int button, double deltaX, double deltaY) {
        ...
        setBounds(new AmpereWindowLayout((int) Math.round(mouseX - dragOffsetX), (int) Math.round(mouseY - dragOffsetY), panelWidth, panelHeight, visible, collapsed));
        return true;
    }
    ...
    private List<AmpereItemNbtInspector.InspectionLine> currentLines() {
    ...
    private void ensureWrappedLines(int maxWidth) {
        ...
        for (AmpereItemNbtInspector.InspectionLine line : currentLines()) {
    ...
    private void wrapLine(AmpereItemNbtInspector.InspectionLine line, int maxWidth, List<WrappedInspectionLine> target) {
    ...
    private void wrapKeyValue(String prefix, String value, int valueColor, int prefixWidth, int continuationOffset, int maxWidth,
        ...
        if (value == null || value.isEmpty()) {
            target.add(WrappedInspectionLine.of(List.of(
                new AmpereItemNbtInspector.TextToken(prefix, AmpereColors.packetGray()),
                new AmpereItemNbtInspector.TextToken("", valueColor)
            ), 0));
            return;
        }
    ...
    private void drawInspectionLine(GuiGraphicsExtractor context, WrappedInspectionLine line, int x, int y) {
        ...
        for (AmpereItemNbtInspector.TextToken token : line.tokens()) {
    ...
    private void wrapTokenLine(List<AmpereItemNbtInspector.TextToken> source, int maxWidth, List<WrappedInspectionLine> target) {
        if (source == null || source.isEmpty()) {
            target.add(WrappedInspectionLine.plain("", 0, AmpereColors.textMuted()));
            return;
        }
        ...
        List<AmpereItemNbtInspector.TextToken> current = new ArrayList<>();
        int currentWidth = 0;
        boolean continuation = false;

        for (AmpereItemNbtInspector.TextToken token : source) {
            if (token == null || token.text() == null || token.text().isEmpty()) continue;
            ...
            while (!remaining.isEmpty()) {
                ...
                current.add(new AmpereItemNbtInspector.TextToken(piece, token.color()));
    ...
    private WrappedInspectionLine keyValueWrappedLine(String prefix, String value, int valueColor, int offset) {
        List<AmpereItemNbtInspector.TextToken> tokens = new ArrayList<>();
        if (prefix != null && !prefix.isEmpty()) tokens.add(new AmpereItemNbtInspector.TextToken(prefix, AmpereColors.packetGray()));
        tokens.add(new AmpereItemNbtInspector.TextToken(value == null ? "" : value, valueColor));
        return WrappedInspectionLine.of(tokens, prefix == null || prefix.isEmpty() ? offset : 0);
    }

    private int leadingWhitespaceWidth(List<AmpereItemNbtInspector.TextToken> tokens) {
        int width = 0;
        for (AmpereItemNbtInspector.TextToken token : tokens) {
            if (token == null || token.text() == null || token.text().isEmpty()) continue;
    ...
    private void handleFooter(String label) {
        ...
        switch (label) {
            case "Copy Raw" -> {
                ...
                AmpereNotifications.copied("Copied raw item NBT.");
            }
            case "Copy Give" -> {
                MC.keyboardHandler.setClipboard(inspection.giveCommand());
                AmpereNotifications.copied("Copied item give command.");
            }
            case "Copy Nice" -> {
                MC.keyboardHandler.setClipboard(inspection.prettyCopyText());
                AmpereNotifications.copied("Copied item NBT summary.");
            }
            case "Send Admin" -> {
                if (AmpereAdminToolsBridge.openFilledAdminEditor(inspection.stack())) {
                    AmpereNotifications.show("Loaded item into Admin Tools.", 0xFF35D873);
                } else {
                    AmpereMessaging.sendPrefixed("Admin Tools: could not fill NBT editor.");
                }
            }
        }
    }
    ...
    private record WrappedInspectionLine(List<AmpereItemNbtInspector.TextToken> tokens, int offset) {
        static WrappedInspectionLine plain(String text, int offset, int color) {
            return new WrappedInspectionLine(List.of(new AmpereItemNbtInspector.TextToken(text, color)), offset);
        }

        static WrappedInspectionLine of(List<AmpereItemNbtInspector.TextToken> tokens, int offset) {
