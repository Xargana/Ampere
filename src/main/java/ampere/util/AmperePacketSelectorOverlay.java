...
public class AmperePacketSelectorOverlay extends AmpereOverlayBase {
    ...
    private final SearchableSelector<Class<? extends Packet<?>>> selector =
        new SearchableSelector<>(packetClass -> packetSearchKeys.computeIfAbsent(packetClass, AmperePacketSelectorOverlay::searchKey));
    ...
    public AmperePacketSelectorOverlay(Font textRenderer) {
        this.textRenderer = textRenderer;
        sortedC2sPackets.addAll(AmperePacketRegistry.getC2SPackets());
        sortedC2sPackets.sort(Comparator.comparing(AmperePacketNamer::getFriendlyName, String.CASE_INSENSITIVE_ORDER));
        sortedS2cPackets.addAll(AmperePacketRegistry.getS2CPackets());
        sortedS2cPackets.sort(Comparator.comparing(AmperePacketNamer::getFriendlyName, String.CASE_INSENSITIVE_ORDER));
        ...
        this.allPackets.sort(Comparator.comparing(AmperePacketNamer::getFriendlyName, String.CASE_INSENSITIVE_ORDER));
    ...
    private void finishOpen() {
        ...
        AmpereOverlayManager.get().bringToFrontParent(this);
    }
    ...
    private void renderPacketRowSimple(GuiGraphicsExtractor context, Class<? extends Packet<?>> packetClass, int x, int y, int width, int index) {
        ...
        int rowColor = selected ? AmpereColors.packetRowSelectedBg(false) : AmpereColors.packetRowBg(c2s, index, false);
        int color = selected ? AmpereColors.packetRowSelectedText() : AmpereColors.packetRowText(c2s, index);
        CompactSurfaces.tintedRow(context, x, y, width, rowHeight(), rowColor);

        String name = AmperePacketNamer.getFriendlyName(packetClass);
        ...
        CompactSurfaces.divider(context, x + 4, y + rowHeight() - 1, width - 8, AmpereColors.packetRowDivider());
    }
    ...
    private static String searchKey(Class<? extends Packet<?>> packetClass) {
        ...
        return (packetClass.getSimpleName() + '\n' + AmperePacketNamer.getFriendlyName(packetClass))
