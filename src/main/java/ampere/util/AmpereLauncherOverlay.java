...
import ampere.gui.vanillaui.direct.DirectUiButton;
import ampere.gui.vanillaui.direct.DirectUiInsets;
import ampere.gui.vanillaui.direct.DirectRenderContext;
import ampere.gui.vanillaui.direct.DirectCompactRow;
import ampere.gui.vanillaui.direct.DirectAccordionSection;
import ampere.gui.vanillaui.direct.DirectSpacer;
import ampere.gui.vanillaui.direct.DirectIconLabel;
import ampere.gui.vanillaui.direct.DirectMetricLabel;
import ampere.gui.vanillaui.direct.DirectPanel;
...
import ampere.gui.vanillaui.components.ConnectedButton;
import ampere.mixin.accessor.AmpereHandledScreenAccessor;
...
public class AmpereLauncherOverlay extends AmpereOverlayBase {
    ...
    private final AmpereMacroListOverlay macroListOverlay;
    private final AmpereFabricatorOverlay fabricatorOverlay;
    private final AmpereLANSyncOverlay lanSyncOverlay;
    private final AmpereQueueEditorOverlay queueEditorOverlay;
    private final AmperePacketLoggerOverlay packetLoggerOverlay;
    private final AmpereCustomFilterOverlay customFilterOverlay;
    private Supplier<AmperePacketLoggerOverlay> packetLoggerOverlaySupplier;
    private Supplier<AmpereServerInfoOverlay> serverInfoOverlaySupplier;
    ...
    private AmpereKeybindOverlay keybindOverlay;
    ...
    public AmpereLauncherOverlay(AmpereMacroListOverlay macroListOverlay,
                                   AmpereFabricatorOverlay fabricatorOverlay,
                                   AmpereLANSyncOverlay lanSyncOverlay,
                                   AmpereQueueEditorOverlay queueEditorOverlay,
                                   AmperePacketLoggerOverlay packetLoggerOverlay,
                                   AmpereCustomFilterOverlay customFilterOverlay) {
        this(macroListOverlay, fabricatorOverlay, lanSyncOverlay, queueEditorOverlay, packetLoggerOverlay, customFilterOverlay, "Ampere-launcher", true, true);
    }
    ...
    public AmpereLauncherOverlay(AmpereMacroListOverlay macroListOverlay,
        ...
        this.overlayId = overlayId == null || overlayId.isBlank() ? "Ampere-launcher" : overlayId;
    ...
    public void setKeybindOverlay(AmpereKeybindOverlay overlay) {
        this.keybindOverlay = overlay;
    }

    public void setServerDataOverlay(AmpereServerInfoOverlay overlay) {
        this.serverInfoOverlay = overlay;
        this.serverInfoOverlaySupplier = () -> this.serverInfoOverlay;
    }

    public void setPacketLoggerOverlaySupplier(Supplier<AmperePacketLoggerOverlay> supplier) {
        this.packetLoggerOverlaySupplier = supplier == null ? () -> this.packetLoggerOverlay : supplier;
    }

    public void setServerDataOverlaySupplier(Supplier<AmpereServerInfoOverlay> supplier) {
    ...
    private AmperePacketLoggerOverlay packetLoggerOverlay() {
        AmperePacketLoggerOverlay overlay = packetLoggerOverlaySupplier == null ? packetLoggerOverlay : packetLoggerOverlaySupplier.get();
        if (overlay != null) AmpereOverlayManager.get().register(overlay);
        return overlay;
    }

    private AmpereServerInfoOverlay serverInfoOverlay() {
        AmpereServerInfoOverlay overlay = serverInfoOverlaySupplier == null ? serverInfoOverlay : serverInfoOverlaySupplier.get();
        if (overlay != null) AmpereOverlayManager.get().register(overlay);
        return overlay;
    }
    ...
    private void buildUi() {
        ...
        DirectUiButton macrosButton = actionButton("Macros", UiAssets.ICON_MACROS, DirectUiButton.Variant.SECONDARY, () -> {
            AmpereMacroEditorOverlay macroEditorOverlay = AmpereMacroEditorOverlay.getSharedOverlay();
            if (macroEditorOverlay != null && macroEditorOverlay.isVisible()) {
                if (macroListOverlay != null) macroListOverlay.setVisible(false);
                AmpereOverlayManager.get().bringToFront(macroEditorOverlay);
        ...
        addPairRow(mainMenuSection.content(),
            actionButton("Logger", UiAssets.ICON_PACKET_LOGGER, DirectUiButton.Variant.SECONDARY, () -> {
                AmperePacketLoggerOverlay overlay = packetLoggerOverlay();
        ...
        addPairRow(mainMenuSection.content(),
            actionButton("Server Info", UiAssets.ICON_SERVER_INFO, DirectUiButton.Variant.SECONDARY, () -> {
                AmpereServerInfoOverlay overlay = serverInfoOverlay();
                if (overlay != null) overlay.toggle();
            }).setGrowX(true),
        ...
        if (includeScreenSection) {
            ...
            addPairRow(mainMenuSection.content(),
                actionButton("Close", null, DirectUiButton.Variant.SECONDARY,
                    closeWithoutPacketAction != null ? closeWithoutPacketAction : () -> AmpereGuiActions.closeCurrentScreen(MC, false)).setGrowX(true),
            ...
            addPairRow(mainMenuSection.content(),
                    actionButton("Title", null, DirectUiButton.Variant.SECONDARY, AmpereGuiClipboardUtil::copyGuiTitleJson).setGrowX(true),
    ...
    private DirectUiButton addFullButton(ampere.gui.vanillaui.direct.DirectUiColumn parent, String label, Identifier icon, Runnable action) {
        DirectUiButton button = actionButton(label, icon, DirectUiButton.Variant.SECONDARY, action).setGrowX(true);
        parent.add(button);
        return button;
    }

    private void addPairRow(ampere.gui.vanillaui.direct.DirectUiColumn parent, DirectUiButton left, DirectUiButton right) {
    ...
    public AmpereWindowLayout getBounds() {
        return new AmpereWindowLayout(panelX, panelY, panelWidth, panelHeight, visible, !mainMenuSection.isExpanded());
    }

    @Override
    public void setBounds(AmpereWindowLayout bounds) {
        if (bounds == null) return;
        AmpereWindowLayout clamped = clampToViewport(bounds);
        panelX = clamped.x;
        panelY = clamped.y;
    ...
    public void saveLayout() {
        AmpereSharedState.get().setWindowLayout(getOverlayId(), getBounds());
        AmpereConfig config = AmpereConfig.getGlobal();
        AmpereConfig.ModuleCategoryLayout layout = config.moduleCategoryLayouts.computeIfAbsent(SHARED_MAIN_MENU_LAYOUT, ignored -> new AmpereConfig.ModuleCategoryLayout());
        layout.x = panelX;
        layout.y = panelY;
    ...
    public void restoreLayout() {
        AmpereWindowLayout saved = AmpereSharedState.get().getWindowLayout(getOverlayId());
        boolean savedVisible = saved == null || saved.visible;
        boolean savedCollapsed = saved != null && saved.collapsed;
        AmpereConfig config = AmpereConfig.getGlobal();
        AmpereConfig.ModuleCategoryLayout layout = config.moduleCategoryLayouts.get(SHARED_MAIN_MENU_LAYOUT);
        if (layout == null) {
            AmpereConfig.ModuleCategoryLayout old = config.moduleCategoryLayouts.get(LEGACY_MODULE_UTILITIES_LAYOUT);
            if (old != null) {
                layout = new AmpereConfig.ModuleCategoryLayout();
                layout.x = old.x;
                layout.y = old.y;
        ...
        if (layout != null && layout.x >= 0 && layout.y >= 0) {
            setBounds(new AmpereWindowLayout(layout.x, layout.y, panelWidth, panelHeight, savedVisible, layout.collapsed || savedCollapsed));
        } else if (saved != null) {
            setBounds(saved);
        }
    }
    ...
    public void render(GuiGraphicsExtractor context, int mouseX, int mouseY, float delta) {
        ...
        AmpereWindowLayout clamped = clampToViewport(new AmpereWindowLayout(
            panelX,
            panelY,
            panelWidth,
            panelHeight,
    ...
    private AmpereWindowLayout clampToViewport(AmpereWindowLayout bounds) {
        DirectViewport viewport = surface.viewport();
        int margin = 4;
    ...
    private void sendDesync() {
        if (!AmpereGuiActions.desyncCurrentScreen(MC)) {
            AmpereMessaging.sendPrefixed("Failed to desync: no open networked GUI.");
        }
    }

    private void saveGui() {
        AmpereGuiActions.saveCurrentGui(MC);
    }

    private void loadGui() {
        if (AmpereModule.get().restoreSavedScreenUiBehavior()) {
    ...
    private void disconnectAndSend() {
        ...
        if (MC.getConnection() != null) {
            ...
            MC.getConnection().getConnection().disconnect(Component.literal("Disconnecting (Ampere)"));
        }
    }
    ...
    private void submitChat(String raw) {
        ...
        if (AmpereCommands.isAmpereCommandMessage(message)) {
            ...
            if (!body.isBlank()) AmpereCommands.dispatch(body);
    ...
    private void updateSyncMetrics() {
        ...
        if (MC.player != null && MC.player.containerMenu != null) {
            ...
            if (MC.screen instanceof AbstractContainerScreen<?> handledScreen) {
                Slot focusedSlot = ((AmpereHandledScreenAccessor) handledScreen).getFocusedSlot();
                if (focusedSlot != null) {
                    slotText = Integer.toString(AmpereInventoryHelper.toUserVisibleSlot(MC, focusedSlot.index));
                }
            }
        }
