...
public final class AmpereCompatManager {
    ...
    public static String effectiveCommandPrefix() {
        ampere.modules.AmpereModule module = ampere.modules.AmpereModule.get();
        String stored = module == null ? AmpereConfig.getGlobal().commandPrefix : module.getCommandPrefix();
    ...
    public static List<AmpereMacro> getMeteorMacros() {
        ...
        List<AmpereMacro> macros = new ArrayList<>();
        try {
            ...
            for (Object meteorMacro : iterable) {
                AmpereMacro converted = convertMeteorMacro(meteorMacro);
                if (converted != null) macros.add(converted);
            }
        } catch (Exception e) {
            AmpereClientAddon.LOG.warn("[Ampere] Failed to read Meteor macros reflectively.", e);
        }
        return macros;
    }
    ...
    public static List<String> getMeteorMacroNames() {
        ...
        for (AmpereMacro macro : getMeteorMacros()) {
            if (macro != null && macro.name != null && !macro.name.isBlank()) names.add(macro.name);
        }
        return names;
    }

    public static AmpereMacro getMeteorMacro(String macroName) {
    ...
    public static boolean importToMeteor(AmpereMacro packUtilMacro) {
        return false;
    }

    public static List<String> getAmpereModuleNames() {
        List<String> names = new ArrayList<>(PackModuleRegistry.names());
    ...
    public static List<String> getMeteorOnlyModuleNames() {
        ...
        List<String> nativeNames = getAmpereModuleNames();
        List<String> names = new ArrayList<>();
        try {
            ...
            AmpereClientAddon.LOG.warn("[Ampere] Failed to read Meteor modules reflectively.", e);
        }
        return names;
    }

    public static List<String> getMeteorModuleNames() {
        List<String> names = getAmpereModuleNames();
    ...
    public static boolean toggleMeteorModule(String moduleName, ToggleModuleAction.ToggleMode toggleMode) {
        ...
        if (PackModuleRegistry.toggle(moduleName, toggleMode)) return true;
        ...
        try {
            ...
            AmpereClientAddon.LOG.warn("[Ampere] Failed to toggle Meteor module '{}' reflectively.", moduleName, e);
            return false;
        }
    }
    ...
    public static boolean startBaritoneGoTo(Minecraft mc, int x, int y, int z) {
        ...
        try {
            ...
            AmpereClientAddon.LOG.warn("[Ampere] Failed to start Baritone goto reflectively, falling back to chat command.", t);
    ...
    public static boolean startBaritoneMine(Minecraft mc, List<String> blockNames) {
        ...
        try {
            ...
            AmpereClientAddon.LOG.warn("[Ampere] Failed to start Baritone mine reflectively, falling back to chat command.", t);
    ...
    public static void stopBaritone(Minecraft mc) {
        ...
        try {
            ...
            AmpereClientAddon.LOG.warn("[Ampere] Failed to stop Baritone reflectively, falling back to chat command.", t);
        }
    ...
    private static AmpereMacro convertMeteorMacro(Object meteorMacro) {
        ...
        try {
            ...
            if (name == null || messages == null) {
                try {
                    ...
                    AmpereClientAddon.LOG.warn("[Ampere] Settings iteration fallback failed.", e);
                }
            }
            ...
            AmpereClientAddon.LOG.warn("[Ampere] Error converting Meteor macro.", e);
            return null;
        }
    }
