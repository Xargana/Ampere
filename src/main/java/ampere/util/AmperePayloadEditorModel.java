...
public final class AmperePayloadEditorModel {
    ...
    public List<AmperePayloadTemplate.Field> bodyFields = new ArrayList<>();
    ...
    public static AmperePayloadEditorModel fromAction(PayloadAction action) {
        AmperePayloadEditorModel model = new AmperePayloadEditorModel();
        if (action == null) {
            model.bodyFields = new ArrayList<>(AmperePayloadTemplate.defaultBrandTemplate().fields());
        ...
        if (action.payloadData != null && !action.payloadData.isBlank()) {
            try {
                raw = AmperePayloadSupport.parsePayloadBytes(action.payloadData);
                haveRaw = true;
            } catch (Throwable ignored) {
            }
        }
        List<AmperePayloadTemplate.Field> fields = AmperePayloadTemplate.parseFields(action.payloadFields);
        if (!fields.isEmpty()) {
    ...
    public PayloadAction toAction(PayloadAction base) {
        ...
        action.payloadFields = AmperePayloadTemplate.serializeFields(bodyFields);
        action.payloadData = AmperePayloadSupport.toHex(bodyBytes);
        action.payloadJson = "";
    ...
    public AmperePayloadTemplate.Template toTemplate() {
        return new AmperePayloadTemplate.Template(
            channel,
            AmperePayloadTemplate.PayloadDirection.parse(direction),
            AmperePayloadTemplate.PayloadPhase.parse(phase),
            AmperePayloadTemplate.EncodingMode.parse(inferMode(), channel),
            bodyFields == null ? List.of() : bodyFields
        );
    }

    public boolean applyPacketScript(String script) {
        AmperePayloadEditorModel parsed = copy();
        ...
        parsed.bodyFields = new ArrayList<>(AmperePayloadTemplate.parseFields(String.join("\n", writerLines)));
    ...
    public boolean applyBodyHex(String hex) {
        try {
            ...
            validationError = AmperePayloadSupport.safeMessage(t);
            return false;
        }
    }
    ...
    public String packetScript() {
        ...
        sb.append('\n');
        sb.append(AmperePayloadTemplate.serializeFields(bodyFields));
        return sb.toString();
    }
    ...
    public String utf8View() {
        String text = AmperePayloadSupport.decodeLikelyUtf8Text(bodyBytes);
    ...
    public String logicalText() {
        ...
        sb.append("bodyHex = ").append(AmperePayloadSupport.toHex(bodyBytes)).append('\n');
        if (packetId >= 0) {
            sb.append("logicalHex = ").append(AmperePayloadSupport.toHex(logicalBytes())).append('\n');
        } else {
    ...
    public byte[] logicalBytes() {
        ...
        byte[] channelBytes = AmperePayloadSupport.encodeMinecraftStringPayload(channel == null ? "" : channel);
    ...
    private boolean rebuildFromFields(boolean strict) {
        AmperePayloadTemplate.Template template = toTemplate();
    ...
    private void setBodyBytes(byte[] bytes, Provenance source) {
        ...
        lastValidBytes = bodyBytes.clone();
        bodyFields = new ArrayList<>(AmperePayloadSupport.inferEditablePayloadFields(bodyBytes));
        encodingMode = AmperePayloadSupport.inferEditablePayloadMode(channel, bodyBytes);
        decodedKind = inferDecodedKind(bodyBytes);
    ...
    private void refreshCommandApiFromBytes() {
        Integer parsed = AmperePayloadSupport.tryParseCommandApiValue(null, channel, bodyBytes);
        if (parsed != null) {
    ...
    private String inferMode() {
        ...
        return AmperePayloadSupport.inferEditablePayloadMode(channel, bodyBytes);
    }

    private static String inferDecodedKind(byte[] bytes) {
        if (AmperePayloadSupport.decodeMinecraftStringPayload(bytes) != null) return "Minecraft String";
        if (AmperePayloadSupport.decodeJavaWriteUtfPayload(bytes) != null) return "Java writeUTF";
        String utf8 = AmperePayloadSupport.decodeLikelyUtf8Text(bytes);
        if (utf8 != null && !utf8.isBlank()) return "Raw UTF-8";
        return "Binary Hex";
    }

    private AmperePayloadEditorModel copy() {
        AmperePayloadEditorModel copy = new AmperePayloadEditorModel();
        copy.copyFrom(this);
        return copy;
    }

    private void copyFrom(AmperePayloadEditorModel other) {
