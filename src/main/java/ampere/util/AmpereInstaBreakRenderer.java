...
import net.minecraft.client.renderer.rendertype.AmpereRenderTypes;
...
public final class AmpereInstaBreakRenderer {
    ...
    private AmpereInstaBreakRenderer() {
    }
    ...
    public static void initialize() {
        LevelRenderEvents.COLLECT_SUBMITS.register(context -> {
            ...
            if (drawSides(finalShape) && ((finalSideColor >>> 24) & 0xFF) > 0) {
                context.submitNodeCollector().submitCustomGeometry(poseStack, AmpereRenderTypes.storageEspFillSeeThrough(), (pose, buffer) -> fillBox(pose, buffer, box, finalSideColor));
            }
            if (drawLines(finalShape)) {
                context.submitNodeCollector().submitCustomGeometry(poseStack, AmpereRenderTypes.storageEspLinesSeeThrough(), (pose, buffer) -> renderBox(pose, buffer, box, finalColor));
            }
        });
    }
