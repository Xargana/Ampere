...
public interface IAmpereOverlay {
    enum OverlayScope {
        HOST_SCREEN,
        MODULE_MENU,
        CONTAINER_GUI,
    ...
    default AmpereWindowLayout getBounds() {
        return new AmpereWindowLayout(0, 0, getMinWidth(), getMinHeight(), isVisible(), isCollapsed());
    }

    default void setBounds(AmpereWindowLayout bounds) {
    }
    ...
    default void restoreLayout() {
        AmpereWindowLayout layout = AmpereSharedState.get().getWindowLayout(getOverlayId());
        if (layout != null) {
            setBounds(layout);
        }
    }
