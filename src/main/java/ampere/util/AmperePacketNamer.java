...
public class AmperePacketNamer {
    ...
    public static String getFriendlyName(Packet<?> packet, String direction) {
        ...
        if (name != null) return cache(cacheKey, stripSuffix(name));

        String typeName = resolveNameFromPacketType(packet, direction);
        if (typeName != null) return cache(cacheKey, stripSuffix(typeName));

        String inspected = inspectObfuscatedPacket(packet, packetClass.getSimpleName());
        if (inspected != null) return cache(cacheKey, inspected);

        String readable = deriveReadableClassName(packetClass);
        return cache(cacheKey, readable != null ? readable : UNKNOWN_PACKET);
    }
    ...
    private static String resolveRegistryName(Class<? extends Packet<?>> packetClass) {
        ...
        String direct = AmperePacketRegistry.getName(packetClass);
        if (direct != null) return direct;

        for (Class<? extends Packet<?>> registeredClass : AmperePacketRegistry.PACKETS) {
            if (registeredClass == packetClass || registeredClass.isAssignableFrom(packetClass)) {
                String name = AmperePacketRegistry.getName(registeredClass);
    ...
    private static String resolveNameFromText(String text) {
        ...
        for (Class<? extends Packet<?>> registeredClass : AmperePacketRegistry.PACKETS) {
            String registeredName = AmperePacketRegistry.getName(registeredClass);
    ...
    private static String resolveDirectionalRegistryName(Class<? extends Packet<?>> packetClass, String direction) {
        for (Class<? extends Packet<?>> registeredClass : getDirectionalRegistry(direction)) {
            if (registeredClass == packetClass) {
                return AmperePacketRegistry.getName(registeredClass);
            }
        }
        return null;
    }
    ...
    private static Iterable<Class<? extends Packet<?>>> getDirectionalRegistry(String direction) {
        if ("C2S".equalsIgnoreCase(direction)) return AmperePacketRegistry.getC2SPackets();
        if ("S2C".equalsIgnoreCase(direction)) return AmperePacketRegistry.getS2CPackets();
        return AmperePacketRegistry.PACKETS;
    }
    ...
    private static String normalizePacketKey(String value) {
        ...
        return NORMALIZED_CACHE.computeIfAbsent(value, AmperePacketNamer::normalizePacketKeyUncached);
    }
