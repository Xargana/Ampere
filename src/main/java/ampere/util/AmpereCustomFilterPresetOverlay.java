...
public class AmpereCustomFilterPresetOverlay extends AmpereOverlayBase {
    ...
    private List<AmperePresetManager.PresetEntry> cachedBuiltInEntries = List.of();
    private List<AmperePresetManager.PresetEntry> cachedUserEntries = List.of();
    ...
    public AmpereCustomFilterPresetOverlay(Font textRenderer) {
    ...
    public String getOverlayId() {
        return "Ampere-custom-filter-presets";
    }
    ...
    public void render(GuiGraphicsExtractor context, int mouseX, int mouseY, float delta) {
        ...
        buttons.clear();
        AmpereWindowLayout clamped = clampToScreen(this, new AmpereWindowLayout(panelX, panelY, panelWidth, panelHeight, visible, collapsed));
        panelX = clamped.x;
        panelY = clamped.y;
        panelWidth = clamped.width;
        panelHeight = clamped.height;
        AmpereWindowLayout bounds = new AmpereWindowLayout(panelX, panelY, panelWidth, panelHeight, visible, collapsed);
        ...
        try {
            ...
            AmpereText.draw(context, textRenderer,
                "C2S " + module.getC2SPackets().size() + " | S2C " + module.getS2CPackets().size(),
                AmpereText.Tone.MUTED, x, y, false);
            y += infoLineHeight();
    ...
    private int renderPresetSection(GuiGraphicsExtractor context, int mouseX, int mouseY, float delta, int x, int y, int width, int sectionHeight) {
                ...
                listLayout.forEachVisibleRow(rows.size(), (index, rowY) -> {
                    ...
                    if (row.header) {
                        ...
                        int textColor = selected
                            ? AmpereColors.rowSelectedText()
                            : (row.entry.builtIn() ? 0xFFB7D4FF : AmpereColors.textLight());
                        String prefix = row.entry.builtIn() ? "[DEV] " : "[USR] ";
                        String name = AmpereText.trimToWidth(textRenderer, prefix + row.entry.name(), contentWidth - 70, AmpereText.Tone.BODY);
                        AmpereText.draw(context, textRenderer, name, textColor, x + 5, rowTextY, false);

                        String counts = row.entry.c2sCount() + " / " + row.entry.s2cCount();
                        int countsWidth = AmpereText.width(textRenderer, counts, AmpereText.Tone.BODY);
                        AmpereText.draw(context, textRenderer, counts, textColor, x + contentWidth - countsWidth - 1, rowTextY, false);
                    }
        ...
        AmpereText.draw(context, textRenderer, footer, AmpereText.Tone.MUTED, x + 4, y + listHeight + footerGap(), false);
        return y + listHeight + footerHeight() + footerGap();
    }

    private List<PresetRow> getCachedPresetRows() {
        AmperePresetManager manager = AmperePresetManager.get();
        List<AmperePresetManager.PresetEntry> builtInEntries = manager.getBuiltInPresetEntries();
        List<AmperePresetManager.PresetEntry> userEntries = manager.getUserPresetEntries();
        ...
        for (AmperePresetManager.PresetEntry entry : builtInEntries) {
        ...
        if (userEntries.isEmpty()) {
            ...
            for (AmperePresetManager.PresetEntry entry : userEntries) {
                rows.add(PresetRow.entry(entry));
            }
        }
    ...
    private void saveNamedPreset() {
        ...
        if (name == null) {
            AmpereMessaging.sendPrefixed("Enter a preset name first.");
            return;
        }
        if (AmperePresetManager.get().savePreset(name)) {
    ...
    private void loadSelectedPreset() {
        ...
        if (presetName == null) {
            AmpereMessaging.sendPrefixed("Select a preset first.");
            return;
        }
        if (AmperePresetManager.get().loadPreset(presetName)) {
            selectedPresetName = AmperePresetManager.get().getPresetEntry(presetName) != null
                ? AmperePresetManager.get().getPresetEntry(presetName).name()
                : presetName;
    ...
    private void overwriteSelectedPreset() {
        String presetName = resolveTargetPresetName();
        if (presetName == null) {
            AmpereMessaging.sendPrefixed("Select a user preset first.");
            return;
        }
        AmperePresetManager.PresetEntry entry = AmperePresetManager.get().getPresetEntry(presetName);
        if (entry == null || entry.builtIn()) {
            AmpereMessaging.sendPrefixed("Select a user preset to overwrite.");
            return;
        }
        if (AmperePresetManager.get().overwriteUserPreset(entry.name())) {
    ...
    private void deleteSelectedPreset() {
        ...
        AmperePresetManager.PresetEntry entry = AmperePresetManager.get().getPresetEntry(presetName);
        if (entry == null || entry.builtIn()) {
            AmpereMessaging.sendPrefixed("Developer presets cannot be deleted.");
            return;
        }
        if (AmperePresetManager.get().deleteUserPreset(entry.name())) {
    ...
    private void resetDefaults() {
        ...
        AmpereMessaging.sendPrefixed("Reset current filter to default packet lists.");
    }
    ...
    private String resolveTargetPresetName() {
        if (selectedPresetName != null) {
            AmperePresetManager.PresetEntry selectedEntry = AmperePresetManager.get().getPresetEntry(selectedPresetName);
            if (selectedEntry != null) return selectedEntry.name();
        }

        String typedName = sanitizeNameField();
        if (typedName == null) return null;
        AmperePresetManager.PresetEntry typedEntry = AmperePresetManager.get().getPresetEntry(typedName);
        return typedEntry != null ? typedEntry.name() : null;
    }

    private AmperePresetManager.PresetEntry getSelectedPresetEntry() {
        if (selectedPresetName == null) return null;
        return AmperePresetManager.get().getPresetEntry(selectedPresetName);
    }

    private boolean canSaveNamed() {
        String name = sanitizeNameField();
        return name != null && !AmperePresetManager.get().isReservedPresetName(name);
    }
    ...
    private boolean canOverwriteSelected() {
        AmperePresetManager.PresetEntry entry = getSelectedPresetEntry();
    ...
    private final class PresetListState {
        ...
        private PresetRow getRowAt(double mouseY) {
            int idx = (int) (((mouseY - y - 2) + scroll.visualOffsetInt()) / AmpereCustomFilterPresetOverlay.this.presetRowStep());
    ...
    private static final class PresetRow {
        ...
        private final AmperePresetManager.PresetEntry entry;

        private PresetRow(boolean header, boolean note, String label, AmperePresetManager.PresetEntry entry) {
            this.header = header;
            this.note = note;
            this.label = label;
            this.entry = entry;
        ...
        private static PresetRow entry(AmperePresetManager.PresetEntry entry) {
