...
import ampere.gui.vanillaui.direct.DirectHudPanelRenderer;
...
public final class AmpereQueueRenderer {
    ...
    private static void ensureCache(Font textRenderer, int panelWidth, int maxLines) {
        ...
        String modeStr = shared.getQueueDisplayDelayMode() == AmpereSharedState.DelayMode.MS ? "ms" : "t";
    ...
    private static void rebuildCache(Font textRenderer, AmpereSharedState.QueueRenderSnapshot snapshot, boolean isSending, String modeStr, int maxLines, int panelWidth, int queueRevision, long now) {
        List<AmpereSharedState.QueuedPacket> packets = snapshot.packets();
        ...
        if (packets.isEmpty()) {
            ...
            for (int i = 0; i < visibleCount; i++) {
                ...
                String simpleName = AmperePacketNamer.getFriendlyName(qp.packet);
