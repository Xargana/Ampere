...
public class AmperePresetManager {
    private static final File PRESETS_FOLDER = new File(AmpereClientAddon.FOLDER, "presets");
    private static final AmperePresetManager INSTANCE = new AmperePresetManager();
    ...
    public record PresetEntry(AmperePacketPreset preset, boolean builtIn) {
        public String name() {
    ...
    private AmperePresetManager() {
        if (!PRESETS_FOLDER.exists()) {
            PRESETS_FOLDER.mkdirs();
        }
    }

    public static AmperePresetManager get() {
        return INSTANCE;
    }
    ...
    public boolean savePreset(String name) {
        ...
        if (sanitized == null) {
            AmpereMessaging.sendPrefixed("§ePreset name cannot be empty.");
            return false;
        }
        if (isReservedPresetName(sanitized)) {
            AmpereMessaging.sendPrefixed("§cThat preset name is reserved for developer templates.");
            return false;
        }
    ...
    public String savePacketCancellerPreset(Set<String> c2sPackets, Set<String> s2cPackets) {
        String name = nextAvailableUserPresetName();
        savePresetObject(new AmperePacketPreset(name,
    ...
    public boolean overwriteUserPreset(String name) {
        String sanitized = sanitizePresetName(name);
        if (sanitized == null) {
            AmpereMessaging.sendPrefixed("§eSelect a user preset first.");
            return false;
        }
        if (isReservedPresetName(sanitized)) {
            AmpereMessaging.sendPrefixed("§cDeveloper presets cannot be overwritten.");
            return false;
        }

        File file = new File(PRESETS_FOLDER, sanitized + ".json");
        if (!file.exists()) {
            AmpereMessaging.sendPrefixed("§cUser preset not found: " + sanitized);
            return false;
        }
    ...
    public boolean loadPreset(String name) {
        ...
        if (entry == null) {
            AmpereMessaging.sendPrefixed("§cPreset not found: " + name);
            return false;
        }

        applyPreset(entry.preset());
        AmpereMessaging.sendPrefixed("§aLoaded preset: " + entry.name());
        return true;
    }
    ...
    public boolean deleteUserPreset(String name) {
        ...
        if (isReservedPresetName(sanitized)) {
            AmpereMessaging.sendPrefixed("§cDeveloper presets cannot be deleted.");
            return false;
        }
        ...
        if (!file.delete()) {
            AmpereMessaging.sendPrefixed("§cFailed to delete preset: " + sanitized);
            return false;
        }

        invalidateUserPresetCache();
        AmpereMessaging.sendPrefixed("§aDeleted preset: " + sanitized);
        return true;
    }
    ...
    public boolean importSharedPreset(String presetName, String presetJson, String sender) {
        ...
        try {
            AmperePacketPreset preset = gson.fromJson(presetJson, AmperePacketPreset.class);
            if (preset == null) return false;
            preset.name = createUniquePresetName(sanitized);
            savePresetObject(preset, false);
            invalidateUserPresetCache();
            AmpereMessaging.sendPrefixed("§aReceived preset from " + sender + ": " + preset.name);
            return true;
        } catch (Exception e) {
            AmpereClientAddon.LOG.error("Failed to import shared preset", e);
            AmpereMessaging.sendPrefixed("§cFailed to import preset: " + e.getMessage());
            return false;
        }
    }
    ...
    public List<PresetEntry> getUserPresetEntries() {
        ...
        for (File file : files) {
            try (FileReader reader = new FileReader(file)) {
                AmperePacketPreset preset = gson.fromJson(reader, AmperePacketPreset.class);
                if (preset == null) continue;
                if (preset.name == null || preset.name.isBlank()) {
                    preset.name = file.getName().replace(".json", "");
                }
                if (isReservedPresetName(preset.name)) {
                    AmpereClientAddon.LOG.warn("Ignoring user preset with reserved built-in name: {}", preset.name);
                    continue;
                }
                ...
                AmpereClientAddon.LOG.error("Failed to read preset file {}", file.getName(), e);
            }
        }
    ...
    private AmperePacketPreset captureCurrentPreset(String name) {
        ...
        return new AmperePacketPreset(name, c2s, s2c);
    }

    private AmperePacketPreset presetFromClasses(String name,
                                                   Collection<Class<? extends Packet<?>>> c2sPackets,
                                                   Collection<Class<? extends Packet<?>>> s2cPackets) {
        return new AmperePacketPreset(name, encodePacketNames(c2sPackets), encodePacketNames(s2cPackets));
    }

    private AmperePacketPreset presetFromPacketNames(String name, Set<String> c2sNames, Set<String> s2cNames) {
        return new AmperePacketPreset(
            name,
    ...
    private Set<Class<? extends Packet<?>>> resolvePacketNames(Collection<String> names, boolean c2s) {
        ...
        for (String packetName : names) {
            ...
            Class<? extends Packet<?>> packetClass = AmperePacketRegistry.getPacket(resolvedName);
            if (packetClass == null) {
                packetClass = AmperePacketRegistry.getPacket(AmperePacketNamer.getFriendlyName(resolvedName));
            }
            if (packetClass == null) {
                try {
            ...
            if (packetClass != null && pool.contains(packetClass)) {
                ...
                AmpereClientAddon.LOG.warn("Unknown packet name in preset: {}", packetName);
            }
        }
        return resolved;
    }

    private void savePresetObject(AmperePacketPreset preset, boolean notify) {
        File file = new File(PRESETS_FOLDER, preset.name + ".json");
        try (FileWriter writer = new FileWriter(file)) {
            gson.toJson(preset, writer);
            if (notify) AmpereMessaging.sendPrefixed("§aSaved preset: " + preset.name);
        } catch (IOException e) {
            AmpereClientAddon.LOG.error("Failed to save preset", e);
            AmpereMessaging.sendPrefixed("§cFailed to save preset: " + e.getMessage());
        }
    }

    private void applyPreset(AmperePacketPreset preset) {
        AmpereModule module = AmpereModule.get();
