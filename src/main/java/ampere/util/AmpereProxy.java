...
public class AmpereProxy {
    ...
    public String name = "";
    public AmpereProxyType type = AmpereProxyType.Socks5;
    ...
    public synchronized int checkStatus(int timeoutMs) {
        ...
        int primary = checkType(type == AmpereProxyType.Socks4 ? AmpereProxyType.Socks4 : AmpereProxyType.Socks5, timeoutMs);
        if (primary == 1) return 1;
        if (primary == 3) timeout = true;
        int fallback = checkType(type == AmpereProxyType.Socks4 ? AmpereProxyType.Socks5 : AmpereProxyType.Socks4, timeoutMs);
    ...
    private int checkType(AmpereProxyType checkType, int timeoutMs) {
        try {
            Instant before = Instant.now();
            boolean alive = checkType == AmpereProxyType.Socks4 ? isSocks4(timeoutMs) : isSocks5(timeoutMs);
            if (alive) {
                status = Status.ALIVE;
    ...
    public CompoundTag toTag() {
        ...
        tag.putString("type", type == null ? AmpereProxyType.Socks5.name() : type.name());
    ...
    public AmpereProxy fromTag(CompoundTag tag) {
        name = tag.getStringOr("name", "");
        String typeName = tag.getStringOr("type", AmpereProxyType.Socks5.name());
        try {
            type = AmpereProxyType.valueOf(typeName);
        } catch (IllegalArgumentException e) {
            type = typeName.toLowerCase(Locale.ROOT).contains("4") ? AmpereProxyType.Socks4 : AmpereProxyType.Socks5;
        }
    ...
    public boolean equals(Object obj) {
        ...
        if (!(obj instanceof AmpereProxy proxy)) return false;
