...
public class AmpereContextMenu<T> {
    ...
    public AmpereContextMenu(CompactTheme theme, Font font, java.util.function.Function<T, String[]> itemProvider, int lineHeight) {
        this.theme = theme;
        this.font = font;
    ...
    private void ensureMetrics(String[] items) {
        int screenWidth = Math.max(1, AmpereUiScale.getVirtualScreenWidth());
        int screenHeight = Math.max(1, AmpereUiScale.getVirtualScreenHeight());
    ...
    private int visibleItemCount(String[] items) {
        int available = Math.max(1, AmpereUiScale.getVirtualScreenHeight() - 4 - (PADDING_Y * 2));
    ...
    private void clampToScreen() {
        ...
        int w = width(items);
        int h = height(items);
        int sw = Math.max(1, AmpereUiScale.getVirtualScreenWidth());
        int sh = Math.max(1, AmpereUiScale.getVirtualScreenHeight());
    ...
    public void render(GuiGraphicsExtractor ctx, int mx, int my) {
        ...
        for (int i = 0; i < visibleItems; i++) {
            ...
            if (hov) CompactSurfaces.tintedRow(ctx, x + 1, iy, w - 2, lineHeight, AmpereColors.popupHover());
            String display = UiText.trimToWidth(font, items[i], Math.max(1, w - PADDING_X * 2), theme.fontFor(UiTone.BODY), AmpereColors.textLight());
            UiText.draw(ctx, font, display, theme.fontFor(UiTone.BODY),
                hov ? 0xFFFFFFFF : AmpereColors.textLight(),
