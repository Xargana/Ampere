...
import ampere.util.AmpereSharedState.QueuedPacket;
...
public class AmpereQueueEditorOverlay extends AmpereOverlayBase {
    ...
    public AmpereQueueEditorOverlay(Font textRenderer) {
    ...
    public boolean isCaptureMode() {
        return AmpereSharedState.get().isCaptureMode();
    }
    ...
    public boolean isMouseOver(double mouseX, double mouseY) {
        ...
        int panelHeight = collapsed
            ? HEADER_HEIGHT
            : clampToScreen(this, new AmpereWindowLayout(panelX, panelY, PANEL_WIDTH, getPanelHeight(displayQueue), visible, collapsed)).height;
    ...
    private int getContentStartOffset() {
        int toolbarRows = AmpereLANSync.getInstance().isInSession() ? 4 : 3;
    ...
    private void maybeAutoReorderDelayedQueue(AmpereSharedState shared) {
    ...
    public void render(GuiGraphicsExtractor context, int mouseX, int mouseY, float delta) {
        ...
        AmpereWindowLayout clamped = clampToScreen(
            this,
            new AmpereWindowLayout(panelX, panelY, PANEL_WIDTH, getPanelHeight(cachedDisplayQueue), visible, collapsed)
        );
        panelX = clamped.x;
        panelY = clamped.y;
        PANEL_WIDTH = clamped.width;
        ...
        bx = drawAutoToolbarButton(context, bx, row1Y, toolbarButtonHeight(), "Copy", mouseX, mouseY,
            ...
            () -> {
                AmpereClipboardHelper.copyToClipboard(getCurrentQueue());
                AmpereNotifications.copied("Queue copied.");
            }) + gap;
        bx = drawAutoToolbarButton(context, bx, row1Y, toolbarButtonHeight(), "Paste", mouseX, mouseY,
            "Replace queue from clipboard",
            () -> {
                if (AmpereSharedState.get().getStaggeredQueue().isEmpty()) {
                    AmpereSharedState.QueuedPacket.resetIdCounter();
                }
                List<QueuedPacket> newQueue = AmpereClipboardHelper.pasteFromClipboard();
                if (newQueue != null) {
                    AmpereSharedState.get().setDelayedPackets(newQueue);
                    clearSelection();
                    scrollOffset = 0;
                    AmpereMessaging.sendPrefixed("§aQueue pasted!");
                }
            }) + gap;
        bx = drawAutoToolbarButton(context, bx, row1Y, toolbarButtonHeight(), "Dupe", mouseX, mouseY,
            "Duplicate all packets in the queue",
            () -> {
                List<QueuedPacket> currentQueue = AmpereSharedState.get().getDelayedPackets();
                if (!currentQueue.isEmpty()) {
                    ...
                    AmpereSharedState.get().setDelayedPackets(duplicatedQueue);
                    clearSelection();
                    AmpereMessaging.sendPrefixed("§aDuplicated " + currentQueue.size() + " packet(s).");
                }
            }) + gap;
        bx = drawAutoToolbarButton(context, bx, row1Y, toolbarButtonHeight(), "Ins", mouseX, mouseY,
            ...
            () -> {
                List<QueuedPacket> newPackets = AmpereClipboardHelper.pasteFromClipboard();
                if (newPackets != null && !newPackets.isEmpty()) {
                    List<QueuedPacket> combinedQueue = new ArrayList<>(AmpereSharedState.get().getDelayedPackets());
                    combinedQueue.addAll(newPackets);
                    AmpereSharedState.get().setDelayedPackets(combinedQueue);
                    clearSelection();
                    AmpereMessaging.sendPrefixed("§aInserted " + newPackets.size() + " packet(s).");
                }
            }) + gap;
        ...
        drawFixedToolbarButton(context, bx, row1Y, replayWidth, toolbarButtonHeight(), "Replay", CompactOverlayButton.Variant.GHOST, true, mouseX, mouseY,
            ...
            () -> {
                if (AmpereSharedState.get().restoreLastFlushedQueue()) {
                    clearSelection();
                    scrollOffset = 0;
                    AmpereMessaging.sendPrefixed("§aRestored - packets will regenerate on send");
                } else {
                    AmpereMessaging.sendPrefixed("§cNo history");
                }
            });

        boolean flushOnDelay = AmpereModule.get().shouldFlushQueueOnDelayDisable();
        boolean captureAsExact = AmpereModule.get().isCaptureAsExact();
        ...
        drawFixedToolbarStateButton(context, leftX, row2Y, flushWidth, toolbarButtonHeight(), "FLUSH ON DELAY", flushOnDelay, mouseX, mouseY,
            ...
            () -> {
                ...
                AmpereMessaging.sendPrefixed(newValue
                    ? "\u00a7aFlush on Delay ON"
                    : "\u00a7eFlush on Delay OFF");
            });
        drawFixedToolbarButton(context, leftX + flushWidth + gap, row2Y, captureModeBtnWidth, toolbarButtonHeight(),
            ...
            () -> {
                ...
                AmpereSharedState.ReplayMode bulkMode = nowExact
                    ? AmpereSharedState.ReplayMode.EXACT
                    : AmpereSharedState.ReplayMode.REGENERATE;
                int updated = 0;
                for (AmpereSharedState.QueuedPacket qp : AmpereSharedState.get().getDelayedPackets()) {
                    if (qp.getReplayMode() != bulkMode) {
                        qp.setReplayMode(bulkMode);
                        updated++;
                    }
                }
                AmpereMessaging.sendPrefixed(nowExact
        ...
        drawFixedToolbarButton(context, leftX + flushWidth + gap + captureModeBtnWidth + gap, row2Y, clearWidth, toolbarButtonHeight(), "Clear", CompactOverlayButton.Variant.DANGER, true, mouseX, mouseY,
            ...
            () -> {
                int cleared = AmpereSharedState.get().clearQueuedPackets();
                clearSelection();
                scrollOffset = 0;
                AmpereModule.get().notifyClearQueuedPacketsUiResult(cleared);
            });

        boolean captureModeOn = AmpereSharedState.get().isCaptureMode();
        String delayModeLabel = AmpereSharedState.get().getDelayMode() == AmpereSharedState.DelayMode.TICKS ? "Ticks" : "Ms";
        ...
        drawFixedToolbarButton(context, leftX, row3Y, modeWidth, toolbarButtonHeight(), delayModeLabel, CompactOverlayButton.Variant.GHOST, true, mouseX, mouseY,
            ...
            () -> {
                AmpereSharedState.get().toggleDelayMode();
                String newMode = AmpereSharedState.get().getDelayMode() == AmpereSharedState.DelayMode.TICKS ? "Ticks" : "Ms";
                AmpereMessaging.sendPrefixed("\u00a7aDelay mode: " + newMode);
            });
        drawFixedToolbarStateButton(context, leftX + modeWidth + gap, row3Y, captureWidth, toolbarButtonHeight(), "Capture", captureModeOn, mouseX, mouseY,
            ...
            () -> {
                boolean newCapture = !AmpereSharedState.get().isCaptureMode();
                AmpereSharedState.get().setCaptureMode(newCapture);
                AmpereMessaging.sendPrefixed(newCapture ? "\u00a7aCapture Real Delays ON" : "\u00a7eCapture Real Delays OFF");
            });
        ...
        drawFixedToolbarButton(context, rowRight - sendWidth, row3Y, sendWidth, toolbarButtonHeight(), "Send Q", CompactOverlayButton.Variant.SUCCESS, true, mouseX, mouseY,
            ...
            () -> {
                if (MC.getConnection() != null) {
                    int count = AmpereSharedState.get().flushDelayedPackets(MC.getConnection());
                    AmpereModule.get().notifyFlushQueuedPacketsUiResult(count);
                }
            });

        boolean inLanSession = AmpereLANSync.getInstance().isInSession();
        if (inLanSession) {
            ...
            drawFixedToolbarButton(context, leftX + syncWidth + gap, row4Y, syncExecWidth, toolbarButtonHeight(), "Sync Exec", CompactOverlayButton.Variant.SUCCESS, true, mouseX, mouseY,
                ...
                () -> AmpereLANSync.getInstance().sendQueuedPackets());
        }

        if (cachedDisplayQueue.isEmpty()) {
            AmpereText.draw(context, textRenderer, "Queue empty", AmpereText.Tone.MUTED, panelX + 10, getContentStartY(), false);
        } else {
            int visibleHeight = getlistViewportHeight(panelHeight);
            ...
            ampere.gui.vanillaui.UiScissorStack.global().push(context,
                ampere.gui.vanillaui.UiBounds.of(listX + 1, clipTop, Math.max(0, listWidth - 2), Math.max(0, clipBottom - clipTop)));
            ...
            for (int i = 0; i < cachedDisplayQueue.size(); i++) {
                ...
                int textColor = AmpereColors.textPrimary();
                if (grouped) {
                ...
                String label = "#" + packet.getId() + " " + AmperePacketNamer.getFriendlyName(packet.packet);
                ...
                UiText.draw(context, textRenderer, trimmed, theme.fontFor(UiTone.BODY), selected ? AmpereColors.rowSelectedText() : textColor, rowX + 7, rowTextY, false);
    ...
    public boolean mouseClicked(double mouseX, double mouseY, int button) {
        ...
        for (RowRegion region : rowRegions) {
            ...
            if (region.overMode(mouseX, mouseY) && button == 0) {
                AmpereSharedState.ReplayMode next = region.packet.isExactReplay()
                    ? AmpereSharedState.ReplayMode.REGENERATE
                    : AmpereSharedState.ReplayMode.EXACT;
    ...
    public boolean mouseScrolled(double mouseX, double mouseY, double amount) {
        ...
        List<QueuedPacket> displayQueue = cachedDisplayQueue.isEmpty() ? getCurrentQueue() : cachedDisplayQueue;
        int panelHeight = clampToScreen(this, new AmpereWindowLayout(panelX, panelY, PANEL_WIDTH, getPanelHeight(displayQueue), visible, collapsed)).height;
        int visibleHeight = getlistViewportHeight(panelHeight);
    ...
    private void handleSyncButton() {
        ...
        if (!sync.isInSession()) {
            AmpereMessaging.sendPrefixed("\u00a7cNot in LAN session");
            return;
        }

        List<QueuedPacket> queue = getCurrentQueue();
        if (queue.isEmpty()) {
            AmpereMessaging.sendPrefixed("\u00a7cQueue is empty");
            return;
        }

        String queueData = AmpereClipboardHelper.serializeQueueToBase64(queue);
        if (queueData == null) {
            AmpereMessaging.sendPrefixed("\u00a7cFailed to serialize queue");
            return;
        }

        sync.broadcastQueueSync(queueData);
        AmpereMessaging.sendPrefixed("\u00a7aQueue sent to " + sync.getConnectedClientCount() + " clients");
    }
    ...
    private void handleIncrementalDelay(int increment) {
        ...
        if (selectedPacketId != -1) {
            for (QueuedPacket packet : queue) {
                if (packet.getId() == selectedPacketId) {
                    ...
                    String modeStr = AmpereSharedState.get().getQueueDisplayDelayMode() == AmpereSharedState.DelayMode.TICKS ? "ticks" : "ms";
                    AmpereMessaging.sendPrefixed("\u00a7aPacket #" + packet.getId() + " delay: " + packet.getDelay() + " " + modeStr);
                    return;
                }
            }
            clearSelection();
            AmpereMessaging.sendPrefixed("\u00a7cSelected packet not found");
            return;
        }
        ...
        AmpereMessaging.sendPrefixed("\u00a7aApplied +" + increment + " incremental delays");
    }
