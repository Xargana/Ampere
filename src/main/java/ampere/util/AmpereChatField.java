...
public class AmpereChatField {
    ...
    public AmpereChatField(Minecraft mc, Font textRenderer, int x, int y, int width, int height, boolean submitOnEnter) {
        this.mc = mc;
    ...
    public void setPlaceholder(Component placeholder) {
        placeholderText = placeholder == null ? "" : AmpereText.sanitizeUiLabel(placeholder.getString());
    ...
    private boolean handleSubmit() {
        ...
        if (AmpereCommands.isBlockedPanicCommandMessage(value)) {
            field.setText("");
            field.setFocused(false);
            return true;
        }
    ...
    private boolean sendChat(String message) {
        if (AmpereCommands.isAmpereCommandMessage(message)) {
            if (AmpereCommands.isBlockedPanicCommandMessage(message)) return true;
            String body = AmpereCommands.commandBody(message);
            if (!body.isBlank()) AmpereCommands.dispatch(body);
            return true;
        }
        if (mc.getConnection() == null) {
            AmpereClientAddon.LOG.warn("Failed to send Ampere chat message because network handler was null.");
            return false;
        }
