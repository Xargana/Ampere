...
public final class AmperePayloadTemplate {
    ...
    public enum EncodingMode {
        ...
        public static EncodingMode parse(String value, String channel) {
            ...
            if (AmperePayloadSupport.isBrandChannel(channel)) return MINECRAFT_BYTEBUF;
    ...
    public enum FieldType {
        ...
        MINECRAFT_STRING("writeMcString", AmperePayloadSupport.defaultBrandPayloadString()),
        ...
        public BuildResult build() {
            ...
            for (Field field : enabledFields) {
                try {
                    ...
                    errors.add(field.type().label() + ": " + AmperePayloadSupport.safeMessage(e));
                }
            }
    ...
    public record BuildResult(byte[] bytes, List<String> warnings, List<String> errors, Preview preview) {
        public BuildResult {
            ...
            preview = preview == null ? AmperePayloadTemplate.preview(bytes) : preview;
        }
    ...
    public static Template fromAction(PayloadAction action) {
        ...
        if (fields.isEmpty()) {
            if (AmperePayloadSupport.isBrandChannel(channel)) {
                fields = List.of(new Field(FieldType.MINECRAFT_STRING, AmperePayloadSupport.defaultBrandPayloadString(), true));
    ...
    public static Template defaultBrandTemplate() {
        return new Template("minecraft:brand", PayloadDirection.C2S, PayloadPhase.PLAY, EncodingMode.MINECRAFT_BYTEBUF,
            List.of(new Field(FieldType.MINECRAFT_STRING, AmperePayloadSupport.defaultBrandPayloadString(), true)));
    }
    ...
    public static Template presetBrand(String brand) {
        return new Template("minecraft:brand", PayloadDirection.C2S, PayloadPhase.PLAY, EncodingMode.MINECRAFT_BYTEBUF,
            List.of(new Field(FieldType.MINECRAFT_STRING,
                brand == null || brand.isBlank() ? AmperePayloadSupport.defaultBrandPayloadString() : brand, true)));
    }
    ...
    public static Preview preview(byte[] bytes) {
        ...
        return new Preview(
            AmperePayloadSupport.toCompactHex(raw, 96),
            previewUtf8(raw),
    ...
    public static void applyTemplate(PayloadAction action, Template template) {
        ...
        if (result.ok()) {
            action.payloadData = AmperePayloadSupport.toHex(result.bytes());
        }
    }
    ...
    private static void validateHeader(String channel, PayloadDirection direction, PayloadPhase phase, EncodingMode mode,
        ...
        try {
            ...
            errors.add(AmperePayloadSupport.safeMessage(e));
        }
        if (channel != null && !channel.contains(":")) {
            warnings.add("Use a namespaced channel like namespace:path.");
        }
        if (AmperePayloadSupport.isBrandChannel(channel) && mode != EncodingMode.MINECRAFT_BYTEBUF) {
    ...
    private static void writeField(ByteArrayOutputStream out, Field field, EncodingMode mode, List<String> warnings) throws Exception {
        ...
        switch (field.type()) {
            ...
            case MINECRAFT_STRING, IDENTIFIER -> out.write(AmperePayloadSupport.encodeMinecraftStringPayload(decodeEscapedText(value)));
    ...
    private static void writeBuffer(ByteArrayOutputStream out, FriendlyByteBuf buf) {
        byte[] bytes = AmperePayloadSupport.toByteArray(buf);
    ...
    private static String previewMinecraftString(byte[] bytes) {
        String decoded = AmperePayloadSupport.decodeMinecraftStringPayload(bytes);
