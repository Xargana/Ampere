...
public final class AmpereWindowBranding {
    public static final String WINDOW_TITLE = "Ampere Client";
    private static final String ICON_ROOT = "assets/Ampere/icons/window/";
    ...
    private static void applyPending(Minecraft client) {
        ...
        boolean hidden = ampere.modules.PackHideState.isActive();
        boolean ok;
        if (hidden) {
    ...
    private static InputStream openIconStream(String fileName) throws IOException {
        InputStream stream = AmpereWindowBranding.class.getClassLoader().getResourceAsStream(ICON_ROOT + fileName);
        if (stream == null) {
            throw new IOException("Missing Ampere window icon resource: " + fileName);
        }
        return stream;
    }
}
