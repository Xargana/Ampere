...
public final class AmpereText {
    ...
    public static int colorFor(Tone tone) {
        return switch (tone) {
            case TITLE, LABEL, BODY -> AmpereColors.textPrimary();
            case MUTED -> AmpereColors.textMuted();
            case ACCENT -> AmpereColors.accent();
        };
    }
    ...
    public static int width(Font renderer, String value, Tone tone) {
        return ampere.gui.vanillaui.components.UiText.width(renderer, value, fontIdFor(tone), colorFor(tone));
    }

    public static String trimToWidth(Font renderer, String value, int maxWidth, Tone tone) {
        return ampere.gui.vanillaui.components.UiText.trimToWidth(renderer, value, maxWidth, fontIdFor(tone), colorFor(tone));
    }

    public static void draw(GuiGraphicsExtractor context, Font renderer, String value, Tone tone, int x, int y, boolean shadow) {
        ampere.gui.vanillaui.components.UiText.draw(context, renderer, value, fontIdFor(tone), colorFor(tone), x, y, shadow);
    }

    public static void draw(GuiGraphicsExtractor context, Font renderer, String value, int color, int x, int y, boolean shadow) {
        ampere.gui.vanillaui.components.UiText.draw(context, renderer, value, UiAssets.FONT_BODY, color, x, y, shadow);
    }
