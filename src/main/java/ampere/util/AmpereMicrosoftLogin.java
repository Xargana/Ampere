...
public final class AmpereMicrosoftLogin {
    ...
    private AmpereMicrosoftLogin() {
    }

    public static String getRefreshToken(Consumer<String> callback) {
        AmpereMicrosoftLogin.callback = callback;
        startServer();
    ...
    public static LoginData login(String refreshToken) {
        JsonObject tokenResponse = AmpereHttp.postForm("https://login.live.com/oauth20_token.srf", "client_id=" + CLIENT_ID + "&refresh_token=" + refreshToken + "&grant_type=refresh_token&redirect_uri=http://127.0.0.1:" + PORT);
        ...
        JsonObject xbl = AmpereHttp.postJson("https://user.auth.xboxlive.com/user/authenticate", "{\"Properties\":{\"AuthMethod\":\"RPS\",\"SiteName\":\"user.auth.xboxlive.com\",\"RpsTicket\":\"d=" + accessToken + "\"},\"RelyingParty\":\"http://auth.xboxlive.com\",\"TokenType\":\"JWT\"}");
        if (xbl == null || !xbl.has("Token")) return new LoginData();

        JsonObject xsts = AmpereHttp.postJson("https://xsts.auth.xboxlive.com/xsts/authorize", "{\"Properties\":{\"SandboxId\":\"RETAIL\",\"UserTokens\":[\"" + xbl.get("Token").getAsString() + "\"]},\"RelyingParty\":\"rp://api.minecraftservices.com/\",\"TokenType\":\"JWT\"}");
        ...
        JsonObject mc = AmpereHttp.postJson("https://api.minecraftservices.com/authentication/login_with_xbox", "{\"identityToken\":\"XBL3.0 x=" + uhs + ";" + xsts.get("Token").getAsString() + "\"}");
        if (mc == null || !mc.has("access_token")) return new LoginData();
        String mcToken = mc.get("access_token").getAsString();

        JsonObject ownership = AmpereHttp.getJson("https://api.minecraftservices.com/entitlements/mcstore", mcToken);
        if (ownership == null || !hasGameOwnership(ownership)) return new LoginData();

        JsonObject profile = AmpereHttp.getJson("https://api.minecraftservices.com/minecraft/profile", mcToken);
    ...
    private static void startServer() {
        ...
        try {
            ...
            server.createContext("/", AmpereMicrosoftLogin::handleRequest);
            server.setExecutor(Executors.newVirtualThreadPerTaskExecutor());
            server.start();
        } catch (IOException e) {
            stopServer();
            AmpereMessaging.sendPrefixed("Failed to start Microsoft login server.");
        }
    }
    ...
    private static void handleCode(String code) {
        JsonObject response = AmpereHttp.postForm("https://login.live.com/oauth20_token.srf", "client_id=" + CLIENT_ID + "&code=" + code + "&grant_type=authorization_code&redirect_uri=http://127.0.0.1:" + PORT);
        Consumer<String> current = callback;
