    ...
    public static <T extends Packet<?>> T regenerate(T original) {
        ...
        if (handler != null) {
            try {
                ...
                AmpereClientAddon.LOG.error("[Ampere] Failed to regenerate {}: {}",
    ...
    private static void registerAllHandlers() {
        ...
        register(ServerboundContainerClickPacket.class, (packet, mc) -> {
            ...
            try {
                ...
                AmpereClientAddon.LOG.error("[Ampere] ClickSlot regeneration failed: {}", e.getMessage());
                return packet;
            }
        });
        ...
        register(ServerboundCustomPayloadPacket.class, (packet, mc) -> {
            ...
            String channel = AmperePayloadSupport.payloadChannel(payload);
            byte[] rawBytes = AmperePayloadSupport.extractPayloadBytes(payload);

            if (channel == null || channel.isBlank()) {
                AmpereClientAddon.LOG.warn("[Ampere] Cannot regenerate ServerboundCustomPayloadPacket - channel is blank");
                return null;
            }

            return AmperePayloadSupport.createC2SPacket(channel, rawBytes);
        });
    }
