package ampere.addons;

import ampere.AmpereClientAddon;
import ampere.api.AmpereAddon;
...
public final class AddonManager {
    ...
    private static final List<AmpereAddon> LOADED = new ArrayList<>();
    ...
    public static void init() {
        ...
        for (EntrypointContainer<AmpereAddon> container : FabricLoader.getInstance()
                .getEntrypointContainers("Ampere", AmpereAddon.class)) {
            ModContainer mod;
            try {
                mod = container.getProvider();
            } catch (Throwable t) {
                AmpereClientAddon.LOG.error("[Addons] Failed to inspect an addon entrypoint", t);
                continue;
            }
            ...
            if (isDisabledOnRestart(modId)) {
                report.markDisabled("Disabled in Ampere config. Enable it and restart to load.");
                continue;
            }
            ...
            try {
                ...
                AmpereClientAddon.LOG.error("[Addons] Failed to construct addon '{}'", modId, t);
                continue;
            }
            ...
            try {
                ...
                if (declared <= 0) {
                    ...
                    AmpereClientAddon.LOG.warn("[Addons] Addon '{}' declared invalid API v{}; skipping it.", modId, declared);
                    continue;
                }
                if (declared > ApiVersion.CURRENT) {
                    report.markNeedsNewerApi("Needs API v" + declared + ", host provides v" + ApiVersion.CURRENT + ".");
                    AmpereClientAddon.LOG.warn("[Addons] Addon '{}' needs API v{} but this client provides v{}; skipping it.",
                            modId, declared, ApiVersion.CURRENT);
                    continue;
                }
                if (declared < ApiVersion.CURRENT) {
                    AmpereClientAddon.LOG.info("[Addons] Addon '{}' built against API v{} (current v{}); loading.",
                            modId, declared, ApiVersion.CURRENT);
                }
                ...
                AmpereClientAddon.LOG.error("[Addons] Addon '{}' does not declare apiVersion(); skipping it.", modId, e);
            } catch (Throwable t) {
                report.markFailed("prepare", "Failed to prepare: " + shortError(t));
                AmpereClientAddon.LOG.error("[Addons] Error preparing addon '{}'", modId, t);
            }
        }
        ...
        if (!LOADED.isEmpty()) {
            AmpereClientAddon.LOG.info("[Addons] Loaded {} addon(s).", LOADED.size());
        }
        if (config.disabledAddonIds != null) {
            for (String id : config.disabledAddonIds) {
                ...
                report.markDisabled("Disabled in Ampere config, but no matching addon entrypoint was discovered.");
    ...
    private static boolean runPhase(LoadedAddon la, String phase, Runnable body) {
        ...
        try {
            ...
            AmpereClientAddon.LOG.error("[Addons] Addon '{}' threw during {}", la.modId, phase, t);
            return false;
        } finally {
            currentAddonId = null;
    ...
    private static void rollbackRegistrations(String addonId) {
        ...
        rollbackStep(addonId, report, "macro actions", () -> ampere.api.macro.MacroActionRegistry.unregisterAddon(addonId));
        rollbackStep(addonId, report, "presets", () -> ampere.api.macro.MacroPresetRegistry.unregisterAddon(addonId));
        rollbackStep(addonId, report, "events", () -> ampere.api.event.AddonEvents.unregisterAddon(addonId));
        rollbackStep(addonId, report, "hud", () -> ampere.api.hud.HudElements.unregisterAddon(addonId));
        rollbackStep(addonId, report, "modules", () -> ampere.modules.PackModuleRegistry.unregisterAddonModules(addonId));
        rollbackStep(addonId, report, "commands", () -> ampere.commands.AmpereCommands.unregisterAddonCommands(addonId));
    ...
    private static void rollbackStep(String addonId, AddonReport report, String kind, Runnable body) {
        try {
            ...
            AmpereClientAddon.LOG.warn("[Addons] Addon '{}' rollback failed for {}", addonId, kind, t);
        }
    }
    ...
    private static void applyMetadata(AmpereAddon addon, ModMetadata meta) {
    ...
    private static int metadataColor(ModMetadata meta, int fallback) {
        CustomValue color = meta.containsCustomValue("Ampere:color") ? meta.getCustomValue("Ampere:color") : null;
    ...
    public static List<AmpereAddon> loaded() {
    ...
    public static void surfaceFailuresOnJoin() {
        ...
        AmpereNotifications.warning(issues + " addon issue(s). Open Addons for details.");
    }
    ...
    private record LoadedAddon(String modId, AmpereAddon addon) {}
    ...
    public static final class AddonReport {
        ...
        private void copyRuntimeMetadata(AmpereAddon addon) {
