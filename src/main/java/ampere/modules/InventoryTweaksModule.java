...
public final class InventoryTweaksModule extends PackModule {
    ...
    InventoryTweaksModule() {
        ...
        option(PackModuleOption.keybind("sorting-key", "Sort Key", AmpereBindUtil.encodeMouseButton(GLFW.GLFW_MOUSE_BUTTON_MIDDLE))
    ...
    public static boolean handleSortMouse(int button, Slot hoveredSlot) {
        ...
        if (!AmpereBindUtil.isMouseBind(bind) || AmpereBindUtil.decodeMouseButton(bind) != button) return false;
    ...
    public static boolean handleSortKey(int keyCode, Slot hoveredSlot) {
        ...
        if (module == null || !module.isEnabled() || !module.bool("sorting-enabled")) return false;
        int bind = module.integer("sorting-key");
        if (AmpereBindUtil.isMouseBind(bind) || bind != keyCode) return false;
        return module.startSorting(hoveredSlot);
    }
    ...
    private boolean startSorting(Slot focusedSlot) {
        ...
        if (handler == null || !handler.getCarried().isEmpty()) {
            AmpereMessaging.sendPrefixed("Inventory Tweaks: clear your cursor before sorting.");
            return true;
        }
        ...
        if (actions.isEmpty()) {
            AmpereMessaging.sendPrefixed("Inventory Tweaks: already sorted.");
            return true;
        }
        sortingOperation = new SortingOperation(handler.containerId, actions, Math.max(0, integer("sorting-delay")));
        AmpereMessaging.sendPrefixed("Inventory Tweaks: sorting " + slots.size() + " slots.");
        return true;
    }
    ...
    private List<Slot> sortableSlotsFor(Slot focusedSlot, AbstractContainerMenu handler) {
        boolean focusedPlayer = AmpereInventoryHelper.isInventorySlot(MC, focusedSlot);
    ...
    private void handleContainerSynced(int containerId) {
        ...
        if (bool("auto-steal") && bool("auto-dump")) {
            if (!warnedAutoConflict) {
                ...
                AmpereMessaging.sendPrefixed("Inventory Tweaks: Auto Dump disabled because Auto Steal is enabled.");
            }
        }
    ...
    private void startTimedOperation(boolean steal, boolean automatic) {
        ...
        AbstractContainerMenu handler = MC.player.containerMenu;
        if (!canSteal(handler)) {
            if (!automatic) AmpereMessaging.sendPrefixed("Inventory Tweaks: this menu is not enabled for Steal/Dump.");
            return;
        }

        List<TimedClick> clicks = buildTimedClicks(handler, steal);
        if (clicks.isEmpty()) {
            if (!automatic) AmpereMessaging.sendPrefixed("Inventory Tweaks: nothing to " + (steal ? "steal." : "dump."));
            return;
        }
        ...
        if (!automatic) AmpereMessaging.sendPrefixed("Inventory Tweaks: " + (steal ? "stealing " : "dumping ") + clicks.size() + " stacks.");
    }
    ...
    private void tickAutoDrop() {
        ...
        for (int slot = start; slot < 36; slot++) {
            ...
            AmpereDropHelper.dropFromInventorySlot(MC, slot, 0);
            return;
        }
    }
    ...
    private final class SortingOperation {
        ...
        private boolean tick() {
            ...
            AmpereInventoryHelper.swapHandlerSlots(MC, action.fromSlot(), action.toSlot());
        ...
        private boolean tick() {
            ...
            while (index < clicks.size()) {
                ...
                if (click.drop()) {
                    ...
                    try {
                        AmpereInventoryClickHelper.click(MC, click.slot(), 1, ContainerInput.THROW);
                    } finally {
                        if (dropBackwards) MC.player.setYRot(oldYaw);
                    }
                } else {
                    AmpereInventoryClickHelper.click(MC, click.slot(), 0, ContainerInput.QUICK_MOVE);
                }
