...
final class PackModuleWorldRenderer {
    ...
    static void initialize() {
        ...
        LevelRenderEvents.COLLECT_SUBMITS.register(context -> {
            ...
            context.submitNodeCollector().submitCustomGeometry(poseStack, AmpereRenderTypes.storageEspFillSeeThrough(), (pose, buffer) -> {
            ...
            context.submitNodeCollector().submitCustomGeometry(poseStack, AmpereRenderTypes.storageEspLinesSeeThrough(), (pose, buffer) -> {
                if (drawTracers) {
    ...
    private static StorageSnapshot storageSnapshot(PackModule module, ClientLevel level, Player player, float tickDelta) {
        ...
        AmperePerf.endJoinSpike("join.storageEsp.scan", perf, 6_000_000L);
        return next;
    }
    ...
    private static StorageSnapshot blockSnapshot(PackModule module, ClientLevel level, Player player) {
        ...
        StorageSnapshot next = new StorageSnapshot(gameTime, revision, chunkX, chunkZ, List.copyOf(boxes), List.copyOf(traces));
        blockSnapshot = next;
        AmperePerf.endJoinSpike("join.blockEsp.scan", perf, 6_000_000L);
        return next;
    }
