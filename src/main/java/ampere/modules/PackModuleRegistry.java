...
public final class PackModuleRegistry {
    ...
    public static void initialize(AmpereConfig config) {
        ...
        ampere.util.AmpereEssentialBridge.restoreIfOrphaned(config);
    }
    ...
    public static AddonRegistrationResult registerAddonModuleDetailed(PackModule module, String addonId) {
        ...
        if (module.category() == null) {
            module.assignCategory(PackModuleCategory.registerAddon(addonId,
                ampere.addons.AddonManager.scopedCategoryLabel(null)));
        }
        MODULES.put(id, module);
        invalidateCaches(true);
        ampere.addons.AddonManager.recordAcceptedRegistration("module", id);
    ...
    private static AddonRegistrationResult rejectAddonModule(String addonId, String id, String reason) {
        AmpereClientAddon.LOG.warn("[Modules] Rejecting addon module '{}': {}", id, reason);
        ampere.addons.AddonManager.recordRejectedRegistration(addonId, "module", id, reason);
    ...
    private static void reportAddonModuleError(PackModule module, String hook, Throwable t) {
        AmpereClientAddon.LOG.error("[Modules] Addon module '{}' threw in {}; disabling it.", module.id(), hook, t);
        String id = module.id();
        int colon = id == null ? -1 : id.indexOf(':');
        if (colon > 0) {
            ampere.addons.AddonManager.recordRuntimeError(id.substring(0, colon), "Module " + id + " threw in " + hook);
        }
        try { module.setEnabled(false); } catch (Throwable ignored) {}
        try {
            ampere.util.AmpereMessaging.sendPrefixed("\u00a7cAddon module '" + module.name() + "' errored and was disabled.");
    ...
    public static boolean toggle(String idOrName, ampere.util.macro.ToggleModuleAction.ToggleMode mode) {
        PackModule module = get(idOrName);
        if (module == null) return false;
        ampere.util.macro.ToggleModuleAction.ToggleMode resolvedMode =
            mode == null ? ampere.util.macro.ToggleModuleAction.ToggleMode.TOGGLE : mode;
        if (PackHideState.isActive()
            && !PackHideState.isHideModule(module)
            && (resolvedMode == ampere.util.macro.ToggleModuleAction.ToggleMode.ENABLE
                || resolvedMode == ampere.util.macro.ToggleModuleAction.ToggleMode.TOGGLE)) {
            return false;
        }
        switch (resolvedMode) {
    ...
    public static void tick() {
        ...
        boolean profileJoin = AmperePerf.isJoinWindowActive();
        for (PackModule module : activeModules()) {
            if (profileJoin) {
                long perf = AmperePerf.beginJoin();
                tickModule(module);
                AmperePerf.endJoinSpike("join.module.tick." + module.id(), perf);
            } else {
                tickModule(module);
            }
        }
    ...
    public static boolean tickMenuKey(int keyCode) {
        ...
        boolean pressed = AmpereBindUtil.isBindPressed(MC, keyCode);
        ...
        for (PackModule module : MODULES.values()) {
            if (module.isEnabled() || module.ticksWhenDisabled()) {
                if (profileJoin) {
                    long perf = AmperePerf.beginJoin();
                    joinModule(module);
                    AmperePerf.endJoinSpike("join.module.onGameJoin." + module.id(), perf);
                } else {
                    joinModule(module);
                }
            }
    ...
    private static void tickModuleKeybinds() {
        ...
        for (PackModule module : keyboundModules()) {
            ...
            boolean pressed = AmpereBindUtil.isBindPressed(MC, bind);
            boolean wasPressed = KEY_STATES.getOrDefault(module.id(), false);
            if (pressed && !wasPressed && AmpereInputGate.canRunAmpereKeybinds() && module.hasActivationToggle()) module.toggle();
