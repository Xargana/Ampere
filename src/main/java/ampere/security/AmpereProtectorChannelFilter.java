...
public final class AmpereProtectorChannelFilter {
    ...
    private AmpereProtectorChannelFilter() {
    }
    ...
    public static Verdict filter(Packet<?> packet) {
        ...
        if (!AmpereProtector.shouldFilterChannels()) return PASS;

        if (AmpereProtector.isUserBypass(packet)) return PASS;
        ...
        if (AmpereProtector.isVanillaMode()) {
            return DROP;
        }
        ...
        if (AmpereProtectorTracker.isWhitelistedChannel(id)) return PASS;
    ...
    private static Verdict rebuildRegister(RegistrationPayload original) {
        ...
        try {
            for (Identifier channel : original.channels()) {
                if (AmpereProtectorTracker.isWhitelistedChannel(channel)) {
                    kept.add(channel);
                }
            }
        } catch (Throwable t) {
            if (DEBUG) {
                AmpereClientAddon.LOG.debug("[AmpereProtector] register payload inspection failed: {}", t.getMessage());
            }
            return DROP;
        }
    ...
    private static RegistrationPayload newRegistrationPayload(RegistrationPayload original, List<Identifier> channels) {
        ...
        AmpereClientAddon.LOG.warn("[AmpereProtector] No compatible RegistrationPayload constructor; dropping packet.");
        return null;
    }
