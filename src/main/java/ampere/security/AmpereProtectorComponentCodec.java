...
public record AmpereProtectorComponentCodec(Codec<Component> wrapped) implements Codec<Component> {

    @Override
    public <T> DataResult<Pair<Component, T>> decode(DynamicOps<T> ops, T input) {
        DataResult<Pair<Component, T>> result = wrapped.decode(ops, input);
        if (!AmpereProtector.shouldTagPacketComponents()) return result;
        if (!AmpereProtectorPacketContext.isProcessingPacket()) return result;
        Minecraft mc;
        try {
    ...
    private static void markTree(Component component) {
        ...
        if (contents instanceof AmpereFromPacketAccess access) access.Ampere$setFromPacket();
