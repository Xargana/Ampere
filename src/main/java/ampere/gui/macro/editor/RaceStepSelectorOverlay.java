...
public final class RaceStepSelectorOverlay extends AmpereOverlayBase {
    ...
    private void renderRow(GuiGraphicsExtractor context, Option option, int x, int y, int width, int index, float mouseX, float mouseY) {
        ...
        int accent = "Condition".equalsIgnoreCase(option.category()) ? AmpereColors.packetCyan() : AmpereColors.accent();
        ...
        CompactSurfaces.divider(context, x + 4, y + ROW_HEIGHT - 1, width - 8, AmpereColors.packetRowDivider());
    }
    ...
    private AmpereWindowLayout clampToViewport(AmpereWindowLayout layout) {
        DirectViewport viewport = surface.viewport();
        int margin = 4;
        ...
        return new AmpereWindowLayout(x, y, width, height, layout.visible, layout.collapsed);
    }
}
