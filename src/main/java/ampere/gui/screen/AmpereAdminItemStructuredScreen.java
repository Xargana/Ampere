...
public final class AmpereAdminItemStructuredScreen extends Screen {
    ...
    private final CompactTextInput modifier = input("Ampere:modifier").setText("Ampere:modifier");
    ...
    public AmpereAdminItemStructuredScreen(Screen parent, PackModule module, PackModuleOption option, Mode mode) {
    ...
    private void addRow(List<RegistryRow> rows, String filter, String id, String label) {
        String resolved = label == null || label.isBlank() ? AmpereRegistryLabels.identifier(id) : label;
    ...
    public boolean mouseClicked(MouseButtonEvent event, boolean doubleClick) {
        ...
        if (event.button() == 0 && rawBounds.contains(mx, my)) {
            saveDraft();
            if (minecraft != null) minecraft.setScreen(new AmpereAdminItemOptionScreen(parent, module, option,
                mode == Mode.ENCHANTMENTS ? AmpereAdminItemOptionScreen.Mode.ENCHANTMENTS : AmpereAdminItemOptionScreen.Mode.ATTRIBUTES));
            return true;
        }
    ...
    private void selectEntry(int index) {
        ...
        if (mode == Mode.ENCHANTMENTS) {
            ...
            modifier.setText(extract(entry, "id", "Ampere:modifier"));
        }
    }
    ...
    private String attributeEntry(String type) {
        ...
        String modifierId = sanitizeIdentifier(modifier.text(), "Ampere:modifier");
