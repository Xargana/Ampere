...
public class AmpereHudElementSettingsScreen extends Screen {
    ...
    public AmpereHudElementSettingsScreen(Screen parent, String id) {
    ...
    public void extractRenderState(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float delta) {
        ...
        try {
            ...
            drawTopBar(graphics, x, y, w, h, HEADER_H, AmpereHudManager.label(id), mx, my);
            ...
            enumDropdowns.clear();
            ampere.gui.vanillaui.UiScissorStack.global().push(graphics,
                ampere.gui.vanillaui.UiBounds.of(x + 2, bodyTop, Math.max(0, w - 9), Math.max(0, bodyBottom - bodyTop)));
            try {
                ...
                ampere.gui.vanillaui.UiScissorStack.global().pop(graphics);
            }
            ...
            boolean enabled = AmpereHudManager.state(id).enabled;
    ...
    private void row(GuiGraphicsExtractor graphics, Row row, int x, int y, int w, int mx, int my) {
        ...
        if (row.type == RowType.BOOL) {
            ...
            int pickW = 44;
            int color = AmpereHudManager.parseColor(AmpereHudManager.setting(id, row.key), defaultColor(row.key));
            int swatchX = valueX;
            int swatchY = y + 5;
            ...
            double value = AmpereHudManager.doubleSetting(id, row.key, row.min);
            renderSlider(graphics, row, valueX, y + 5, valueW, 12, value, row.key != null && row.key.equals(focusedKey));
        } else if (row.type == RowType.TEXT) {
            String value = row.key.equals(focusedKey) ? editingText : AmpereHudManager.setting(id, row.key);
    ...
    private void renderDropdown(Row row, int x, int y, int w, int h) {
        ...
        int selected = Math.max(0, choices.indexOf(AmpereHudManager.setting(id, row.key)));
        ...
        dropdown.setBounds(x, y, w, h)
            ...
            .setOnSelect(index -> {
                AmpereHudManager.setSetting(id, row.key, choices.get(index));
                clearHudFocus();
            });
    ...
    private void renderModulePicker(GuiGraphicsExtractor graphics, int sw, int sh, int mx, int my) {
        ...
        ampere.gui.vanillaui.UiScissorStack.global().push(graphics,
            ampere.gui.vanillaui.UiBounds.of(x + 2, viewTop, Math.max(0, w - 4), Math.max(0, y + h - 4 - viewTop)));
        try {
    ...
    public boolean mouseClicked(MouseButtonEvent event, boolean doubled) {
        ...
        if (hover(mx, my, x + 94, footerY, 78, 20)) {
            AmpereHudManager.resetElement(id);
            clearHudFocus();
            return true;
        }
        if (hover(mx, my, x + 180, footerY, 86, 20)) {
            AmpereHudManager.toggle(id);
            return true;
        }
    ...
    public boolean mouseScrolled(double mouseX, double mouseY, double scrollX, double scrollY) {
        double virtualX = AmpereUiScale.toVirtual(mouseX);
        double virtualY = AmpereUiScale.toVirtual(mouseY);
        if (colorPicker != null) {
    ...
    private void handleRowClick(Row row, int button, int mx, int my, int valueX, int valueW) {
        ...
        if (row.type == RowType.BOOL) {
            if ("enabled".equals(row.key)) AmpereHudManager.toggle(id);
            else AmpereHudManager.setSetting(id, row.key, Boolean.toString(!bool(row.key)));
            clearHudFocus();
        } else if (row.type == RowType.COLOR) {
            ...
            if (mx >= fieldX && mx < fieldX + 50) {
                focusedKey = row.key;
                editingText = AmpereHudManager.setting(id, row.key);
                return;
            }
    ...
    private void openColorPicker(String key, UiBounds anchor) {
        ...
        int initial = AmpereHudManager.parseColor(AmpereHudManager.setting(id, key), fallback);
        colorPicker = new ColorPicker(anchor, initial, fallback,
            AmpereUiScale.getVirtualScreenWidth(), AmpereUiScale.getVirtualScreenHeight(), argb -> {
                AmpereHudManager.setSetting(id, key, String.format(Locale.ROOT, "%08X", argb));
                clearColorPicker();
            });
    }
    ...
    private int defaultColor(String key) {
        return AmpereHudManager.parseColor(AmpereHudManager.defaultSetting(id, key), 0xFFFFFFFF);
    }

    private void setNumeric(Row row, double value) {
        double clamped = Math.max(row.min, Math.min(row.max, value));
        AmpereHudManager.setSetting(id, row.key, formatNumber(clamped, row.step));
    }
    ...
    private void commitFocus() {
        ...
        if (row.type == RowType.NUMBER) {
            ...
            AmpereHudManager.setSetting(id, row.key, editingText.trim());
        }
        clearHudFocus();
    }
    ...
    private List<Row> rows() {
        ...
        if (AmpereHudManager.WATERMARK.equals(id)) {
            rows.add(Row.section("Logo"));
            rows.add(Row.number("logo-width", "Width", RowType.NUMBER, 48, 420, 1));
        } else if (!AmpereHudManager.ARMOR.equals(id) && !AmpereHudManager.INVENTORY.equals(id)) {
            rows.add(Row.section("Text"));
            if (!AmpereHudManager.COMPASS.equals(id)) {
        ...
        if (AmpereHudManager.boolSetting(id, "background")) rows.add(Row.color("background-color", "Background Color"));

        rows.add(Row.section("Outline"));
        rows.add(Row.bool("outline", "Outline"));
        if (AmpereHudManager.boolSetting(id, "outline")) {
            rows.add(Row.color("outline-color", "Outline Color"));
            rows.add(Row.number("outline-width", "Outline Width", RowType.NUMBER, 1, 6, 1));
        }

        if (AmpereHudManager.ACTIVE_MODULES.equals(id)) {
            ...
            String mode = AmpereHudManager.setting(id, "color-mode");
            ...
            if (AmpereHudManager.boolSetting(id, "module-info") || AmpereHudManager.boolSetting(id, "show-keybind")) rows.add(Row.color("module-info-color", "Info Color"));
        ...
        if (AmpereHudManager.ITEM_COUNTER.equals(id)) {
    ...
    private List<String> hiddenModuleIds() {
        ...
        String raw = AmpereHudManager.setting(AmpereHudManager.ACTIVE_MODULES, "hidden-modules");
    ...
    private void toggleHiddenModule(String moduleId) {
        ...
        AmpereHudManager.setSetting(AmpereHudManager.ACTIVE_MODULES, "hidden-modules", String.join("|", ids));
    }
    ...
    private boolean visualOnlyElement() {
        return AmpereHudManager.ARMOR.equals(id) || AmpereHudManager.INVENTORY.equals(id) || AmpereHudManager.COMPASS.equals(id) || AmpereHudManager.WATERMARK.equals(id);
    }
    ...
    private boolean bool(String key) {
        if ("enabled".equals(key)) return AmpereHudManager.state(id).enabled;
        return AmpereHudManager.boolSetting(id, key);
    }
