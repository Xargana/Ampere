...
public class AmpereHudEditorScreen extends Screen {
    ...
    public AmpereHudEditorScreen(Screen parent) {
        super(Component.literal("Ampere HUD Editor"));
        this.parent = parent;
    ...
    public void extractRenderState(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float delta) {
        ...
        try {
            ...
            AmpereHudManager.render(graphics, font, true, selectedId, mx, my);
    ...
    private void renderToolbar(GuiGraphicsExtractor graphics, int sw, int sh, int mx, int my) {
        ...
        int titleW = UiText.width(font, "Ampere HUD Editor", THEME.fontFor(UiTone.BODY), TEXT) + 30;
        ...
        CompactWindow.renderFrame(UiContexts.overlay(graphics, font, mx, my), UiBounds.of(x, y, w, h),
            "Ampere HUD Editor", false, false, false, hover(mx, my, x, y, w, 16), true, 7, 7, 16);
    ...
    private boolean intersectsHudElement(int x, int y, int w, int h) {
        for (String id : AmpereHudManager.elementIds()) {
            if (!AmpereHudManager.state(id).enabled) continue;
            AmpereHudManager.ElementBounds bounds = AmpereHudManager.bounds(id, font);
    ...
    private void renderAddPicker(GuiGraphicsExtractor graphics, int mx, int my) {
        int w = 150;
        int rowH = 18;
        int contentH = 24 + AmpereHudManager.elementIds().size() * rowH;
        int h = Math.min(contentH, Math.max(80, AmpereUiScale.getVirtualScreenHeight() - 8));
        int viewH = Math.max(20, h - 24);
        pickerScroll = clamp(pickerScroll, 0, Math.max(0, contentH - h));
        int x = Math.min(pickerX, Math.max(0, AmpereUiScale.getVirtualScreenWidth() - w - 4));
        int y = Math.min(pickerY, Math.max(0, AmpereUiScale.getVirtualScreenHeight() - h - 4));
        CompactWindow.renderFrame(UiContexts.overlay(graphics, font, mx, my), UiBounds.of(x, y, w, h),
            "Add / Toggle", false, false, false, hover(mx, my, x, y, w, 24), true, 7, 7, 24);
        ampere.gui.vanillaui.UiScissorStack.global().push(graphics,
            ampere.gui.vanillaui.UiBounds.of(x + 2, y + 24, Math.max(0, w - 4), Math.max(0, h - 26)));
        try {
            ...
            if (enabled) CompactSurfaces.indicator(graphics, x + 5, cy + 1, 3, rowH - 4, GREEN);
                    draw(graphics, AmpereHudManager.label(id), x + 12, cy + 4, enabled ? TEXT : MUTED, w - 18);
                }
            cy += rowH;
            }
        } finally {
    ...
    public boolean mouseClicked(MouseButtonEvent event, boolean doubled) {
        ...
        if (addPickerOpen) {
            ...
            if (picked != null) {
                AmpereHudManager.setEnabled(picked, true);
                int[] snapped = snappedPosition(picked, mx, my);
                AmpereHudManager.move(picked, snapped[0], snapped[1], AmpereUiScale.getVirtualScreenWidth(), AmpereUiScale.getVirtualScreenHeight());
                selectedId = picked;
                addPickerOpen = false;
                pickerScroll = 0;
                return true;
        ...
        String hit = AmpereHudManager.hit(font, mx, my);
        if (event.button() == GLFW.GLFW_MOUSE_BUTTON_RIGHT) {
            if (hit != null) {
                minecraft.setScreen(new AmpereHudElementSettingsScreen(this, hit));
            } else {
                pickerX = mx;
                pickerY = my;
                pickerScroll = 0;
        ...
        if (event.button() == GLFW.GLFW_MOUSE_BUTTON_LEFT && hit != null) {
            selectedId = hit;
            draggingId = hit;
            AmpereHudManager.ElementBounds bounds = AmpereHudManager.bounds(hit, font);
    ...
    public boolean mouseDragged(MouseButtonEvent event, double dx, double dy) {
        ...
        AmpereHudManager.move(draggingId, snapped[0], snapped[1], AmpereUiScale.getVirtualScreenWidth(), AmpereUiScale.getVirtualScreenHeight());
        moved = true;
        return true;
    }
    ...
    public boolean keyPressed(KeyEvent input) {
        ...
        if (selectedId != null && input.key() == GLFW.GLFW_KEY_DELETE) {
            AmpereHudManager.setEnabled(selectedId, false);
            return true;
        }
        if (selectedId != null) {
            int step = (input.modifiers() & GLFW.GLFW_MOD_CONTROL) != 0 ? VISUAL_GRID_SIZE : 1;
            AmpereHudManager.ElementBounds bounds = AmpereHudManager.bounds(selectedId, font);
            int x = bounds.x();
            int y = bounds.y();
            boolean moved = true;
            switch (input.key()) {
            ...
            if (moved) {
                ...
                AmpereHudManager.move(selectedId, snapped[0], snapped[1], AmpereUiScale.getVirtualScreenWidth(), AmpereUiScale.getVirtualScreenHeight());
                return true;
            }
        }
        return true;
    ...
    private SnapResult snapAxis(String id, int value, int size, int screenSize, boolean horizontal) {
        ...
        for (String other : AmpereHudManager.elementIds()) {
            if (other.equals(id) || !AmpereHudManager.state(other).enabled) continue;
            AmpereHudManager.ElementBounds ob = AmpereHudManager.bounds(other, font);
