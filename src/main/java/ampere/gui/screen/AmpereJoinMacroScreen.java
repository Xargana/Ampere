...
public final class AmpereJoinMacroScreen extends Screen {
    ...
    private AmpereChatField searchField;
    private int scroll;
    ...
    public AmpereJoinMacroScreen(Screen parent) {
    ...
    protected void init() {
        ...
        int y = TOP_PANEL_Y;
        if (searchField == null) {
            searchField = new AmpereChatField(minecraft, font, x + 22, y + 48, panelW() - 44, 18, false);
    ...
    private void renderMethodRow(GuiGraphicsExtractor graphics, int mouseX, int mouseY) {
        ...
        addButton(dropdownX, y, dropdownW, 18, AmpereJoinMacroController.timing().label(), CompactOverlayButton.Variant.PRIMARY, true, () -> {
            toggleDropdown(DropdownKind.METHOD, methodX, methodY, methodW, methodH);
        });

        int triggerRowY = y + 24;
        graphics.text(font, AmpereJoinMacroController.keepEnabled() ? "Repeat On" : "Run On", x, triggerRowY + 5, 0xFFEFE8E4, false);
        triggerX = dropdownX;
        triggerY = triggerRowY;
        triggerW = 118;
        triggerH = 18;
        addButton(triggerX, triggerY, triggerW, triggerH, triggerLabel(AmpereJoinMacroController.triggerJoin()), CompactOverlayButton.Variant.PRIMARY, true, () -> {
            toggleDropdown(DropdownKind.TRIGGER, triggerX, triggerY, triggerW, triggerH);
        });

        String keepLabel = AmpereJoinMacroController.keepEnabled() ? "Stays Enabled" : "Clears After";
        addToggleButton(triggerX + triggerW + GAP, triggerY, 118, 18, keepLabel,
            AmpereJoinMacroController.keepEnabled(), "join-macro:keep-enabled", true, () -> {
            AmpereJoinMacroController.setKeepEnabled(!AmpereJoinMacroController.keepEnabled());
        });
    }

    private void renderStatus(GuiGraphicsExtractor graphics, int x, int y, int maxWidth) {
        String selected = AmpereJoinMacroController.selectedMacroName();
        if (selected.isBlank()) {
        ...
        String modeLine = AmpereJoinMacroController.modeSummary();
    ...
    public boolean mouseClicked(MouseButtonEvent event, boolean doubled) {
        ...
        if (event.button() == 0) {
            ...
            if (rowIndex >= 0) {
                ...
                String selected = AmpereJoinMacroController.selectedMacroName();
                AmpereJoinMacroController.setSelectedMacro(row.name.equalsIgnoreCase(selected) ? "" : row.name);
                return true;
            }
        }
        if (searchField != null) searchField.setFocused(false);
    ...
    public boolean mouseDragged(MouseButtonEvent event, double dx, double dy) {
        event = virtualEvent(event);
        dx = AmpereUiScale.toVirtual(dx);
        dy = AmpereUiScale.toVirtual(dy);
        if (openDropdown != DropdownKind.NONE) return true;
        if (macroScrollbarDragging) {
    ...
    private AmpereMacro selectedMacro() {
        String selected = AmpereJoinMacroController.selectedMacroName();
        return selected.isBlank() ? null : AmpereMacroManager.get().get(selected);
    }

    private void openEditor(AmpereMacro macro) {
        AmpereMacroEditorOverlay editor = AmpereMacroEditorOverlay.getSharedOverlay();
        AmpereOverlayManager.get().register(editor, ampere.util.IAmpereOverlay.OverlayScope.HOST_SCREEN);
        editor.openForJoinMacroMenu(macro, saved -> {
            if (saved != null && saved.name != null && !saved.name.isBlank()) {
                AmpereJoinMacroController.setSelectedMacro(saved.name);
                AmpereNotifications.show("Join macro selected: " + saved.name, 0xFF66E08A);
            }
        });
        if (minecraft != null) minecraft.setScreen(new AmpereOverlayHostScreen(editor, new AmpereJoinMacroScreen(parent)));
    }
    ...
    private boolean handleOpenDropdownClick(double mouseX, double mouseY, int button) {
        ...
        if (index >= 0 && index < itemCount) {
            if (openDropdown == DropdownKind.METHOD) {
                AmpereJoinMacroController.setTiming(AmpereJoinMacroController.Timing.values()[index]);
            } else if (openDropdown == DropdownKind.TRIGGER) {
                AmpereJoinMacroController.setTriggerJoin(triggerOptions()[index]);
            }
        }
        openDropdown = DropdownKind.NONE;
        return true;
    ...
    private String[] dropdownLabels() {
        if (openDropdown == DropdownKind.TRIGGER) {
            AmpereJoinMacroController.TriggerJoin[] targets = triggerOptions();
            String[] labels = new String[targets.length];
            for (int i = 0; i < targets.length; i++) labels[i] = triggerLabel(targets[i]);
            return labels;
        }

        AmpereJoinMacroController.Timing[] methods = AmpereJoinMacroController.Timing.values();
    ...
    private boolean dropdownActive(int index) {
        if (openDropdown == DropdownKind.TRIGGER) {
            AmpereJoinMacroController.TriggerJoin[] targets = triggerOptions();
            return index >= 0 && index < targets.length && targets[index] == AmpereJoinMacroController.triggerJoin();
        }
        AmpereJoinMacroController.Timing[] methods = AmpereJoinMacroController.Timing.values();
        return index >= 0 && index < methods.length && methods[index] == AmpereJoinMacroController.timing();
    }
    ...
    private AmpereJoinMacroController.TriggerJoin[] triggerOptions() {
        if (AmpereJoinMacroController.keepEnabled()) {
            return new AmpereJoinMacroController.TriggerJoin[] {
                AmpereJoinMacroController.TriggerJoin.ANY,
                AmpereJoinMacroController.TriggerJoin.SECOND,
                AmpereJoinMacroController.TriggerJoin.THIRD,
                AmpereJoinMacroController.TriggerJoin.FOURTH,
                AmpereJoinMacroController.TriggerJoin.FIFTH,
                AmpereJoinMacroController.TriggerJoin.SIXTH_PLUS
            };
        }

        return new AmpereJoinMacroController.TriggerJoin[] {
            AmpereJoinMacroController.TriggerJoin.FIRST,
            AmpereJoinMacroController.TriggerJoin.SECOND,
            AmpereJoinMacroController.TriggerJoin.THIRD,
            AmpereJoinMacroController.TriggerJoin.FOURTH,
            AmpereJoinMacroController.TriggerJoin.FIFTH,
    ...
    private String triggerLabel(AmpereJoinMacroController.TriggerJoin triggerJoin) {
        return triggerJoin.displayLabel(AmpereJoinMacroController.keepEnabled());
    }
    ...
    private int screenWidth() {
        int scaled = AmpereUiScale.getVirtualScreenWidth();
        return scaled <= 0 ? width : scaled;
    }

    private int screenHeight() {
        int scaled = AmpereUiScale.getVirtualScreenHeight();
