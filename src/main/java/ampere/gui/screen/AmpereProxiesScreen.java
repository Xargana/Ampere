...
import ampere.util.AmpereProxyType;
...
public class AmpereProxiesScreen extends Screen {
    ...
    private void rebuildButtons() {
        ...
        buttons.add(CompactOverlayButton.create(actionX, 96, actionWidths[0], 18, Component.literal("Type: " + type), b -> {
            type = type == AmpereProxyType.Socks5 ? AmpereProxyType.Socks4 : AmpereProxyType.Socks5;
            rebuildButtons();
        ...
        buttons.add(CompactOverlayButton.create(actionX, 96, actionWidths[3], 18, Component.literal(AmpereProxyManager.get().isRefreshing() ? "Refreshing" : "Refresh"), b -> refreshProxies()).setVariant(CompactOverlayButton.Variant.SECONDARY));
        ...
        buttons.add(CompactOverlayButton.create(actionX, 96, actionWidths[6], 18, Component.literal("Config"), b -> this.minecraft.setScreen(new AmpereProxyConfigScreen(this))).setVariant(CompactOverlayButton.Variant.SECONDARY));
        ...
        List<AmpereProxy> proxies = filteredProxies();
        ...
        for (int i = firstVisible; i < proxies.size() && rowY + ROW_HEIGHT - 3 <= proxyRowsBottom(); i++) {
            ...
            CompactOverlayButton check = CompactOverlayButton.create(proxyRowRight() - 122, rowY + 4, 54, 16, Component.literal(proxy.status == AmpereProxy.Status.CHECKING ? "..." : "Check"), b -> checkProxy(proxy));
            check.setVariant(CompactOverlayButton.Variant.PRIMARY);
            check.active = proxy.status != AmpereProxy.Status.CHECKING;
    ...
    private void addProxy() {
        AmpereProxy proxy = proxyFromFields();
        if (proxy == null) return;
        if (AmpereProxyManager.get().add(proxy)) {
            selectedProxy = null;
            clearProxyFields();
    ...
    private void updateProxy() {
        ...
        AmpereProxy updated = proxyFromFields();
        if (updated == null) return;
        if (AmpereProxyManager.get().update(selectedProxy, updated)) {
    ...
    private void toggleProxy(AmpereProxy proxy) {
        if (proxy == null) return;
        AmpereProxyManager.get().setEnabled(proxy, !proxy.enabled);
        showProxyToast(proxy.enabled ? "Enabled " + proxy.displayName() + "." : "Disabled proxy.", proxy.enabled ? SUCCESS : MUTED);
        rebuildButtons();
    }

    private void deleteProxy(AmpereProxy proxy) {
        if (proxy == null) return;
        AmpereProxyManager.get().remove(proxy);
        if (proxy.equals(selectedProxy)) selectedProxy = null;
        showProxyToast("Deleted " + proxy.displayName() + ".", MUTED);
        rebuildButtons();
    }

    private void checkProxy(AmpereProxy proxy) {
        if (proxy == null || proxy.status == AmpereProxy.Status.CHECKING) return;
        rebuildButtons();
        Thread thread = new Thread(() -> {
            proxy.checkStatus(AmpereProxyManager.get().getTimeoutMs());
    ...
    private void refreshProxies() {
        AmpereProxyManager manager = AmpereProxyManager.get();
    ...
    private void importProxies() {
        ...
        if (selectedFile != null) {
            ...
            int added = AmpereProxyManager.get().importFromFile(file);
    ...
    private AmpereProxy proxyFromFields() {
    ...
    public void extractRenderState(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float delta) {
        ...
        try {
            ...
            List<AmpereProxy> proxies = filteredProxies();
            int total = AmpereProxyManager.get().all().size();
            int enabled = AmpereProxyManager.get().getEnabled() == null ? 0 : 1;
            ...
            String refreshing = AmpereProxyManager.get().isRefreshing() ? "REFRESHING" : "";
            ...
            if (AmpereProxyManager.get().all().isEmpty()) {
    ...
    private void selectProxy(AmpereProxy proxy) {
        selectedProxy = proxy;
        if (proxy != null) {
            ...
            type = proxy.type == null ? AmpereProxyType.Socks5 : proxy.type;
        }
        rebuildButtons();
    }

    private List<AmpereProxy> filteredProxies() {
        List<AmpereProxy> proxies = AmpereProxyManager.get().all();
        String query = normalize(searchQuery);
        if (query.isEmpty() && filter == ProxyFilter.ALL) return proxies;
        List<AmpereProxy> filtered = new ArrayList<>();
    ...
    private boolean matchesFilter(AmpereProxy proxy) {
        return switch (filter) {
            case ALL -> true;
            case ALIVE -> proxy.status == AmpereProxy.Status.ALIVE;
            case DEAD -> proxy.status == AmpereProxy.Status.DEAD;
            case UNCHECKED -> proxy.status == AmpereProxy.Status.UNCHECKED || proxy.status == AmpereProxy.Status.CHECKING;
            case ENABLED -> proxy.enabled;
        };
    }

    private boolean matchesSearch(AmpereProxy proxy, String query) {
    ...
    private String statusText(AmpereProxy proxy) {
        if (proxy == null) return "";
    ...
    private record ProxyRow(AmpereProxy proxy, int y, CompactOverlayButton toggleButton, CompactOverlayButton checkButton, CompactOverlayButton deleteButton) {
    }
}
