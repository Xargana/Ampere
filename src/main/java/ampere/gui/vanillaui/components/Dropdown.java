...
public final class Dropdown implements UiComponent {
    ...
    private void prepareInputLayout() {
        if (!open) return;
        int screenWidth = lastScreenWidth > 0 ? lastScreenWidth : Math.max(1, AmpereUiScale.getVirtualScreenWidth());
        int screenHeight = lastScreenHeight > 0 ? lastScreenHeight : Math.max(1, AmpereUiScale.getVirtualScreenHeight());
