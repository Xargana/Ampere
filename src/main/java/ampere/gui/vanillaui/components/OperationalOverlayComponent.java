...
public final class OperationalOverlayComponent implements UiComponent {
    ...
    public UiBounds bounds() {
        AmpereWindowLayout bounds = overlay.getBounds();
        return UiBounds.of(bounds.x, bounds.y, bounds.width,
            bounds.collapsed ? AmpereWindow.sharedHeaderHeight() : bounds.height);
    }

    @Override
    public void setBounds(UiBounds bounds) {
        if (bounds == null) return;
        AmpereWindowLayout current = overlay.getBounds();
        overlay.setBounds(new AmpereWindowLayout(bounds.x(), bounds.y(), bounds.width(), bounds.height(), current.visible, current.collapsed));
    }
