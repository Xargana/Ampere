...
import ampere.gui.vanillaui.components.*;
...
public final class DirectScrollList<T> {
    ...
    private int borderColor = AmpereColors.secondary();
    private int backgroundColor = AmpereColors.listBg();
    private int selectedTextColor = AmpereColors.rowSelectedText();
    private int normalTextColor = 0xFFFFFFFF;
    private int disabledTextColor = AmpereColors.textDim();
    ...
    private int secondaryTextColor = AmpereColors.textDim();

    public DirectScrollList(int x, int y, int width, int height, int rowHeight, int scrollbarWidth,
    ...
    private void renderContent(GuiGraphicsExtractor ctx, Font textRenderer, double mouseX, double mouseY) {
        viewport.renderSimple(ctx, items.size(), (index, bounds) -> {
            ...
            if (label != null && !label.isEmpty()) {
                ...
                String trimmedLabel = AmpereText.trimToWidth(textRenderer, label, labelMaxWidth, AmpereText.Tone.BODY);
                AmpereText.draw(ctx, textRenderer, trimmedLabel, textColor, textX, textY, false);

                if (badge != null && !badge.isEmpty()) {
                    int labelWidth = AmpereText.width(textRenderer, trimmedLabel, AmpereText.Tone.BODY);
                    int badgeX = textX + labelWidth + 4;
                    int badgeColor = badgeColorExtractor != null ? badgeColorExtractor.apply(item) : 0xFFFFAA00;
                    AmpereText.draw(ctx, textRenderer, badge, badgeColor, badgeX, textY, false);
                }

                if (secondary != null && !secondary.isEmpty()) {
                    int secondaryWidth = textRenderer.width(secondary);
                    int secondaryX = bounds.x + bounds.width - secondaryWidth - 6;
                    AmpereText.draw(ctx, textRenderer, secondary, secondaryTextColor, secondaryX, textY, false);
                }
            }
        });
    }
