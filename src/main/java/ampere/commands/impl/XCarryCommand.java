...
import ampere.commands.args.XCarrySlotArgumentType;
...
public class XCarryCommand extends Command {
    ...
    public void build(LiteralArgumentBuilder<AmpereCommandSource> root) {
        root.executes(ctx -> {
            ...
            AmpereMessaging.sendPrefixed("§eUsage: §f" + prefix + "xcarry <slot>");
            AmpereMessaging.sendPrefixed("§7Slots: §fcraft1§7..§fcraft4§7, §fhelmet§7, §fchestplate§7, §fleggings§7, §fboots§7, §foffhand§7, §fcursor§7.");
            return SUCCESS;
        });
        root.then(RequiredArgumentBuilder.<AmpereCommandSource, Integer>argument("slot", XCarrySlotArgumentType.slot())
    ...
    private static int stash(int targetSlot) {
        ...
        if (p == null || mc.gameMode == null) {
            AmpereMessaging.sendPrefixed("§cNot in a world.");
            return SUCCESS;
        }

        ItemStack heldStack = p.getMainHandItem().copy();
        if (heldStack == null || heldStack.isEmpty()) {
            AmpereMessaging.sendPrefixed("§cHold an item first.");
            return SUCCESS;
        }

        if (p.containerMenu != p.inventoryMenu) {
            AmpereMessaging.sendPrefixed("§cOpen your inventory first (E), then run the command.");
            return SUCCESS;
        }
        ...
        try {
            ...
            if (container.getCarried().isEmpty()) {
                AmpereMessaging.sendPrefixed("§cCould not pick up the held item.");
                return SUCCESS;
            }
            ...
            if (targetSlot == XCarrySlotArgumentType.CURSOR) {
                ...
                AmpereMessaging.sendPrefixed("§aHolding §f" + itemName + "§a on cursor (XCarry).");
                return SUCCESS;
            }

            if (targetSlot < 0 || targetSlot >= container.slots.size()) {
                AmpereMessaging.sendPrefixed("§cInvalid slot (" + targetSlot + ").");
            ...
            shared.setXCarryForced(true);
            shared.setXCarryActive(true);
            AmpereMessaging.sendPrefixed("§aStashed §f" + itemName + "§a into §f" + slotName + "§a.");
            return SUCCESS;
        } finally {
    ...
    private static LinkedHashSet<Integer> mergedForcedSlots(AmpereSharedState shared) {
