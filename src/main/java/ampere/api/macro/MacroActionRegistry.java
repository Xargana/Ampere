...
import ampere.gui.macro.editor.ActionFieldSchema;
...
public final class MacroActionRegistry {
    ...
    public static AddonRegistrationResult registerDetailed(MacroActionEntry entry) {
        ...
        try {
            ...
            AmpereClientAddon.LOG.warn("[MacroActions] Rejecting action '{}' because its factory threw during validation", typeId, t);
    ...
    private static AddonRegistrationResult reject(String ownerAddonId, String kind, String id, String reason) {
        AmpereClientAddon.LOG.warn("[MacroActions] Rejecting {} '{}': {}", kind, id, reason);
    ...
    private static String validateProbe(String typeId, MacroActionEntry entry, MacroAction probe, ActionFieldSchema schema) {
        ...
        try {
            ...
            AmpereClientAddon.LOG.warn("[MacroActions] Rejecting '{}' because toTag() threw", typeId, t);
        ...
        if (schema != null) {
            for (ampere.gui.macro.editor.FieldDef field : schema.fields()) {
                String key = field.key();
                if (field.type() == ampere.gui.macro.editor.FieldType.BLOCK_POS) {
