...
public class AmpereProtectorHttpUtilMixin {
    ...
    private static InputStream Ampere$blockLocal(
        ...
        if (!AmpereProtector.shouldBlockLocalUrls()) return original.call(instance);
    ...
    private static void rejectLocal(URL url) {
        ...
        if (AmpereProtectorLocalAddressUtil.shouldBlock(host)) {
            throw new IllegalStateException("[AmpereProtector] Refused resource pack download from local address: " + url);
        }
    }
