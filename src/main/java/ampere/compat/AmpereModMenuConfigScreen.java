...
import ampere.gui.screen.AmpereAddonsScreen;
...
public class AmpereModMenuConfigScreen extends Screen {
    ...
    public AmpereModMenuConfigScreen(Screen parent) {
        super(Component.literal("Ampere Client Settings"));
        this.parent = parent;
    }
    ...
    protected void init() {
        ...
        List<String> prefixes = new ArrayList<>(AmpereCompatManager.COMMAND_PREFIX_CHOICES);
        if (AmpereCompatManager.isMeteorAvailable()) prefixes.remove(".");
        String currentPrefix = AmpereCompatManager.effectiveCommandPrefix();
        ...
        addRenderableWidget(Button.builder(Component.literal("Addons"), b -> this.minecraft.setScreen(new AmpereAddonsScreen(this)))
            .bounds(x, y, w, h).build());
        y += h + gap;

        int half = (w - gap) / 2;
        addRenderableWidget(Button.builder(Component.literal("Donate"), b -> AmpereLinks.open(AmpereLinks.KOFI))
            .bounds(x, y, half, h).build());
        addRenderableWidget(Button.builder(Component.literal("Discord"), b -> AmpereLinks.open(AmpereLinks.DISCORD))
    ...
    private Component keybindLabel(int idx) {
        ...
        return Component.literal(k.label() + ": " + AmpereBindUtil.getBindName(k.getter().getAsInt()));
    }
    ...
    public boolean mouseClicked(MouseButtonEvent event, boolean doubled) {
        if (capturing >= 0) {
            applyCapture(AmpereBindUtil.encodeMouseButton(event.button()));
            return true;
        }
